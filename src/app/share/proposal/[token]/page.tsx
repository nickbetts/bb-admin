...
function PPCBarChart({
  months,
}: {
  months: Array<{ label: string; clicks: number; conversions: number }>;
}) {
      ...
      >
        {[headline1, headline2, headline3].filter(Boolean).map((h, i, arr) => (
          <span key={i} style={{ display: "inline-flex", alignItems: "baseline" }}>
            ...
            {i < arr.length - 1 && (
              <span style={{ fontSize: 20, color: "#1a0dab", margin: "0 4px" }}>|</span>
            )}
          </span>
        ))}
      </div>
...
function StickyNav({ clientName, token }: { clientName: string; token: string }) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 13, fontWeight: 700, color: "#a5b4fc", marginRight: "auto" }}>
          {clientName}
        </span>
          ...
          {NAV_SECTIONS.map((nav) => (
            <button
              ...
              style={{
                ...
                border: "none",
                color: activeSection === nav.id ? "#a5b4fc" : "var(--text-3)",
                padding: "6px 12px",
                borderRadius: 6,
                fontSize: 12,
                fontWeight: 600,
...
export default function ShareProposalPage({ params }: Props) {
  ...
  const ping = useCallback(() => {
    ...
    pinged.current = true;
    fetch(`/api/share/proposal/${token}/ping`, { method: "POST" }).catch(() => {
      /* silent */
    });
  }, [token]);
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: "linear-gradient(145deg, #0c1426 0%, #0f172a 40%, #131f38 72%, #0e1a31 100%)",
          color: "#fff",
      ...
      >
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          >
            {ai?.hero?.description ??
              `A comprehensive digital marketing strategy for ${meta.clientName}.`}
          </p>
      ...
      <div style={{ background: "#0f172a", padding: "3rem 24px" }}>
        ...
        >
          {(
            [
              {
                value: stats ? fmtNum(stats.totalKeywords) : String(topKws.length || "—"),
                label: "Keywords",
              },
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          >
            {ai?.whereYouAreNow?.summary ??
              `This proposal identifies the gaps holding ${meta.clientName} back and maps out the opportunities a focused digital strategy can unlock.`}
          </p>
          <div
            className="prop-obj-grid"
            ...
            {(
              [
                ...(ai?.whereYouAreNow?.gaps?.slice(0, 2).map((g, i) => ({
                  ...
                  {
                    badge: "Problem 01",
                    title: "Low visibility in organic search",
                    body: `${meta.clientName}'s potential clients are searching for your services right now. Without content ranking for those searches, the business is missing from the consideration set entirely.`,
                  },
                  {
                    badge: "Problem 02",
                    title: "Paid spend without a targeting framework",
                    body: "Generic ad campaigns generate clicks from a broad, largely irrelevant audience. Without intent mapping and structured targeting, budgets are wasted on people unlikely to convert.",
                  },
                ]),
                ...(ai?.whereYouAreNow?.positives?.slice(0, 2).map((p, i) => ({
                  ...
                  {
                    badge: "Opportunity 01",
                    title: "High-intent keywords with real search volume",
                    body: `Keyword research reveals significant search volume directly relevant to ${meta.clientName}'s services — many with manageable competition and strong commercial intent.`,
                  },
                  {
                    badge: "Opportunity 02",
                    title: "Content that compounds; paid media that converts",
                    body: "A structured content strategy builds long-term organic visibility while targeted paid campaigns deliver immediate traffic — compounding ROI from the same underlying research.",
                  },
                ]),
              ] as Array<{ badge: string; title: string; body: string }>
            ).map((card) => (
              <div
                key={card.badge}
                style={{
              ...
              >
                ...
                <h3
                  style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", margin: "0 0 8px" }}
                >
                  {card.title}
                </h3>
                ...
                >
                  {(ai.whereYouAreNow.positives ?? []).map((pos, i) => (
                    ...
                    >
                      ...
                      <p
                        style={{ fontSize: 13, color: "var(--text-2)", lineHeight: 1.6, margin: 0 }}
                      >
                        {pos.description}
                      </p>
                    </div>
      ...
      <div style={{ background: "#0f172a", padding: "80px 24px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          >
            Every element of this strategy — content, paid search, and keyword research — is built
            around the same underlying data. They reinforce each other rather than running in
            parallel.
          </p>
          <div
            className="prop-pillars"
          ...
          >
            ...
            >
              ...
              >
                Long-form content assets built around the keywords your ideal clients are actively
                searching. Pillar pages, guides, and articles designed to rank and convert.
              </p>
              {ai?.contentCluster && (
                <span
                  style={{
            ...
            >
              ...
              >
                Structured campaigns across Google Search and Meta, built around your target
                keywords. Ad groups mapped to specific intent with coverage for every stage of the
                buying journey.
              </p>
              <span
                style={{
              ...
              >
                {adGroups.length > 0 ? `${adGroups.length} ad groups` : "Google · Meta"} · 
                {fmtNum(ppcMetrics.clicks)} est. clicks/mo
              </span>
            </div>
            ...
            >
              ...
              >
                In-depth keyword research across every intent stage — informational, commercial, and
                transactional — so content and campaigns speak to clients wherever they are in their
                journey.
              </p>
              <span
                style={{
              ...
              >
                {stats
                  ? `${fmtNum(stats.totalKeywords)} keywords · ${fmtNum(stats.totalSearchVolume)}/mo searches`
                  : "Full keyword map"}
              </span>
            </div>
          </div>
      ...
      {services.length > 0 && (
        <div style={{ background: "var(--bg)", padding: "80px 24px" }}>
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            ...
            >
              This is not a strategy deck. Every campaign has been structured, every keyword list
              researched, and every content brief is ready to act on from day one.
            </p>
            <div
              className="prop-deliv-grid"
              style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}
            >
              {services.slice(0, 6).map((svc, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span>{svc.name}</span>
                      <span style={{ opacity: 0.75, fontWeight: 600, fontSize: 13 }}>
                        {svc.price}
                      </span>
                    </div>
                      ...
                      {rows.map((row, j) => (
                        <div
                          ...
                          style={{
                            ...
                            padding: "5px 0",
                            borderBottom:
                              j < rows.length - 1 ? "1px solid var(--border-subtle)" : "none",
                            fontSize: 13,
                            color: "var(--text)",
                          }}
                        >
      ...
      <Section id="ppc-forecaster">
        ...
        <p style={{ fontSize: 15, color: "var(--text-2)", lineHeight: 1.7, marginBottom: 40 }}>
          Use the sliders below to model different PPC scenarios. Adjust your CPC, monthly budget,
          and expected conversion rate to see how your campaigns could perform.
        </p>

        <div
          className="prop-forecast-grid"
        ...
        >
          ...
          >
            <h3 style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", margin: "0 0 24px" }}>
              Adjust Your Inputs
            </h3>
            <Slider
              label="Average CPC"
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            >
              ...
              <p style={{ fontSize: 40, fontWeight: 900, margin: 0, lineHeight: 1 }}>
                {fmtNum(ppcMetrics.clicks)}
              </p>
            </div>
            <div
              style={{
            ...
            >
              ...
              >
                {ppcMetrics.conversions > 0
                  ? fmtCurrency(Math.round(budget / ppcMetrics.conversions))
                  : "—"}
              </p>
            </div>
          </div>
        ...
        >
          <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", margin: "0 0 20px" }}>
            6-Month Performance Ramp
          </h3>
          <PPCBarChart months={ppcMetrics.months} />
          <p style={{ fontSize: 12, color: "var(--text-3)", marginTop: 12, textAlign: "center" }}>
            Campaigns typically ramp up over 6 months as optimisation improves Quality Scores and ad
            relevancy.
          </p>
        </div>
      </Section>
        ...
        <div style={{ background: "var(--bg)" }}>
          <Section id="ad-previews">
            ...
            <p style={{ fontSize: 15, color: "var(--text-2)", lineHeight: 1.7, marginBottom: 48 }}>
              Below are mockups of how your ads will appear across Google Search and Meta (Facebook
              &amp; Instagram). Each ad is built around your target keywords and campaign goals.
            </p>

            {/* Paid Media Channels Overview */}
            <div
              className="prop-channels"
              style={{
                display: "grid",
              ...
              {(
                [
                  {
                    ...
                    rows: [
                      [
                        "Ad groups",
                        adGroups.length > 0 ? String(adGroups.length) : "Structured",
                      ] as [string, string],
                      ["Match types", "Exact, Phrase, Broad"] as [string, string],
                      ["Intent", "Bottom-of-funnel"] as [string, string],
                      ["Keywords", stats ? fmtNum(stats.totalKeywords) : "Researched"] as [
                        string,
                        string,
                      ],
                    ],
                ...
                >
                  ...
                  >
                    <div
                      aria-hidden="true"
                      style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,.12)" }}
                    />
                    <span
                      style={{
                        fontSize: "1.6rem",
                    ...
                    <span style={{ fontSize: 12, opacity: 0.88, position: "relative" }}>
                      {ch.sub}
                    </span>
                  </div>
            ...
            <div style={{ marginBottom: 48 }}>
              ...
              >
                Your ads will appear at the top of Google Search results when potential customers
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {(adGroups.length > 0
                  ...
                  const descs = [
                    firstSentence ??
                      `${meta.clientName} provides expert ${group.name.toLowerCase()} services.`,
            ...
            <div>
              ...
              >
                Targeted social media ads designed to reach your ideal customers on Facebook and
                Instagram.
              </p>
      ...
      {services.length > 0 && (
        <div style={{ background: "var(--surface)" }}>
          <Section id="services">
            ...
            <p style={{ fontSize: 15, color: "var(--text-2)", lineHeight: 1.7, marginBottom: 40 }}>
              Each service includes a dedicated monthly allocation of hours. Use the sliders to
              explore how adjusting your hours allocation affects your package.
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {(
                serviceTotal ?? services.map((s, i) => ({ ...s, hours: serviceHours[i] ?? 0 }))
              ).map((svc, i) => (
                <div
                  key={i}
                  style={{
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ textAlign: "right" }}>
                      <p
                        style={{ fontSize: 18, fontWeight: 800, color: "var(--accent)", margin: 0 }}
                      >
                        {svc.price}
                      </p>
                      {svc.hours > 0 && (
                        <p style={{ fontSize: 12, color: "var(--text-3)", margin: "2px 0 0" }}>
                          {svc.hours}h/month allocated
                        </p>
                      )}
                    </div>
                  </div>
                  {svc.hours > 0 && (
                    <Slider
                      ...
                      format={(v) => `${v}h/mo`}
                      onChange={(v) =>
                        setServiceHours((prev) => prev.map((h, idx) => (idx === i ? v : h)))
                      }
                    />
                  )}
                </div>
      ...
      {(ai?.keywordClusters ?? []).length > 0 && (
        <Section id="keywords">
          ...
          <p style={{ fontSize: 15, color: "var(--text-2)", lineHeight: 1.7, marginBottom: 32 }}>
            We&apos;ve grouped your target keywords by search intent to ensure campaigns reach the
            right audiences at each stage of the buying journey.
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ fontSize: 13, fontWeight: 600, color: "var(--accent)" }}>
                        {fmtNum(cluster.searchVolume)}/mo searches
                      </span>
                    </div>
                    <p
                      style={{
          ...
          <Section id="content">
            ...
            >
              ...
              <h3 style={{ fontSize: 20, fontWeight: 800, margin: "0 0 8px" }}>
                {ai.contentCluster.pillarPage.title}
              </h3>
              <p style={{ fontSize: 14, color: "#c4b5fd", margin: 0, lineHeight: 1.6 }}>
                {ai.contentCluster.pillarPage.description}
              </p>
            </div>
              ...
              {(
                [
                  ...
                  },
                  {
                    type: "Articles",
                    isPillar: false,
                    count: ai.contentCluster.articles.length,
                    countLabel: "articles",
                  ...
                  title: string;
                  desc: string;
                  chips: Array<{ label: string; color: string; bg: string; border: string }>;
                }>
              ).map((row, i, arr) => (
                <div
                  key={i}
                  className="prop-arch-row"
                  style={{
                    display: "grid",
              ...
              <tbody>
                {topKws.slice(0, 15).map((kw, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? "var(--surface)" : "var(--bg)" }}>
                    ...
                    <td style={{ padding: "10px 16px", color: "var(--accent)", fontWeight: 600 }}>
                      {fmtNum(kw.avgMonthlySearches)}
                    </td>
                    ...
                    <td style={{ padding: "10px 16px", color: "var(--text-2)" }}>
                      {kw.highTopOfPageBidMicros
                        ? `£${(kw.highTopOfPageBidMicros / 1_000_000).toFixed(2)}`
                        : "—"}
                    </td>
                  </tr>
                ))}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 4 }}>
              {(ai!.whyUs ?? []).map((item, i) => (
                <div key={i} style={{ textAlign: "center", padding: "32px 24px" }}>
                  ...
                  <p
                    style={{ fontSize: 15, fontWeight: 700, color: "#c7d2fe", margin: "0 0 10px" }}
                  >
                    {item.title}
                  </p>
                  <p
      ...
      <div
        id="contact"
        style={{ background: "linear-gradient(135deg, #0f0c29, #1e1b4b)", padding: "80px 24px" }}
      >
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 14, marginBottom: 56 }}>
            ...
            {(
              [
                {
                  icon: "📊",
                  title: "PPC Forecast",
                  desc: "Model performance scenarios",
                  href: "ppc-forecaster",
                },
                {
                  icon: "🎨",
                  title: "Ad Mockups",
                  desc: "See your ads in action",
                  href: "ad-previews",
                },
                {
                  icon: "📄",
                  title: "Content Plan",
                  desc: "Topic clusters & article briefs",
                  href: "content",
                },
                {
                  icon: "🔍",
                  title: "Keyword Research",
                  desc: "Target keywords & intent map",
                  href: "keywords",
                },
              ] as Array<{ icon: string; title: string; desc: string; href: string }>
            ).map((card) => (
              <button
                key={card.title}
                onClick={() => {
              ...
              >
                ...
                <div>
                  <strong
                    style={{ display: "block", fontSize: 14, fontWeight: 700, marginBottom: 2 }}
                  >
                    {card.title}
                  </strong>
                ...
                {[
                  {
                    ...
                    desc: "30-minute strategy session — no commitment",
                  },
                  {
                    icon: "📊",
                    title: "Custom roadmap",
                    desc: "We tailor the plan to your timeline and budget",
                  },
                  {
                    icon: "🚀",
                    title: "Fast onboarding",
                    desc: "Campaigns live within 2 weeks of sign-off",
                  },
                ].map((item) => (
                  <div
                    key={item.title}
          ...
          >
            {[
              { Icon: Mail, text: "nick@bettsandburton.com" },
