...
export default async function CronPage() {
  ...
  if (!session.user.permissions.includes("admin.cron")) redirect("/admin");
    ...
    <div className="page">
      <div className="mb-8">
        ...
        <p style={{ color: "var(--text-3)", fontSize: 14, marginTop: 6 }}>
          Monitor the nightly data pipeline, view per-client snapshot coverage, and trigger manual
          runs.
        </p>
      </div>
      <AdminNav active="cron" permissions={session.user.permissions} />
      <CronDashboard />
    </div>
  );
}
