...
const features = [
  { icon: <Library size={20} />, title: "Template library", desc: "Pre-built templates for ecommerce, education, professional services, healthcare, hospitality, and more. Each tuned to the language AI models expect." },
  { icon: <Tag size={20} />, title: "Sector tagging", desc: "Tag your output with sector-specific keywords so AI models can classify your business correctly. Finance, education, charity, SaaS, retail, and others." },
  { icon: <Globe size={20} />, title: "Website crawling", desc: "Point the generator at a URL and it reads page content, headings, meta data, and structure. No manual copy-pasting required." },
  { icon: <Settings size={20} />, title: "Customisable prompts", desc: "Adjust the generation prompt to match your brand voice. Add specific instructions, exclude topics, or emphasise particular services." },
  { icon: <Copy size={20} />, title: "One-click copy", desc: "Copy the generated output to your clipboard with a single click. Paste it into your llms.txt file, documentation, or knowledge base." },
  { icon: <Download size={20} />, title: "Download and export", desc: "Download the output as a .txt or .md file. Ready to deploy to your website root or include in your project documentation." },
];
...
export default function LlmGeneratorPage() {
  ...
  return (
    <div style={{ background: "#09090f", color: "white", fontFamily: "inherit" }}>
      <div style={{ position: "fixed", pointerEvents: "none", zIndex: 1, width: 600, height: 600, borderRadius: "50%", left: mouse.x - 300, top: mouse.y - 300, background: `radial-gradient(circle, rgba(99,102,241,0.06) 0%, transparent 65%)`, transition: "left 0.2s ease-out, top 0.2s ease-out" }} />
      ...
      <nav style={{ position: "fixed", right: 24, top: "50%", transform: "translateY(-50%)", zIndex: 40, display: "flex", flexDirection: "column", gap: 1, background: "rgba(9,9,15,0.88)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "10px 6px", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)" }} className="side-nav">
        ...
        <div style={{ width: "100%", height: 44, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <div style={{ width: 2, height: 32, background: "rgba(255,255,255,0.08)", borderRadius: 2, position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: `${scrollPct}%`, background: `linear-gradient(180deg, ${accent}, #4f46e5)`, transition: "height 0.1s linear", borderRadius: 2 }} />
          </div>
        </div>
      </nav>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", background: "rgba(9,9,15,0.88)", borderBottom: "1px solid rgba(255,255,255,0.06)", height: 64, display: "flex", alignItems: "center", padding: "0 40px", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(255,255,255,0.65)" }}>LLM Generator</span>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <a href="#cta" onClick={(e) => { e.preventDefault(); document.getElementById("cta")?.scrollIntoView({ behavior: "smooth" }); }} style={{ padding: "9px 20px", borderRadius: 8, background: `linear-gradient(135deg, ${accent}, #4f46e5)`, color: "white", fontSize: 13, fontWeight: 600, textDecoration: "none", boxShadow: `0 0 20px rgba(99,102,241,0.3)` }} className="cta-accent-pulse">Talk to us →</a>
        </div>
      </nav>

      {/* ── HERO ── */}
      <section style={{ minHeight: "100vh", paddingTop: 64, position: "relative", overflow: "hidden", display: "flex", alignItems: "center" }}>
        <div style={{ position: "absolute", width: "65%", paddingBottom: "65%", top: "-15%", left: "-15%", pointerEvents: "none", borderRadius: "50%", background: `radial-gradient(circle, rgba(99,102,241,0.22) 0%, transparent 65%)`, transform: `translateY(${parallaxY * -0.25}px)` }} className="orb-1" />
        <div style={{ position: "absolute", width: "55%", paddingBottom: "55%", bottom: "-15%", right: "-10%", pointerEvents: "none", borderRadius: "50%", background: `radial-gradient(circle, rgba(79,70,229,0.20) 0%, transparent 65%)`, transform: `translateY(${parallaxY * -0.15}px)` }} className="orb-2" />
        ...
        {particles.map((p) => (
          <div key={p.id} className="hero-particle" style={{ position: "absolute", top: p.top, left: p.left, pointerEvents: "none", width: p.size, height: p.size, borderRadius: "50%", background: p.id % 3 === 0 ? `rgba(99,102,241,0.9)` : p.id % 3 === 1 ? "rgba(79,70,229,0.9)" : "rgba(255,255,255,0.8)", opacity: p.opacity, animationDuration: p.dur, animationDelay: p.delay }} />
        ))}
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "80px 40px", display: "grid", gridTemplateColumns: "1fr 480px", gap: 72, alignItems: "center", width: "100%", position: "relative", boxSizing: "border-box" }} className="hero-grid">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 28, flexWrap: "wrap" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 20, background: `rgba(99,102,241,0.12)`, border: `1px solid rgba(99,102,241,0.3)` }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, color: accentLight, letterSpacing: "0.08em", textTransform: "uppercase" }}>AI Visibility</span>
              </div>
            </div>
            <h1 style={{ fontSize: 56, fontWeight: 900, lineHeight: 1.05, letterSpacing: "-0.04em", marginBottom: 28, color: "white" }}>
              ...
              <br />
              <span className="hw hw2" style={{ background: `linear-gradient(90deg, ${accentLight}, ${accent})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>visible to AI.</span>
            </h1>
            <p style={{ fontSize: 18, color: "rgba(255,255,255,0.6)", lineHeight: 1.8, maxWidth: 500, marginBottom: 32, fontWeight: 400 }}>
              Generate sector-specific llms.txt context files so AI models understand your business. Pick a template, customise the output, and deploy in minutes.
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 36 }}>
              {[
                "6 sector templates ready to use",
                "Website crawling extracts page content automatically",
                "Customisable prompts to match brand voice",
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <a href="#mockup" onClick={(e) => { e.preventDefault(); document.getElementById("mockup")?.scrollIntoView({ behavior: "smooth" }); }} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 24px", borderRadius: 10, background: `linear-gradient(135deg, ${accent}, #4f46e5)`, color: "white", fontSize: 14, fontWeight: 600, textDecoration: "none", boxShadow: `0 0 24px rgba(99,102,241,0.35)` }} className="cta-accent-pulse">
          ...
          <div style={{ position: "relative" }}>
            <div style={{ position: "absolute", inset: -40, borderRadius: "50%", background: `radial-gradient(circle, rgba(99,102,241,0.15) 0%, transparent 70%)`, pointerEvents: "none" }} />
            <div className="mockup-3d" style={{ background: "rgba(255,255,255,0.04)", border: `1px solid rgba(99,102,241,0.2)`, borderRadius: 20, overflow: "hidden", position: "relative" }}>
              <div style={{ padding: "18px 22px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", background: `rgba(99,102,241,0.05)` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "white" }}>LLM.txt Generator</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 1 }}>Choose a sector template</div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ padding: "16px 22px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                  {[
                    { sector: "Ecommerce", desc: "Product catalogue, pricing, shipping", active: false },
                    { sector: "Education", desc: "Courses, programmes, admissions", active: true },
                    { sector: "Professional Services", desc: "Team, expertise, case studies", active: false },
                    { sector: "Healthcare", desc: "Services, conditions, practitioners", active: false },
                    { sector: "Hospitality", desc: "Venues, menus, booking info", active: false },
                    { sector: "Charity / Non-Profit", desc: "Mission, impact, donation info", active: false },
                  ].map((t, i) => (
                    <div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: t.active ? `rgba(99,102,241,0.12)` : "rgba(255,255,255,0.03)", border: t.active ? `1px solid rgba(99,102,241,0.35)` : "1px solid rgba(255,255,255,0.06)", cursor: "pointer" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                        <div style={{ width: 8, height: 8, borderRadius: "50%", background: t.active ? accent : "rgba(255,255,255,0.1)", border: t.active ? `2px solid ${accentLight}` : "2px solid rgba(255,255,255,0.15)" }} />
                        <span style={{ fontSize: 12, fontWeight: 700, color: t.active ? "white" : "rgba(255,255,255,0.5)" }}>{t.sector}</span>
                      </div>
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", paddingLeft: 16 }}>{t.desc}</div>
                    </div>
                  ))}
                </div>
                <div style={{ marginTop: 12, padding: "10px 14px", borderRadius: 8, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 8 }}>
                  <Globe size={12} color="rgba(255,255,255,0.3)" />
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", flex: 1 }}>brightsparks.co.uk</span>
                  <span style={{ fontSize: 10, color: "#10b981", fontWeight: 600 }}>Crawled</span>
                </div>
              </div>

              <div style={{ padding: "12px 22px 18px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ padding: "8px 14px", borderRadius: 8, background: `linear-gradient(135deg, ${accent}, #4f46e5)`, fontSize: 11, fontWeight: 700, color: "white" }}>Generate</div>
                <div style={{ display: "flex", gap: 6 }}>
                  <div style={{ padding: "6px 10px", borderRadius: 6, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", fontSize: 10, color: "rgba(255,255,255,0.35)", display: "flex", alignItems: "center", gap: 4 }}><Copy size={10} /> Copy</div>
                  <div style={{ padding: "6px 10px", borderRadius: 6, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", fontSize: 10, color: "rgba(255,255,255,0.35)", display: "flex", alignItems: "center", gap: 4 }}><Download size={10} /> .txt</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ── STATS ── */}
      <section id="stats-row" className="reveal-section" style={{ padding: "80px 40px", background: `linear-gradient(180deg, rgba(99,102,241,0.04) 0%, rgba(79,70,229,0.03) 100%)`, borderTop: "1px solid rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="stats-grid">
            {[
              { val: s1, suffix: " templates", label: "Sector-specific templates for ecommerce, education, services, and more", note: "Ready to customise", color: accent },
              { val: "Auto", suffix: " crawl", label: "Point it at a URL and the generator reads content, headings, and meta tags", note: "No manual extraction", color: "#4f46e5", isText: true },
              { val: "1", suffix: "-click", label: "Copy the output or download as a file. Ready to deploy to your site.", note: "Instant export", color: "#10b981" },
            ].map((s, i) => (
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.45)", maxWidth: 560, margin: "0 auto", lineHeight: 1.7 }}>
              Pick a sector template, point the crawler at the website, customise the prompt, and generate a context file that AI models can read and understand.
            </p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="features-grid">
            {features.map((f, i) => (
              <div key={i} className="feature-card stagger-in" style={{ padding: "28px 24px", borderRadius: 16, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", animationDelay: `${i * 0.08}s`, transition: "transform 0.35s ease, border-color 0.3s ease, box-shadow 0.3s ease" }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: `rgba(99,102,241,0.12)`, border: `1px solid rgba(99,102,241,0.25)`, display: "flex", alignItems: "center", justifyContent: "center", color: accentLight, marginBottom: 16 }}>{f.icon}</div>
      ...
      <section id="mockup" className="reveal-section" style={{ padding: "120px 40px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: accentLight, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 16 }} className="blur-reveal">Generated output</p>
          ...
          <div className="mockup-3d" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid rgba(99,102,241,0.15)`, borderRadius: 20, overflow: "hidden", position: "relative" }}>
            <div style={{ padding: "18px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>brightsparks.co.uk</span>
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                <div style={{ padding: "5px 10px", borderRadius: 5, background: `rgba(99,102,241,0.12)`, border: `1px solid rgba(99,102,241,0.25)`, fontSize: 10, fontWeight: 600, color: accentLight, display: "flex", alignItems: "center", gap: 4 }}><Copy size={10} /> Copy</div>
                <div style={{ padding: "5px 10px", borderRadius: 5, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", fontSize: 10, color: "rgba(255,255,255,0.35)", display: "flex", alignItems: "center", gap: 4 }}><Download size={10} /> Download</div>
              </div>
            </div>
            <div style={{ padding: "20px 24px", fontFamily: "'SF Mono', 'Fira Code', 'Consolas', monospace", fontSize: 12, color: "rgba(255,255,255,0.5)", lineHeight: 1.8, whiteSpace: "pre-wrap", maxHeight: 280, overflow: "hidden", position: "relative" }}>
              <span style={{ color: accentLight }}>## BrightSparks Academy</span>
              {"\n\n"}
              <span style={{ color: "rgba(255,255,255,0.35)" }}>{">"} Education provider specialising in football summer camps, youth development programmes, and coaching courses for ages 6-16.</span>
              {"\n\n"}
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 60, background: "linear-gradient(transparent, rgba(9,9,15,0.95))" }} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="cta" className="reveal-section" style={{ padding: "120px 40px 140px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", width: "80%", paddingBottom: "40%", bottom: "-20%", left: "50%", transform: "translateX(-50%)", background: `radial-gradient(ellipse, rgba(99,102,241,0.12) 0%, transparent 65%)`, pointerEvents: "none" }} />
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center", position: "relative" }}>
          ...
          <h2 style={{ fontSize: 50, fontWeight: 900, letterSpacing: "-0.04em", marginBottom: 20, color: "white", lineHeight: 1.05 }} className="blur-reveal">
            Be found by AI.<br />
            <span style={{ background: `linear-gradient(90deg, ${accentLight}, ${accent}, #4f46e5)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Not just search engines.</span>
          </h2>
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.55)", lineHeight: 1.8, marginBottom: 40 }}>
            AI models are the new discovery layer. Give them structured context about your business and services. The LLM Generator makes it straightforward.
          </p>
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            <a href="/login" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "15px 28px", borderRadius: 12, background: `linear-gradient(135deg, ${accent}, #4f46e5)`, color: "white", fontSize: 15, fontWeight: 700, textDecoration: "none", boxShadow: `0 0 32px rgba(99,102,241,0.4)` }} className="cta-accent-pulse">
          ...
          <p style={{ marginTop: 24, fontSize: 12, color: "rgba(255,255,255,0.25)" }}>LLM Generator is included with every StratOS plan.</p>
        </div>
      </section>
