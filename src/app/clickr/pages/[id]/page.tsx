...
export default function ClickrPageDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const brief = (() => {
    try {
      return JSON.parse(page.briefJson) as { url?: string; brief?: string; campaignType?: string };
    } catch {
      return {};
    }
  })();
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "32px 24px" }}>
        ...
        >
          ...
          >
            {brief.url && (
              <div>
                <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 12 }}>Source URL</span>
                <br />
                <span style={{ fontSize: 13 }}>{brief.url}</span>
              </div>
            )}
            {brief.campaignType && (
              <div>
                <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 12 }}>Campaign type</span>
                <br />
                <span style={{ fontSize: 13 }}>{brief.campaignType}</span>
              </div>
            )}
            {brief.brief && (
              <div style={{ gridColumn: "1 / -1" }}>
                <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 12 }}>Brief</span>
                <br />
                <span style={{ fontSize: 13 }}>{brief.brief}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {page.publicSlug ? (
          ...
          >
            ...
            >
              <span style={{ color: "rgba(255,255,255,0.2)" }}>●</span>
              <span>lp.bettsandburton.com/lp/{page.publicSlug}</span>
              <a
                href={`/lp/${page.publicSlug}`}
                target="_blank"
