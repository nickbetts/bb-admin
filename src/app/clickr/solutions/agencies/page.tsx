...
import {
  Zap, Users, Globe, GitBranch, MessageSquare, BarChart2,
  Layers, Bell, Webhook, FileText, CheckCircle, ArrowRight,
  Clock, Star, Shield,
} from "lucide-react";
...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "https://clickr.marketing/solutions/agencies" },
  openGraph: {
    title: "clickr for Agencies | Scale Landing Page Production",
    description: "Build landing pages for every client campaign in under 60 seconds. CRM integrations, white-label subdomains, multi-client management.",
    url: "https://clickr.marketing/solutions/agencies",
  },
};
...
const features = [
  ...
  },
  {
    ...
    body: "Publish to client-branded subdomains like `summer-sale.clickr.marketing`. Custom domain support coming soon.",
  },
  {
    icon: Users,
...
const workflow = [
  { step: "01", title: "Paste the URL", body: "Drop in your client's website URL. Meridian scrapes it — brand colours, fonts, logo, testimonials, service descriptions, the lot." },
  { step: "02", title: "Write the brief", body: "Campaign type, target audience, offer. 2 sentences is enough. Meridian handles the rest." },
  { step: "03", title: "Let AI build it", body: "Three passes: initial generation → CRO audit → design & copy refinement. Done in under 60 seconds." },
  { step: "04", title: "Refine with chat", body: "\"Move the form above the fold\" — done. \"Add social proof counters\" — done. No code required." },
  { step: "05", title: "Share for review", body: "One-click magic link. Client reviews, approves. You publish." },
  { step: "06", title: "Track and optimise", body: "Leads flow into your CRM, conversion events fire to your ad accounts. A/B variants ready when you need them." },
];
...
export default function AgenciesPage() {
  return (
    <>
      <ClickrNav />
      <main style={{ background: "#09090f", color: "white", fontFamily: "system-ui, -apple-system, sans-serif", paddingTop: 64 }}>
        ...
        <section style={{ position: "relative", overflow: "hidden", padding: "96px 24px 80px", textAlign: "center" }}>
          ...
          <div style={{ maxWidth: 760, margin: "0 auto", position: "relative" }}>
            ...
            <h1 style={{
              ...
              letterSpacing: "-0.035em", lineHeight: 1.08, margin: "0 0 24px",
            }}>
            ...
            <p style={{ fontSize: "clamp(16px, 2.2vw, 20px)", lineHeight: 1.6, color: "rgba(255,255,255,0.55)", margin: "0 0 40px", maxWidth: 600, marginLeft: "auto", marginRight: "auto" }}>
              Stop losing hours on briefs, design handoffs, and copy revisions. clickr gives your agency a repeatable, AI-powered pipeline — from client brief to published, CRO-audited page in under 60 seconds.
            </p>
            <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
              ...
              <Link href="/pricing" style={{
                ...
                color: "rgba(255,255,255,0.75)", fontSize: 15, fontWeight: 600, textDecoration: "none",
              }}>
                View pricing
              </Link>
            </div>
        ...
        <section style={{ padding: "72px 24px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            <p style={{ textAlign: "center", fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(255,255,255,0.28)", marginBottom: 48 }}>
              THE PROBLEMS WE SOLVE
            </p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }} className="csol-3col">
              {painPoints.map(({ icon: Icon, headline, body }) => (
                <div key={headline} style={{
                  ...
                  <h3 style={{ fontSize: 16, fontWeight: 700, margin: "0 0 10px", color: "rgba(255,255,255,0.9)" }}>{headline}</h3>
                  <p style={{ fontSize: 14, lineHeight: 1.65, color: "rgba(255,255,255,0.45)", margin: 0 }}>{body}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section style={{ padding: "72px 24px", background: "rgba(255,255,255,0.015)", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            <p style={{ textAlign: "center", fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(255,255,255,0.28)", marginBottom: 12 }}>
              HOW IT WORKS
            </p>
            <h2 style={{ textAlign: "center", fontSize: "clamp(28px, 4vw, 42px)", fontWeight: 800, letterSpacing: "-0.03em", margin: "0 0 56px" }}>
              ...
              {workflow.map(({ step, title, body }, i) => (
                <div key={step} style={{
                  ...
                  <div style={{ paddingLeft: 20 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, margin: "4px 0 8px", color: "rgba(255,255,255,0.9)" }}>{title}</h3>
                    <p style={{ fontSize: 14, lineHeight: 1.65, color: "rgba(255,255,255,0.45)", margin: 0 }}>{body}</p>
                  </div>
                </div>
              ))}
        ...
        <section style={{ padding: "72px 24px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            <h2 style={{ textAlign: "center", fontSize: "clamp(28px, 4vw, 42px)", fontWeight: 800, letterSpacing: "-0.03em", margin: "0 0 12px" }}>
              Built for agency workflows
            </h2>
            <p style={{ textAlign: "center", fontSize: 16, color: "rgba(255,255,255,0.45)", margin: "0 0 56px", maxWidth: 520, marginLeft: "auto", marginRight: "auto" }}>
              Every feature is designed around the reality of managing paid campaigns across multiple clients.
            </p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="csol-3col">
              {features.map(({ icon: Icon, title, body }) => (
                <div key={title} style={{
                  ...
                  <h3 style={{ fontSize: 14, fontWeight: 700, margin: "0 0 8px", color: "rgba(255,255,255,0.88)" }}>{title}</h3>
                  <p style={{ fontSize: 13, lineHeight: 1.65, color: "rgba(255,255,255,0.42)", margin: 0 }}>{body}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section style={{ padding: "80px 24px", textAlign: "center", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <div style={{
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.5)", margin: "0 0 32px", lineHeight: 1.6 }}>
              Start free. No credit card. No lock-in. Build your first client landing page in the next 60 seconds.
            </p>
            <Link href="/signup" style={{
              display: "inline-flex", alignItems: "center", gap: 8,
