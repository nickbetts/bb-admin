...
export default function NewGrandPlanPage() {
  ...
  useEffect(() => {
    if (clientId) {
      ...
      if (client && !title) {
        setTitle(`${client.name} — Go-to-Market Plan`);
      }
    }
  ...
  return (
    <div className="page" style={{ maxWidth: 720 }}>
      ...
      {clientId && (
        <div style={sectionStyle}>
          <h3 style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", marginBottom: 16 }}>Link Existing Records</h3>
          {loadingSources ? (
            ...
            <p style={{ fontSize: 13, color: "var(--text-4)" }}>Select a client to see available records.</p>
          )}
        </div>
      )}
