...
const CONTENT_TYPES: { value: ContentType; label: string; description: string; colour: string }[] = [
  { value: "blog", label: "Blog Article", description: "1,000–1,500 words, SEO-optimised, H2 structure", colour: "#2563eb" },
  { value: "whitepaper", label: "Whitepaper", description: "2,000–3,000 words, authoritative research report", colour: "#7c3aed" },
  { value: "case_study", label: "Case Study", description: "800–1,200 words, Challenge → Approach → Results", colour: "#059669" },
  { value: "social", label: "Social Media Copy", description: "LinkedIn, Instagram, Facebook, X and TikTok variants", colour: "#ea580c" },
];
...
export default function NewContentGeneratorPage() {
  ...
  function handleClientChange(id: string) {
    ...
    if (c?.website) setWebsiteUrl(c.semrushDomain ?? c.website);
  }
  ...
  async function handleStartResearch() {
    ...
    try {
      ...
      const createRes = await fetch("/api/tools/content-generator", {
        ...
        body: JSON.stringify({ clientId, brief, contentTypes: selectedTypes, websiteUrl: websiteUrl || undefined }),
      });
      if (!createRes.ok) {
  ...
  const runResearch = useCallback(async (id: string, extraCompetitors?: string[]) => {
    ...
    try {
      ...
      setCompetitorEntries(
        (data.competitors ?? []).map((c) => ({ domain: c.domain, status: "valid" as const, commonKeywords: c.commonKeywords })),
      );
    } catch (err) {
      clearInterval(pollInterval);
  ...
  async function addCompetitor() {
    const raw = competitorInput.trim().toLowerCase().replace(/^https?:\/\//, "").replace(/^www\./, "").replace(/\/$/, "");
    ...
    try {
      ...
      setCompetitorEntries((prev) =>
        prev.map((e) => e.domain === raw ? { ...e, status: data.commonKeywords > 0 ? "valid" : "no-overlap", commonKeywords: data.commonKeywords } : e),
      );
      setCompetitors((prev) => [...prev.filter((c) => c.domain !== raw), { domain: raw, commonKeywords: data.commonKeywords }]);
    } catch {
      setCompetitorEntries((prev) =>
  ...
  function toggleKeyword(ideaId: string, field: "primaryKeyword" | "secondaryKeywords" | "longTailKeywords", index: number) {
    setIdeas((prev) =>
      prev.map((idea) => {
        if (idea.id !== ideaId) return idea;
        if (field === "primaryKeyword") {
          return { ...idea, primaryKeyword: { ...idea.primaryKeyword, approved: !idea.primaryKeyword.approved } };
        }
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      await fetch(`/api/tools/content-generator/${recordId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ideasJson: JSON.stringify(ideas), selectedIdeasJson: JSON.stringify(ideas) }),
      });
      ...
      const res = await fetch(`/api/tools/content-generator/${recordId}/generate`, { method: "POST" });
      if (!res.ok) {
        const err = (await res.json()) as { error: string };
        throw new Error(err.error);
      }
  ...
  return (
    <div className="page" style={{ maxWidth: 860 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32 }}>
        ...
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "var(--text)", lineHeight: 1 }}>New Content Pack</h1>
          <p style={{ fontSize: 13, color: "var(--text-3)", marginTop: 4 }}>AI-generated content with SEMrush keyword research</p>
        </div>
      </div>
          ...
          <div key={s.n} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{
                ...
                background: step > s.n ? "var(--success)" : step === s.n ? "var(--accent)" : "var(--bg-2)",
                color: step >= s.n ? "white" : "var(--text-3)",
              }}>
                {step > s.n ? <Check style={{ width: 12, height: 12 }} /> : s.n}
              </div>
              <span style={{ fontSize: 13, fontWeight: step === s.n ? 600 : 400, color: step === s.n ? "var(--text)" : "var(--text-3)" }}>
                {s.label}
              </span>
            </div>
          </div>
      ...
      {step === 1 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                ...
                return (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <div style={{ width: 8, height: 8, borderRadius: "50%", background: active ? t.colour : "var(--border)", transition: "background .15s" }} />
                      <span style={{ fontSize: 13, fontWeight: 700, color: active ? t.colour : "var(--text)" }}>{t.label}</span>
                    </div>
                    <p style={{ margin: 0, fontSize: 11, color: "var(--text-3)", lineHeight: 1.4 }}>{t.description}</p>
                  </button>
                );
              })}
            </div>
          ...
          {researchError && (
            <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "12px 16px", background: "var(--danger-bg)", borderRadius: 8, color: "var(--danger)", fontSize: 13 }}>
          ...
          <div>
            ...
            >
              {creating ? <Loader2 style={{ width: 15, height: 15, animation: "spin 1s linear infinite" }} /> : null}
            ...
            <p style={{ marginTop: 8, fontSize: 12, color: "var(--text-3)" }}>
              Runs SEMrush keyword research + competitor analysis + Claude Opus idea generation (~30–60 seconds)
            </p>
          </div>
        </div>
      )}
        ...
        <div>
          ...
          {researching && (
            <div style={{ textAlign: "center", padding: "48px 0" }}>
              <Loader2 style={{ width: 32, height: 32, color: "var(--accent)", animation: "spin 1s linear infinite", margin: "0 auto 16px" }} />
              <p style={{ fontSize: 16, fontWeight: 600, color: "var(--text)", marginBottom: 8 }}>{researchStatus || "Running research…"}</p>
              <p style={{ fontSize: 13, color: "var(--text-3)" }}>Gathering keyword data, analysing competitors and generating ideas with Claude Opus</p>
            </div>
          )}

          {!researching && researchError && (
            <div style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "12px 16px", background: "var(--danger-bg)", borderRadius: 8, color: "var(--danger)", fontSize: 13, marginBottom: 12 }}>
                <AlertCircle style={{ width: 16, height: 16, flexShrink: 0, marginTop: 1 }} />
                {researchError}
              </div>
            ...
            <>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                  {competitorEntries.map((c) => (
                    <div key={c.domain} style={{
                      display: "inline-flex", alignItems: "center", gap: 8, padding: "4px 10px", borderRadius: 20,
                      background: c.status === "valid" ? "var(--success-bg)" : c.status === "no-overlap" ? "var(--warning-bg)" : c.status === "invalid" ? "var(--danger-bg)" : "var(--bg-2)",
                      ...
                      <span style={{ fontSize: 12, fontWeight: 500, color: "var(--text)" }}>{c.domain}</span>
                      {c.commonKeywords !== undefined && (
                        <span style={{ fontSize: 11, color: "var(--text-3)" }}>({c.commonKeywords} shared kws)</span>
                      )}
                      {c.status === "checking" && <Loader2 style={{ width: 11, height: 11, animation: "spin 1s linear infinite", color: "var(--text-3)" }} />}
                      <button onClick={() => removeCompetitor(c.domain)} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, color: "var(--text-3)", display: "flex" }}>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <p style={{ margin: "0 0 24px", fontSize: 13, color: "var(--text-3)" }}>
                  Tick the ideas you want to generate. Click keyword pills to approve or reject them — only approved keywords will be used.
                </p>

                {ideasByType.map(({ value: type, label, colour, ideas: typeIdeas }) => (
                  typeIdeas.length > 0 && (
                    <div key={type} style={{ marginBottom: 32 }}>
                      <h3 style={{ margin: "0 0 12px", fontSize: 14, fontWeight: 700, color: colour }}>
                        {label}
                      </h3>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        {typeIdeas.map((idea) => (
                          <IdeaCard
                            ...
                            onToggleSecondary={(i) => toggleKeyword(idea.id, "secondaryKeywords", i)}
                            onToggleLongTail={(i) => toggleKeyword(idea.id, "longTailKeywords", i)}
                          />
                        ))}
                      </div>
                    </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, paddingTop: 16, borderTop: "1px solid var(--border)" }}>
                ...
                <button
                  ...
                  style={{ display: "inline-flex", alignItems: "center", gap: 8 }}
                >
                  {savingSelections ? <Loader2 style={{ width: 14, height: 14, animation: "spin 1s linear infinite" }} /> : null}
                  {savingSelections ? "Starting…" : `Generate ${selectedCount} Piece${selectedCount !== 1 ? "s" : ""}`}
      ...
      {step === 3 && (
        <div style={{ textAlign: "center", padding: "48px 0" }}>
          {generating ? (
            <>
              <Loader2 style={{ width: 40, height: 40, color: "var(--accent)", animation: "spin 1s linear infinite", margin: "0 auto 20px" }} />
              <p style={{ fontSize: 18, fontWeight: 700, color: "var(--text)", marginBottom: 8 }}>Generating content…</p>
              <p style={{ fontSize: 14, color: "var(--text-3)", maxWidth: 400, margin: "0 auto" }}>
                Claude Opus is writing your {selectedCount} piece{selectedCount !== 1 ? "s" : ""}. This takes 1–3 minutes depending on length.
              </p>
            </>
          ) : generationError ? (
            <>
              <AlertCircle style={{ width: 40, height: 40, color: "var(--danger)", margin: "0 auto 20px" }} />
              <p style={{ fontSize: 18, fontWeight: 700, color: "var(--text)", marginBottom: 8 }}>Generation failed</p>
              <p style={{ fontSize: 14, color: "var(--text-3)", marginBottom: 20 }}>{generationError}</p>
              {recordId && (
                <button className="btn btn-primary" onClick={() => { setGenerationError(null); setGenerating(true); handleGenerate(); }}>
                  Retry
                </button>
              )}
            </>
  ...
  return (
    ...
    >
      ...
      >
        {/* Checkbox */}
        <div style={{
          width: 20, height: 20, borderRadius: 6, border: `2px solid ${idea.selected ? colour : "var(--border)"}`,
          background: idea.selected ? colour : "transparent", display: "flex", alignItems: "center", justifyContent: "center",
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text)" }}>{idea.title}</span>
            <span style={{
              fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 10, textTransform: "uppercase", letterSpacing: ".06em",
          ...
          <p style={{ margin: 0, fontSize: 12, color: "var(--text-3)", lineHeight: 1.5 }}>{idea.summary}</p>
        </div>
        ...
        >
          <ChevronRight style={{ width: 16, height: 16, transform: expanded ? "rotate(90deg)" : "none", transition: "transform .15s" }} />
        </button>
      </div>
      ...
      {expanded && (
        <div style={{ padding: "0 16px 16px", borderTop: "1px solid var(--border)" }}>
          <p style={{ margin: "12px 0 8px", fontSize: 11, fontWeight: 700, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: ".08em" }}>
            Primary keyword
          </p>
          <KeywordPill
  ...
  return (
    <button
      ...
      style={{
        ...
        cursor: "pointer", fontSize: primary ? 13 : 12, fontWeight: approved ? 600 : 400,
        transition: "all .12s",
      }}
    >
      {approved ? <Check style={{ width: 10, height: 10 }} /> : <X style={{ width: 10, height: 10 }} />}
      {keyword}
      {volume !== undefined && <span style={{ opacity: .6, fontSize: 10 }}>{volume >= 1000 ? `${(volume / 1000).toFixed(0)}k` : volume}</span>}
    </button>
  );
}
