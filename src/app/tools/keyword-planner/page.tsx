...
function impressionShareEstimate(maxCpcPounds: number, marketCpcPounds: number): number {
  if (marketCpcPounds <= 0) return 0.5;
  const ratio = maxCpcPounds / marketCpcPounds;
  if (ratio >= 2.5) return 0.8; // approaching absolute IS ceiling
  if (ratio >= 1.8) return 0.72; // strongly above market
  if (ratio >= 1.3) return 0.62; // above top-of-page high bid range
  if (ratio >= 1.0) return 0.5; // at average market CPC (median bidder)
  if (ratio >= 0.75) return 0.36; // below average market CPC
  if (ratio >= 0.5) return 0.2; // well below market
  if (ratio >= 0.3) return 0.1; // rarely appearing
...
function positionCtrMultiplier(impressionShare: number): number {
  if (impressionShare >= 0.6) return 1.4; // top of page (positions 1–2)
  if (impressionShare >= 0.42) return 1.1; // mixed positions 1–3
  if (impressionShare >= 0.26) return 0.8; // positions 2–4
  return 0.5; // below-fold / positions 4+
}
...
function kwKey(groupName: string, kw: string) {
  return `${groupName}::${kw}`;
}
...
export default function KeywordPlannerPage() {
    ...
    fetch("/api/settings")
      ...
      .then((d: Record<string, string>) => {
        if (d.pricingStrategy) {
          try {
            setPricingStrategy(JSON.parse(d.pricingStrategy) as PricingStrategy);
          } catch {
            /* ignore */
          }
        }
  ...
  function toggleGroup(g: AdGroup) {
    ...
    setSelectedKws((prev) => {
      ...
      for (const kw of g.keywords)
        allSelected ? next.delete(kwKey(g.name, kw)) : next.add(kwKey(g.name, kw));
      return next;
    });
  }
  ...
  function addKwToGroup(groupName: string) {
    ...
    if (!kw) return;
    setAdGroups((prev) =>
      prev.map((g) => (g.name === groupName ? { ...g, keywords: [...g.keywords, kw] } : g)),
    );
  ...
  function removeKwFromGroup(groupName: string, kw: string) {
    setAdGroups((prev) =>
      prev.map((g) =>
        g.name === groupName ? { ...g, keywords: g.keywords.filter((k) => k !== kw) } : g,
      ),
    );
    setSelectedKws((prev) => {
      const next = new Set(prev);
      next.delete(kwKey(groupName, kw));
      return next;
    });
  }
  ...
  const avgCompIndex = ideas.length
  ...
  const trendData = Object.entries(trendMap)
    .sort(([a], [b]) => {
      ...
      if (dy !== 0) return dy;
      return (
        (MONTH_ORDER[Object.keys(MONTH_ABBR).find((k) => MONTH_ABBR[k] === am) ?? ""] ?? 0) -
  ...
  const forecastCurveData =
    budgetPounds > 0 ? buildForecastCurve(ideas, budgetPounds, maxCpcPounds, convRatePct) : [];
    ...
    <div className="page" style={{ maxWidth: 1200 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: "var(--text-3)", marginTop: 4 }}>
              AI-powered proposal generation with keyword research, pricing strategy, and competitor
              analysis
            </p>
          </div>
        </div>
                ...
                >
                  {renamingId === r.id ? (
                    <form
                      ...
                      onSubmit={(e) => {
                        e.preventDefault();
                        handleRenameResearch(r.id, renameValue);
                      }}
                    >
                      <input
                        autoFocus
                    ...
                    <>
                      ...
                      <button
                        className="btn btn-ghost btn-sm"
                        style={{ padding: "4px 8px", flexShrink: 0, color: "var(--danger)" }}
                        title="Delete"
          ...
          <div key={num} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span
              style={{
                fontSize: 13,
                fontWeight: 500,
                color: step >= num ? "var(--text)" : "var(--text-3)",
              }}
            >
              {label}
            </span>
            {i < arr.length - 1 && (
              <ChevronRight
                style={{ width: 14, height: 14, color: "var(--text-4)", marginLeft: 4 }}
              />
            )}
          </div>
        ))}
          ...
          <div className="card-header">
            <div>
              ...
              <p className="card-subtitle">
                Describe the business \u2014 AI will generate 200+ keywords organised into ad groups
              </p>
            </div>
          </div>
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "var(--text-3)", marginBottom: 12 }}>
                Select the services to recommend — the AI will use these exact packages in the
                proposal.
              </p>
              {!pricingStrategy ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {((pricingStrategy.focusPackages?.length ?? 0) > 0 ||
                    (pricingStrategy.retainerPackages?.length ?? 0) > 0) && (
                    <div>
                      <p
                        style={{
                          fontSize: 11,
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                        {[
                          ...(pricingStrategy.focusPackages ?? []).map((p) => ({
                            name: p.name,
                            price: `${p.monthlyFee}/mo`,
                          })),
                          ...(pricingStrategy.retainerPackages ?? []).map((p) => ({
                            name: p.name,
      ...
      {step === 2 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {rationale && (
            ...
            >
              <p style={{ fontWeight: 600, color: "var(--accent-text)", marginBottom: 4 }}>
                AI Strategy Note
              </p>
          ...
          >
            <p style={{ fontSize: 13, color: "var(--text-2)" }}>
              <strong style={{ color: "var(--text)" }}>{selectedCount}</strong> of{" "}
              <strong style={{ color: "var(--text)" }}>{totalKws}</strong> keywords selected across{" "}
                  ...
                  <>
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              padding: "8px 20px",
                              borderBottom: "1px solid var(--border-subtle)",
                              background: checked ? "transparent" : "var(--bg)",
                            }}
                          >
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={() => toggleKw(group.name, kw)}
                              style={{
                                accentColor: "var(--accent)",
                                width: 13,
                                height: 13,
                            ...
                            <button
                              onClick={() => removeKwFromGroup(group.name, kw)}
                              style={{
                                background: "none",
                                border: "none",
                                cursor: "pointer",
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setGroupInputs((prev) => ({ ...prev, [group.name]: e.target.value }))
                        }
                      ...
                      <button
                        ...
                        onClick={() => addKwToGroup(group.name)}
                        disabled={!(groupInputs[group.name] ?? "").trim()}
                        style={{ flexShrink: 0 }}
                      >
          ...
          <div style={{ display: "flex", gap: 12, maxWidth: 860 }}>
            ...
            >
              {researching ? (
                <>
                  <Loader2 style={{ width: 16, height: 16 }} className="animate-spin" /> Fetching
                  Google Ads data for {selectedCount} keywords\u2026
                </>
              ) : (
                <>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          >
            <button className="btn btn-ghost btn-sm" onClick={() => setStep(2)}>
              <ChevronLeft style={{ width: 15, height: 15 }} /> Edit Keywords
            </button>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              {activeTab === "keywords" && (
                <>
                  ...
                  >
                    ...
                    {groupedView ? "Flat view" : "Group by ad group"}
                  </button>
                  <button
                    className="btn btn-secondary btn-sm"
              ...
              {showSavePanel ? (
                ...
                >
                  ...
                  <button
                    type="submit"
                    className="btn btn-primary btn-sm"
                    disabled={savingResearch || !researchTitle.trim()}
                  >
                    {savingResearch ? (
          ...
          >
            {(
              ...
              ] as { id: "keywords" | "forecaster"; label: string; icon: React.ReactNode }[]
            ).map((tab) => (
              <button
                key={tab.id}
          ...
          {activeTab === "keywords" && (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
                {[
                  {
                    icon: <BarChart2 style={{ width: 16, height: 16, color: "var(--accent)" }} />,
                    label: "Total Monthly Searches",
                    value: fmtNum(totalSearches),
                    sub: `${ideas.length} keywords with volume`,
                  },
                  {
                    icon: <Target style={{ width: 16, height: 16, color: "var(--warning)" }} />,
                    label: "Avg Competition",
                    value: String(avgCompIndex),
                    sub: "0\u2013100 index",
                  },
                  {
                    icon: <DollarSign style={{ width: 16, height: 16, color: "var(--success)" }} />,
                    label: "Avg CPC",
                    value: `\xa3${avgCpc.toFixed(2)}`,
                    sub: "mid-range bid estimate",
                  },
                  {
                    icon: <Zap style={{ width: 16, height: 16, color: "#0ea5e9" }} />,
                    label: "Est. Monthly Clicks",
                    value: fmtNum(totalClicks),
                    sub: cpcMicros
                      ? `~\xa3${totalCost.toFixed(0)}/mo`
                      : "enter Max CPC to estimate",
                  },
                ].map((m) => (
              ...
              <div className="card">
                <div className="card-header">
                  <div>
                    ...
                    <p className="card-subtitle">
                      {ideas.length} keywords with search volume \u2014 {uniqueGroups.length} ad
                      groups
                    </p>
                  </div>
                </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead>
                      <tr>
                        ...
                        <th
                          style={{ ...thStyle("text"), textAlign: "left" }}
                        ...
                        <th
                          style={{ ...thStyle("avgMonthlySearches"), textAlign: "right" }}
                        ...
                        <th
                          style={{ ...thStyle("competition"), textAlign: "center" }}
                        ...
                        <th
                          style={{ ...thStyle("competitionIndex"), textAlign: "right" }}
                        ...
                        <th
                          style={{ ...thStyle("lowTopOfPageBidMicros"), textAlign: "right" }}
                          onClick={() => handleSort("lowTopOfPageBidMicros")}
                        >
                          Low Bid{" "}
                          <span style={{ opacity: 0.6 }}>
                            {sortField === "lowTopOfPageBidMicros"
                              ? sortDir === "desc"
                                ? "\u2193"
                                : "\u2191"
                              : "\u2195"}
                        ...
                        <th
                          style={{ ...thStyle("highTopOfPageBidMicros"), textAlign: "right" }}
                          onClick={() => handleSort("highTopOfPageBidMicros")}
                        >
                          High Bid{" "}
                          <span style={{ opacity: 0.6 }}>
                            {sortField === "highTopOfPageBidMicros"
                              ? sortDir === "desc"
                                ? "\u2193"
                                : "\u2191"
                              : "\u2195"}
                    ...
                    <tbody>
                      {groupedView
                        ? uniqueGroups.map((groupName) => {
                            ...
                            const gSearches = groupIdeas.reduce(
                              (s, i) => s + i.avgMonthlySearches,
                              0,
                            );
                            const gClicks = groupIdeas.reduce(
                              (s, i) => s + estimateForecast(i, cpcMicros).clicks,
                              0,
                            );
                            const gCost = groupIdeas.reduce(
                                ...
                                >
                                  <td colSpan={9} style={{ padding: "8px 16px" }}>
                                    ...
                                    >
                                      ...
                                      >
                                        {groupIdeas.length} keywords \u00b7 {fmtNum(gSearches)}/mo
                                        searches \u00b7 {fmtNum(gClicks)} est. clicks
                                        {cpcMicros
                                          ? ` \u00b7 \xa3${gCost.toFixed(0)} est. cost`
                                          : ""}
                                      </span>
                                    </div>
                                  </td>
                                    ...
                                    >
                                      ...
                                      <td style={{ ...tdStyle, textAlign: "center" }}>
                                        <span
                                          style={{
                                            ...
                                            fontSize: 11,
                                            fontWeight: 600,
                                            ...competitionBadgeStyle(idea.competition),
                                          }}
                                        >
                                      ...
                                      <td
                                        style={{
                                          ...tdStyle,
                                          textAlign: "right",
                                          color: cpcMicros ? "var(--text-2)" : "var(--text-4)",
                                        }}
                                      >
                                        {cpcMicros ? `\xa3${costPounds.toFixed(2)}` : "\u2014"}
                                      </td>
                                    </tr>
                                  );
                                })}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              <div className="card">
                <div className="card-header">
                  <div>
                    ...
                    <p className="card-subtitle">
                      Adjust budget and bids to see projected performance
                    </p>
                  </div>
                </div>
                <div className="card-body" style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                  ...
                  <div style={{ flex: "1 1 160px", minWidth: 140 }}>
                    ...
                    <p style={{ fontSize: 11, color: "var(--text-4)", marginTop: 5 }}>
                      {dailyBudget
                        ? `\xa3${dailyBudget}/day \xd7 30.4 days`
                  ...
                  <div style={{ flex: "1 1 160px", minWidth: 140 }}>
                    ...
                    <p style={{ fontSize: 11, color: "var(--text-4)", marginTop: 5 }}>
                      {cpcAutoFilled
                        ? "Volume-weighted bid estimate from keyword data"
                        : "Leave blank to use keyword bid estimates"}
                    </p>
                  </div>
                  {/* Conversion Rate */}
                  <div style={{ flex: "1 1 160px", minWidth: 140 }}>
                    ...
                    <p style={{ fontSize: 11, color: "var(--text-4)", marginTop: 5 }}>
                      {crAutoFilled
                        ? "AI estimate based on business type and keywords"
                        : "Estimated % of clicks that convert"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {budgetPounds > 0 ? (
                      ...
                      [
                        {
                          ...
                          label: "Est. Clicks",
                          value: fmtNum(forecastMetrics.clicks),
                          sub: `at \xa3${budgetPounds.toFixed(0)}/mo budget`,
                        },
                        {
                          icon: <Eye style={{ width: 16, height: 16, color: "var(--accent)" }} />,
                          label: "Est. Impressions",
                          value: fmtNum(forecastMetrics.impressions),
                          sub: `CTR: ${forecastMetrics.ctr.toFixed(1)}%`,
                        },
                        {
                          icon: (
                          ...
                          value: fmtNum(forecastMetrics.conversions),
                          sub: `at ${convRatePct.toFixed(1)}% conv. rate`,
                        },
                        {
                          icon: (
                            <DollarSign
                          ...
                          label: "Avg CPA",
                          value:
                            forecastMetrics.avgCpa > 0
                              ? `\xa3${forecastMetrics.avgCpa.toFixed(2)}`
                              : "\u2014",
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
                    {(
                      [
                        {
                          icon: (
                            <DollarSign style={{ width: 16, height: 16, color: "var(--danger)" }} />
                          ),
                          label: "Total Cost",
                          value: `\xa3${forecastMetrics.cost.toFixed(0)}`,
                          sub: "estimated monthly spend",
                        },
                        {
                          icon: (
                            <Target style={{ width: 16, height: 16, color: "var(--accent-2)" }} />
                          ),
                          label: "Avg CTR",
                          value: `${forecastMetrics.ctr.toFixed(2)}%`,
                        ...
                        },
                        {
                          ...
                          label: "Avg CPC",
                          value:
                            forecastMetrics.avgCpc > 0
                              ? `\xa3${forecastMetrics.avgCpc.toFixed(2)}`
                              : "\u2014",
                    ...
                    <div className="card">
                      <div className="card-header">
                        <div>
                          ...
                          <p className="card-subtitle">
                            Estimated conversions at different monthly spend levels \u2014 dot marks
                            your entered budget
                          </p>
                        </div>
                      </div>
                          ...
                          <LineChart data={forecastCurveData}>
                            ...
                            <Line
                              ...
                              dot={(props) => {
                                ...
                                if (payload.budget !== Math.round(budgetPounds))
                                  return <g key={`no-dot-${cx}`} />;
                                return (
                                  <circle
                                    key={`dot-${cx}`}
                                    cx={cx}
                ...
                <div className="card" style={{ padding: 48, textAlign: "center" }}>
                  <Activity
                    style={{ width: 36, height: 36, color: "var(--text-4)", margin: "0 auto 12px" }}
                  />
                  ...
                  <p style={{ fontSize: 12, color: "var(--text-4)", marginTop: 6 }}>
                    The forecaster will project clicks, impressions, and conversions based on the
      ...
      {step === 3 && ideas.length === 0 && (
        <div className="card" style={{ padding: 60, textAlign: "center" }}>
          <Search
            style={{ width: 40, height: 40, color: "var(--text-4)", margin: "0 auto 16px" }}
          />
          <p style={{ fontSize: 14, color: "var(--text-3)" }}>
            No keywords with search volume were found.
          </p>
          <p style={{ fontSize: 12, color: "var(--text-4)", marginTop: 8 }}>
            Try selecting more keywords or adjusting the location.
          </p>
          <button
            className="btn btn-ghost btn-sm"
            style={{ marginTop: 16 }}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: "var(--text-3)" }}>
                    AI-generated proposal using your keyword research data
                  </p>
                </div>
              </div>
              <button
            ...
            >
              ...
              {generatedProposalId && !generatingProposal && (
                ...
                >
                  ...
                  <span style={{ flex: 1 }}>Proposal draft generated.</span>
                </div>
              )}
