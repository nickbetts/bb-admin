...
export default async function ClientPage({ params, searchParams }: Props) {
  ...
  if (client.metaAccountId && !client.metaAccountName && process.env.META_ACCESS_TOKEN) {
    backfills.push(
      fetch(
        `https://graph.facebook.com/v19.0/act_${client.metaAccountId}?fields=name&access_token=${process.env.META_ACCESS_TOKEN}`,
      )
        ...
        .then((data) => {
          if (data?.name) {
            ...
            return prisma.client
              .update({ where: { id: client.id }, data: { metaAccountName: data.name } })
              .then(() => {});
          }
        })
        .catch(() => {}),
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 28 }}>
            ...
            {client.ga4PropertyId ? (
              <span
                ...
                style={{ padding: "6px 14px", fontSize: 13 }}
                title={`GA4: ${client.ga4PropertyName ?? client.ga4PropertyId}`}
              >
                <span
                  style={{
                    width: 7,
                    height: 7,
                    borderRadius: "50%",
                    background: "var(--channel-google_ads)",
                    display: "inline-block",
                  }}
                />
            ...
            {client.metaAccountId ? (
              <span
                ...
                style={{ padding: "6px 14px", fontSize: 13 }}
                title={`Meta Ads: ${client.metaAccountName ?? client.metaAccountId}`}
              >
                <span
                  style={{
                    width: 7,
            ...
            {client.googleAdsCustomerId ? (
              <span
                ...
                style={{ padding: "6px 14px", fontSize: 13 }}
                title={`Google Ads: ${client.googleAdsAccountName ?? client.googleAdsCustomerId}`}
              >
                <span
                  style={{
                    width: 7,
            ...
            {client.searchConsoleSiteUrl ? (
              <span
                ...
                style={{ padding: "6px 14px", fontSize: 13 }}
                title={`Search Console: ${client.searchConsoleSiteUrl}`}
              >
                <span
                  style={{
                    width: 7,
                    height: 7,
                    borderRadius: "50%",
                    background: "var(--channel-search_console)",
                    display: "inline-block",
                  }}
                />
                Search Console ·{" "}
          ...
          <div>
            {client.reports.map((report) => (
              <Link
                ...
                className="flex items-center justify-between border-b border-[var(--border-subtle)] px-7 py-4 no-underline transition-colors hover:bg-[var(--border-subtle)]"
              >
                <div>
                ...
                <span
                  className={
                    report.status === "published" ? "badge badge-green" : "badge badge-slate"
                  }
                >
                  {report.status}
                </span>
