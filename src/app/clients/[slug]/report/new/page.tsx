...
export default function NewReportPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const selectedTemplateSections: { sectionType: string; title: string }[] =
    selectedTemplateId && selectedTemplateId !== "custom"
      ? (() => {
          try {
            return JSON.parse(templates.find((t) => t.id === selectedTemplateId)?.sections ?? "[]");
          } catch {
            return [];
          }
        })()
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div className="card">
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              <label className="form-label">
                Reporting Period <span style={{ color: "var(--danger)" }}>*</span>
              </label>
              <select
                className="form-input"
                value={form.period}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <label
                style={{
                  ...
                  padding: "12px 14px",
                  borderRadius: "var(--r)",
                  border: `1px solid ${selectedTemplateId === "custom" ? "var(--accent)" : "var(--border)"}`,
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: 12, color: "var(--text-3)", margin: "2px 0 0" }}>
                    Choose exactly which sections to include
                  </p>
                </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                      {CONTENT_SECTION_TYPES.map((s) => {
                        const checked = !!customSections.find(
                          (cs) => cs.sectionType === s.sectionType,
                        );
                        return (
                          <label
                            key={s.sectionType}
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={() => toggleCustomSection(s.sectionType)}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button
            ...
            disabled={
              loading ||
              !form.title ||
              (selectedTemplateId === "custom" && customSections.length === 0)
            }
