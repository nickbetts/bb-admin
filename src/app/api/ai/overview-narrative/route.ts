  ...
  try {
        ...
        competitorContext =
          ...
          latest
            .map((c) => {
              ...
              try {
                m = JSON.parse(c.metrics);
              } catch {
                /* ignore */
              }
              const parts: string[] = [`  ${c.domain}:`];
              if (m.organicTraffic)
                parts.push(`    Organic traffic: ${Number(m.organicTraffic).toLocaleString()}`);
              if (m.organicKeywords)
                parts.push(`    Organic keywords: ${Number(m.organicKeywords).toLocaleString()}`);
              if (m.backlinks) parts.push(`    Backlinks: ${Number(m.backlinks).toLocaleString()}`);
              if (m.domainAuthority ?? m.authorityScore)
                parts.push(`    Authority score: ${m.domainAuthority ?? m.authorityScore}`);
    ...
    if (clientGa4PropertyId) {
      ...
      if (aiRefCache.status === "fulfilled" && aiRefCache.value) {
        try {
          const refs = JSON.parse(aiRefCache.value.data) as {
            source: string;
            sessions: number;
            users: number;
          }[];
          if (refs.length > 0) {
            ...
            aiReferralsContext =
              ...
              refs
                .slice(0, 6)
                .map((r) => `${r.source}: ${r.sessions.toLocaleString()} sessions`)
                .join(", ");
          }
        } catch {
          /* ignore */
    ...
    if (clientId) {
      try {
        ...
        if (metaDemoCache) {
          const rows = JSON.parse(metaDemoCache.data) as {
            age: string;
            gender: string;
            impressions: number;
            clicks: number;
    ...
    if (clientGadsCustomerId) {
      try {
        ...
        if (gadsCache) {
          ...
          const audiences = (gadsData.audienceCriteria ?? []).filter(
            (a) => !a.negative && a.displayName,
          );
    ...
    const systemPrompt = `You are a senior cross-channel performance strategist at Betts & Burton, a UK digital marketing agency.
