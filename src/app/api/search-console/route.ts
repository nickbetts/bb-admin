    ...
    try {
      ...
      switch (type) {
        ...
        case "pages":
          return NextResponse.json(
            await withApiCache(cacheKey, GSC_CACHE_TTL_HOURS, () =>
              getGSCTopPages(siteUrl, startDate, endDate),
            ),
          );
        case "daily":
          return NextResponse.json(
            await withApiCache(cacheKey, GSC_CACHE_TTL_HOURS, () =>
              getGSCDailyData(siteUrl, startDate, endDate),
            ),
          );
        case "devices":
          return NextResponse.json(
            await withApiCache(cacheKey, GSC_CACHE_TTL_HOURS, () =>
              getGSCDevices(siteUrl, startDate, endDate),
            ),
          );
        case "countries":
          return NextResponse.json(
            await withApiCache(cacheKey, GSC_CACHE_TTL_HOURS, () =>
              getGSCCountries(siteUrl, startDate, endDate),
            ),
          );
        ...
        case "compare": {
          ...
          return NextResponse.json(
            await withApiCache(compareCacheKey, GSC_CACHE_TTL_HOURS, async () => {
        ...
        case "bulk": {
          ...
          return NextResponse.json(
            await withApiCache(bulkCacheKey, GSC_CACHE_TTL_HOURS, async () => {
              const [
                overview,
                queries,
                pages,
        ...
        case "branded-split": {
          ...
          const brandTerms = brandTermsParam
            ? brandTermsParam
                .split(",")
                .map((t) => t.trim())
                .filter(Boolean)
        ...
        case "discover":
          return NextResponse.json(
            await withApiCache(
              `gsc:discover:${siteUrl}:${startDate}:${endDate}`,
              GSC_CACHE_TTL_HOURS,
        ...
        case "sitemaps":
          return NextResponse.json(
            await withApiCache(`gsc:sitemaps:${siteUrl}`, 24, () => getGSCSitemaps(siteUrl)),
          );
