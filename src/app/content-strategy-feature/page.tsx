...
const features = [
  { icon: <Search size={20} />, title: "SemRush domain import", desc: "Enter a domain and pull the full keyword profile from SemRush. Organic rankings, traffic estimates, and keyword gaps are imported automatically." },
  { icon: <Target size={20} />, title: "Competitor analysis", desc: "Add competitor domains and the tool finds content gaps between you and them. See what they rank for that you do not, with volume and difficulty data." },
  { icon: <Layers size={20} />, title: "Keyword clustering", desc: "Keywords are grouped into topical clusters automatically. Each cluster becomes a pillar topic with supporting content suggestions." },
  { icon: <Calendar size={20} />, title: "Content calendar", desc: "Topics are mapped to a publishing schedule. See what to write, when to publish, and which keywords each piece targets." },
  { icon: <FileText size={20} />, title: "Topic gap analysis", desc: "Finds topics your competitors cover that you have not touched. Sorted by search volume so you can prioritise what matters most." },
  { icon: <Link2 size={20} />, title: "Share links", desc: "Every content strategy gets a shareable link. Send it to clients or team members. They can view the strategy without logging in." },
];

const topicPillars = [
  { name: "Summer Camps", keywords: 24, volume: "14,800", difficulty: "Medium", colour: "#a855f7" },
  { name: "Football Training", keywords: 18, volume: "9,600", difficulty: "High", colour: "#6366f1" },
  { name: "Youth Development", keywords: 12, volume: "5,200", difficulty: "Low", colour: "#10b981" },
  { name: "Goalkeeper Coaching", keywords: 8, volume: "3,100", difficulty: "Low", colour: "#f59e0b" },
];
...
export default function ContentStrategyPage() {
  ...
  return (
    <div style={{ background: "#09090f", color: "white", fontFamily: "inherit" }}>
      <div style={{ position: "fixed", pointerEvents: "none", zIndex: 1, width: 600, height: 600, borderRadius: "50%", left: mouse.x - 300, top: mouse.y - 300, background: `radial-gradient(circle, rgba(168,85,247,0.06) 0%, transparent 65%)`, transition: "left 0.2s ease-out, top 0.2s ease-out" }} />
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", background: "rgba(9,9,15,0.88)", borderBottom: "1px solid rgba(255,255,255,0.06)", height: 64, display: "flex", alignItems: "center", padding: "0 40px", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(255,255,255,0.65)" }}>Content Strategy</span>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <a href="#cta" onClick={(e) => { e.preventDefault(); document.getElementById("cta")?.scrollIntoView({ behavior: "smooth" }); }} style={{ padding: "9px 20px", borderRadius: 8, background: `linear-gradient(135deg, ${accent}, #7c3aed)`, color: "white", fontSize: 13, fontWeight: 600, textDecoration: "none", boxShadow: `0 0 20px rgba(168,85,247,0.3)` }} className="cta-accent-pulse">Talk to us →</a>
        </div>
      </nav>

      {/* ── HERO ── */}
      <section style={{ minHeight: "100vh", paddingTop: 64, position: "relative", overflow: "hidden", display: "flex", alignItems: "center" }}>
        <div style={{ position: "absolute", width: "65%", paddingBottom: "65%", top: "-15%", left: "-15%", pointerEvents: "none", borderRadius: "50%", background: `radial-gradient(circle, rgba(168,85,247,0.25) 0%, transparent 65%)`, transform: `translateY(${parallaxY * -0.25}px)` }} className="orb-1" />
        ...
        {particles.map((p) => (
          <div key={p.id} className="hero-particle" style={{ position: "absolute", top: p.top, left: p.left, pointerEvents: "none", width: p.size, height: p.size, borderRadius: "50%", background: p.id % 3 === 0 ? `rgba(168,85,247,0.9)` : p.id % 3 === 1 ? "rgba(16,185,129,0.9)" : "rgba(255,255,255,0.8)", opacity: p.opacity, animationDuration: p.dur, animationDelay: p.delay }} />
        ))}
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "80px 40px", display: "grid", gridTemplateColumns: "1fr 480px", gap: 72, alignItems: "center", width: "100%", position: "relative", boxSizing: "border-box" }} className="hero-grid">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 28, flexWrap: "wrap" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 20, background: `rgba(168,85,247,0.12)`, border: `1px solid rgba(168,85,247,0.3)` }}>
                <BookOpen size={12} color={accentLight} />
                <span style={{ fontSize: 12, fontWeight: 700, color: accentLight, letterSpacing: "0.08em", textTransform: "uppercase" }}>SemRush Powered</span>
              </div>
            </div>
            <h1 style={{ fontSize: 56, fontWeight: 900, lineHeight: 1.05, letterSpacing: "-0.04em", marginBottom: 28, color: "white" }}>
              ...
              <br />
              <span className="hw hw2" style={{ background: `linear-gradient(90deg, ${accentLight}, ${accent})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>actually get published.</span>
            </h1>
            <p style={{ fontSize: 18, color: "rgba(255,255,255,0.6)", lineHeight: 1.8, maxWidth: 500, marginBottom: 32, fontWeight: 400 }}>
              Pull a SemRush keyword profile, find content gaps against competitors, cluster topics into pillars, and generate a publishing calendar. All from one tool.
            </p>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <a href="#mockup" onClick={(e) => { e.preventDefault(); document.getElementById("mockup")?.scrollIntoView({ behavior: "smooth" }); }} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 24px", borderRadius: 10, background: `linear-gradient(135deg, ${accent}, #7c3aed)`, color: "white", fontSize: 14, fontWeight: 600, textDecoration: "none", boxShadow: `0 0 24px rgba(168,85,247,0.35)` }} className="cta-accent-pulse">
          ...
          <div style={{ position: "relative" }}>
            <div style={{ position: "absolute", inset: -40, borderRadius: "50%", background: `radial-gradient(circle, rgba(168,85,247,0.15) 0%, transparent 70%)`, pointerEvents: "none" }} />
            <div className="mockup-3d" style={{ background: "rgba(255,255,255,0.04)", border: `1px solid rgba(168,85,247,0.2)`, borderRadius: 20, overflow: "hidden", position: "relative" }}>
              <div style={{ padding: "18px 22px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", background: `rgba(168,85,247,0.05)` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "white" }}>Content Strategy</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 1 }}>fcvsoccercamps.com</div>
                  </div>
                </div>
                <div style={{ fontSize: 10, fontWeight: 600, padding: "3px 8px", borderRadius: 4, background: "rgba(168,85,247,0.1)", border: "1px solid rgba(168,85,247,0.25)", color: accentLight }}>4 pillars</div>
              </div>

              <div style={{ padding: "16px 22px", display: "flex", flexDirection: "column", gap: 10 }}>
                {topicPillars.map((pillar, i) => (
                  <div key={i} style={{ padding: "14px 16px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 14 }}>
                    <div style={{ width: 4, height: 36, borderRadius: 2, background: pillar.colour, flexShrink: 0 }} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "white" }}>{pillar.name}</div>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginTop: 2 }}>{pillar.keywords} keywords / {pillar.volume} monthly volume</div>
                    </div>
                    <span style={{ fontSize: 9, fontWeight: 600, padding: "2px 6px", borderRadius: 4, background: pillar.difficulty === "Low" ? "rgba(16,185,129,0.1)" : pillar.difficulty === "Medium" ? "rgba(245,158,11,0.1)" : "rgba(239,68,68,0.1)", border: `1px solid ${pillar.difficulty === "Low" ? "rgba(16,185,129,0.25)" : pillar.difficulty === "Medium" ? "rgba(245,158,11,0.25)" : "rgba(239,68,68,0.25)"}`, color: pillar.difficulty === "Low" ? "#6ee7b7" : pillar.difficulty === "Medium" ? "#fcd34d" : "#fca5a5" }}>{pillar.difficulty}</span>
                  </div>
                ))}
              </div>
              ...
              <div style={{ padding: "12px 22px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)" }}>62 keywords across 4 topic pillars</div>
                <div style={{ padding: "6px 10px", borderRadius: 6, background: `rgba(168,85,247,0.12)`, border: `1px solid rgba(168,85,247,0.25)`, fontSize: 10, fontWeight: 600, color: accentLight }}>View Calendar</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ── STATS ── */}
      <section id="stats-row" className="reveal-section" style={{ padding: "80px 40px", background: `linear-gradient(180deg, rgba(168,85,247,0.04) 0%, rgba(16,185,129,0.03) 100%)`, borderTop: "1px solid rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="stats-grid">
            {[
              { val: s1, suffix: " input modes", label: "Import from SemRush domain analysis or upload a CSV", note: "Flexible data sources", color: accent },
              { val: s2, suffix: " pillars", label: "Keywords are clustered into topical pillars automatically", note: "Powered by keyword grouping", color: "#10b981" },
              { val: "Live", suffix: "", label: "Shareable links with full strategy visible to recipients", note: "No login required", color: "#f59e0b", isText: true },
            ].map((s, i) => (
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 46, fontWeight: 800, letterSpacing: "-0.03em", marginBottom: 18, color: "white" }} className="blur-reveal">
              From domain to content calendar<br />in one session.
            </h2>
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.45)", maxWidth: 560, margin: "0 auto", lineHeight: 1.7 }}>
              Import a SemRush profile, add competitors for gap analysis, and the tool clusters your keywords into content pillars with a publishing schedule.
            </p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="features-grid">
            {features.map((f, i) => (
              <div key={i} className="feature-card stagger-in" style={{ padding: "28px 24px", borderRadius: 16, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", animationDelay: `${i * 0.08}s`, transition: "transform 0.35s ease, border-color 0.3s ease, box-shadow 0.3s ease" }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: `rgba(168,85,247,0.12)`, border: `1px solid rgba(168,85,247,0.25)`, display: "flex", alignItems: "center", justifyContent: "center", color: accentLight, marginBottom: 16 }}>{f.icon}</div>
      ...
      <section id="mockup" className="reveal-section" style={{ padding: "120px 40px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: accentLight, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 16 }} className="blur-reveal">Content calendar</p>
          ...
          <div className="mockup-3d" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid rgba(168,85,247,0.15)`, borderRadius: 20, overflow: "hidden", position: "relative" }}>
            <div style={{ padding: "20px 28px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,0.8)", marginBottom: 14 }}>Publishing schedule</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  { week: "Week 1", topic: "Best Summer Football Camps in the UK", pillar: "Summer Camps", keywords: "summer football camps, kids football camp, football camp UK", status: "Draft" },
                  { week: "Week 2", topic: "How to Improve Your First Touch", pillar: "Football Training", keywords: "improve first touch, football skills drill, ball control", status: "Scheduled" },
                  { week: "Week 3", topic: "What Makes a Good Goalkeeper Coach", pillar: "Goalkeeper Coaching", keywords: "goalkeeper coaching, GK training, save technique", status: "Planned" },
                  { week: "Week 4", topic: "Youth Football Development Pathway", pillar: "Youth Development", keywords: "youth football, development pathway, academy route", status: "Planned" },
                ].map((row, i) => (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "80px 1fr 120px 80px", gap: 12, alignItems: "center", padding: "12px 14px", borderRadius: 10, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.4)" }}>{row.week}</div>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: "white", marginBottom: 2 }}>{row.topic}</div>
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>{row.keywords}</div>
                    </div>
                    <div style={{ fontSize: 10, fontWeight: 600, color: accentLight }}>{row.pillar}</div>
                    <span style={{ fontSize: 9, fontWeight: 600, padding: "2px 6px", borderRadius: 4, textAlign: "center", background: row.status === "Draft" ? "rgba(245,158,11,0.1)" : row.status === "Scheduled" ? "rgba(16,185,129,0.1)" : "rgba(255,255,255,0.04)", border: row.status === "Draft" ? "1px solid rgba(245,158,11,0.25)" : row.status === "Scheduled" ? "1px solid rgba(16,185,129,0.25)" : "1px solid rgba(255,255,255,0.08)", color: row.status === "Draft" ? "#fcd34d" : row.status === "Scheduled" ? "#6ee7b7" : "rgba(255,255,255,0.3)" }}>{row.status}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section id="cta" className="reveal-section" style={{ padding: "120px 40px 140px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", width: "80%", paddingBottom: "40%", bottom: "-20%", left: "50%", transform: "translateX(-50%)", background: `radial-gradient(ellipse, rgba(168,85,247,0.12) 0%, transparent 65%)`, pointerEvents: "none" }} />
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center", position: "relative" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 20, background: `rgba(168,85,247,0.12)`, border: `1px solid rgba(168,85,247,0.25)`, marginBottom: 28 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: accent, boxShadow: `0 0 8px ${accentGlow}` }} className="accent-pulse" />
          ...
          <h2 style={{ fontSize: 50, fontWeight: 900, letterSpacing: "-0.04em", marginBottom: 20, color: "white", lineHeight: 1.05 }} className="blur-reveal">
            Content strategies that<br />
            <span style={{ background: `linear-gradient(90deg, ${accentLight}, ${accent}, #10b981)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>start with real data.</span>
          </h2>
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.55)", lineHeight: 1.8, marginBottom: 40 }}>
            No more spreadsheet-driven content plans. Import from SemRush, cluster topics, generate a calendar, and share with your team or client.
          </p>
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            <a href="/login" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "15px 28px", borderRadius: 12, background: `linear-gradient(135deg, ${accent}, #7c3aed)`, color: "white", fontSize: 15, fontWeight: 700, textDecoration: "none", boxShadow: `0 0 32px rgba(168,85,247,0.4)` }} className="cta-accent-pulse">
          ...
          <p style={{ marginTop: 24, fontSize: 12, color: "rgba(255,255,255,0.25)" }}>SemRush integration included. Content Strategy is part of every StratOS plan.</p>
        </div>
      </section>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "24px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 12, color: "rgba(255,255,255,0.25)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span>&copy; {new Date().getFullYear()} i3 Media. Content Strategy is part of StratOS.</span>
        </div>
