...
const features = [
  { icon: <Layers size={20} />, title: "Multi-channel analysis", desc: "Google Ads and Meta spend analysed together. The AI reads performance data from both platforms and compares campaign-level efficiency." },
  { icon: <Eye size={20} />, title: "Opportunity detection", desc: "Finds campaigns with lost impression share, high CPA, or underperforming ROAS. Each opportunity is sized with an estimated impact." },
  { icon: <ArrowRightLeft size={20} />, title: "Reallocation recommendations", desc: "Specific suggestions for where to move budget. Not generic advice. Actual campaign names, actual amounts, actual projected outcomes." },
  { icon: <TrendingUp size={20} />, title: "Impact projections", desc: "Every recommendation comes with a projected uplift. How many extra conversions. What the ROAS would look like. Grounded in your actual data." },
  { icon: <BarChart3 size={20} />, title: "Ad group context", desc: "Drill into ad group performance within each campaign. See which groups are pulling weight and which are dragging the average down." },
  { icon: <Zap size={20} />, title: "Live API data", desc: "Recommendations use the latest data from Google Ads and Meta APIs. Not last month. Not an export. Current performance with cache refresh." },
];

export default function BudgetIntelligencePage() {
  const accent = "#f59e0b";
  const accentLight = "#fde68a";
  const accentGlow = "rgba(245,158,11,0.6)";
  ...
  return (
    <div style={{ background: "#09090f", color: "white", fontFamily: "inherit" }}>
      <div style={{ position: "fixed", pointerEvents: "none", zIndex: 1, width: 600, height: 600, borderRadius: "50%", left: mouse.x - 300, top: mouse.y - 300, background: `radial-gradient(circle, rgba(245,158,11,0.06) 0%, transparent 65%)`, transition: "left 0.2s ease-out, top 0.2s ease-out" }} />
      ...
      <nav style={{ position: "fixed", right: 24, top: "50%", transform: "translateY(-50%)", zIndex: 40, display: "flex", flexDirection: "column", gap: 1, background: "rgba(9,9,15,0.88)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "10px 6px", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)" }} className="side-nav">
        ...
        <div style={{ width: "100%", height: 44, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, marginTop: 8, paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <div style={{ width: 2, height: 32, background: "rgba(255,255,255,0.08)", borderRadius: 2, position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: `${scrollPct}%`, background: `linear-gradient(180deg, ${accent}, #d97706)`, transition: "height 0.1s linear", borderRadius: 2 }} />
          </div>
        </div>
      </nav>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", background: "rgba(9,9,15,0.88)", borderBottom: "1px solid rgba(255,255,255,0.06)", height: 64, display: "flex", alignItems: "center", padding: "0 40px", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(255,255,255,0.65)" }}>Budget Intelligence</span>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <a href="#cta" onClick={(e) => { e.preventDefault(); document.getElementById("cta")?.scrollIntoView({ behavior: "smooth" }); }} style={{ padding: "9px 20px", borderRadius: 8, background: `linear-gradient(135deg, ${accent}, #d97706)`, color: "white", fontSize: 13, fontWeight: 600, textDecoration: "none", boxShadow: `0 0 20px rgba(245,158,11,0.3)` }} className="cta-accent-pulse">Talk to us →</a>
        </div>
      </nav>

      {/* ── HERO ── */}
      <section style={{ minHeight: "100vh", paddingTop: 64, position: "relative", overflow: "hidden", display: "flex", alignItems: "center" }}>
        <div style={{ position: "absolute", width: "65%", paddingBottom: "65%", top: "-15%", left: "-15%", pointerEvents: "none", borderRadius: "50%", background: `radial-gradient(circle, rgba(245,158,11,0.22) 0%, transparent 65%)`, transform: `translateY(${parallaxY * -0.25}px)` }} className="orb-1" />
        <div style={{ position: "absolute", width: "55%", paddingBottom: "55%", bottom: "-15%", right: "-10%", pointerEvents: "none", borderRadius: "50%", background: `radial-gradient(circle, rgba(217,119,6,0.20) 0%, transparent 65%)`, transform: `translateY(${parallaxY * -0.15}px)` }} className="orb-2" />
        ...
        {particles.map((p) => (
          <div key={p.id} className="hero-particle" style={{ position: "absolute", top: p.top, left: p.left, pointerEvents: "none", width: p.size, height: p.size, borderRadius: "50%", background: p.id % 3 === 0 ? `rgba(245,158,11,0.9)` : p.id % 3 === 1 ? "rgba(217,119,6,0.9)" : "rgba(255,255,255,0.8)", opacity: p.opacity, animationDuration: p.dur, animationDelay: p.delay }} />
        ))}
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "80px 40px", display: "grid", gridTemplateColumns: "1fr 480px", gap: 72, alignItems: "center", width: "100%", position: "relative", boxSizing: "border-box" }} className="hero-grid">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 28, flexWrap: "wrap" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 20, background: `rgba(245,158,11,0.12)`, border: `1px solid rgba(245,158,11,0.3)` }}>
                <Coins size={12} color={accentLight} />
                <span style={{ fontSize: 12, fontWeight: 700, color: accentLight, letterSpacing: "0.08em", textTransform: "uppercase" }}>Spend Optimisation</span>
              </div>
            </div>
            <h1 style={{ fontSize: 56, fontWeight: 900, lineHeight: 1.05, letterSpacing: "-0.04em", marginBottom: 28, color: "white" }}>
              <span className="hw hw1">Stop guessing where to</span>
              <br />
              <span className="hw hw2" style={{ background: `linear-gradient(90deg, ${accentLight}, ${accent})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>move the spend.</span>
            </h1>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <a href="#mockup" onClick={(e) => { e.preventDefault(); document.getElementById("mockup")?.scrollIntoView({ behavior: "smooth" }); }} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 24px", borderRadius: 10, background: `linear-gradient(135deg, ${accent}, #d97706)`, color: "white", fontSize: 14, fontWeight: 600, textDecoration: "none", boxShadow: `0 0 24px rgba(245,158,11,0.35)` }} className="cta-accent-pulse">
                See it in action <ArrowRight size={14} />
              </a>
            </div>
          </div>

          {/* Budget reallocation mockup */}
          <div style={{ position: "relative" }}>
            <div style={{ position: "absolute", inset: -40, borderRadius: "50%", background: `radial-gradient(circle, rgba(245,158,11,0.15) 0%, transparent 70%)`, pointerEvents: "none" }} />
            <div className="mockup-3d" style={{ background: "rgba(255,255,255,0.04)", border: `1px solid rgba(245,158,11,0.2)`, borderRadius: 20, overflow: "hidden", position: "relative" }}>
              <div style={{ padding: "18px 22px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", background: `rgba(245,158,11,0.05)` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 32, height: 32, borderRadius: 8, background: `rgba(245,158,11,0.15)`, border: `1px solid rgba(245,158,11,0.3)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Coins size={16} color={accentLight} />
                  </div>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "white" }}>Budget Recommendations</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 1 }}>BrightSparks Academy - April 2026</div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ padding: "16px 22px", display: "flex", flexDirection: "column", gap: 10 }}>
                {[
                  { campaign: "Summer Camps - Search", platform: "Google Ads", current: "2,400", recommended: "3,200", change: "+33%", reason: "Lost IS: 28% due to budget", impact: "+18 conversions", impactColour: "#10b981" },
                  { campaign: "Brand Awareness", platform: "Meta", current: "1,800", recommended: "1,200", change: "-33%", reason: "CPM up 42% vs Q4, low CTR", impact: "Reallocate to Search", impactColour: "#f59e0b" },
                  { campaign: "Retargeting - All", platform: "Meta", current: "900", recommended: "900", change: "0%", reason: "ROAS 6.2x - performing well", impact: "No change needed", impactColour: "rgba(255,255,255,0.3)" },
                ].map((row, i) => (
                  <div key={i} style={{ padding: "14px 16px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 700, color: "white" }}>{row.campaign}</div>
                        <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 2 }}>{row.platform}</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", textDecoration: row.change !== "0%" ? "line-through" : "none" }}>{row.current}</span>
                          {row.change !== "0%" && <ArrowRight size={10} color="rgba(255,255,255,0.2)" />}
                          <span style={{ fontSize: 12, fontWeight: 700, color: "white" }}>{row.recommended}</span>
                        </div>
                        <div style={{ fontSize: 10, color: row.change.startsWith("+") ? "#10b981" : row.change.startsWith("-") ? "#ef4444" : "rgba(255,255,255,0.3)", fontWeight: 600, marginTop: 2 }}>{row.change}</div>
                      </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                      <span style={{ fontSize: 10, color: "rgba(255,255,255,0.35)" }}>{row.reason}</span>
                      <span style={{ fontSize: 10, fontWeight: 600, color: row.impactColour }}>{row.impact}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* ── STATS ── */}
      <section id="stats-row" className="reveal-section" style={{ padding: "80px 40px", background: `linear-gradient(180deg, rgba(245,158,11,0.04) 0%, rgba(217,119,6,0.03) 100%)`, borderTop: "1px solid rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="stats-grid">
            {[
              { val: s1, suffix: " platforms", label: "Google Ads and Meta spend analysed side by side", note: "Cross-channel intelligence", color: accent },
              { val: s2, suffix: "% live", label: "Recommendations use the latest API data, not exports", note: "Current performance", color: "#d97706" },
              { val: "AI", suffix: " written", label: "Every recommendation includes projected impact and reasoning", note: "Actionable, not generic", color: "#10b981", isText: true },
            ].map((s, i) => (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="features-grid">
            {features.map((f, i) => (
              <div key={i} className="feature-card stagger-in" style={{ padding: "28px 24px", borderRadius: 16, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", animationDelay: `${i * 0.08}s`, transition: "transform 0.35s ease, border-color 0.3s ease, box-shadow 0.3s ease" }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: `rgba(245,158,11,0.12)`, border: `1px solid rgba(245,158,11,0.25)`, display: "flex", alignItems: "center", justifyContent: "center", color: accentLight, marginBottom: 16 }}>{f.icon}</div>
      ...
      <section id="mockup" className="reveal-section" style={{ padding: "120px 40px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: accentLight, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 16 }} className="blur-reveal">Opportunity summary</p>
          ...
          <div className="mockup-3d" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid rgba(245,158,11,0.15)`, borderRadius: 20, overflow: "hidden", position: "relative" }}>
            <div style={{ padding: "24px 28px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
                {[
                  { label: "Total monthly spend", value: "5,100", prefix: "GBP " },
                  { label: "Wasted spend identified", value: "680", prefix: "GBP " },
                  { label: "Potential extra conversions", value: "+22" },
                  { label: "Projected ROAS uplift", value: "+0.8x" },
                ].map((card, i) => (
                  <div key={i} style={{ padding: "16px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", textAlign: "center" }}>
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>{card.label}</div>
                    <div style={{ fontSize: 22, fontWeight: 800, color: "white" }}>{card.prefix || ""}{card.value}</div>
                  </div>
                ))}
              </div>
              <div style={{ padding: "16px", borderRadius: 12, background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.15)", fontSize: 13, color: "rgba(255,255,255,0.5)", lineHeight: 1.7 }}>
                <strong style={{ color: accentLight }}>Summary:</strong> Moving GBP 600 from Meta Brand Awareness to Google Search Summer Camps would recover 28% lost impression share and generate an estimated 18 additional conversions per month. Retargeting is performing well and should keep its current allocation. Review in 2 weeks after changes take effect.
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="cta" className="reveal-section" style={{ padding: "120px 40px 140px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", width: "80%", paddingBottom: "40%", bottom: "-20%", left: "50%", transform: "translateX(-50%)", background: `radial-gradient(ellipse, rgba(245,158,11,0.12) 0%, transparent 65%)`, pointerEvents: "none" }} />
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center", position: "relative" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 20, background: `rgba(245,158,11,0.12)`, border: `1px solid rgba(245,158,11,0.25)`, marginBottom: 28 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: accent, boxShadow: `0 0 8px ${accentGlow}` }} className="accent-pulse" />
          ...
          <h2 style={{ fontSize: 50, fontWeight: 900, letterSpacing: "-0.04em", marginBottom: 20, color: "white", lineHeight: 1.05 }} className="blur-reveal">
            Know where to spend.<br />
            <span style={{ background: `linear-gradient(90deg, ${accentLight}, ${accent}, #d97706)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Know why.</span>
          </h2>
          ...
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            <a href="/login" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "15px 28px", borderRadius: 12, background: `linear-gradient(135deg, ${accent}, #d97706)`, color: "white", fontSize: 15, fontWeight: 700, textDecoration: "none", boxShadow: `0 0 32px rgba(245,158,11,0.4)` }} className="cta-accent-pulse">
          ...
          <p style={{ marginTop: 24, fontSize: 12, color: "rgba(255,255,255,0.25)" }}>Budget Intelligence is included with every StratOS plan.</p>
        </div>
      </section>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "24px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 12, color: "rgba(255,255,255,0.25)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span>&copy; {new Date().getFullYear()} i3 Media. Budget Intelligence is part of StratOS.</span>
        </div>
      ...
      <style>{`
        ...
        @keyframes accent-pulse-anim { 0%, 100% { opacity: 1; box-shadow: 0 0 10px rgba(245,158,11,0.9); } 50% { opacity: 0.35; box-shadow: 0 0 24px rgba(245,158,11,0.2); } }
        ...
        @keyframes pulse-accent { 0%, 100% { box-shadow: 0 0 20px rgba(245,158,11,0.4); } 50% { box-shadow: 0 0 40px rgba(245,158,11,0.75), 0 0 80px rgba(245,158,11,0.15); } }
        ...
        .feature-card:hover { transform: translateY(-6px) scale(1.015) !important; border-color: rgba(245,158,11,0.2) !important; box-shadow: 0 16px 48px rgba(245,158,11,0.12); }
        .stat-card:hover { transform: translateY(-6px) scale(1.025) !important; border-color: rgba(245,158,11,0.2) !important; box-shadow: 0 16px 48px rgba(245,158,11,0.12); }
        @keyframes card-float { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-10px); } }
        @keyframes card-glow { 0%, 100% { box-shadow: 0 4px 32px rgba(245,158,11,0.08), 0 0 0 1px rgba(245,158,11,0.1); } 50% { box-shadow: 0 8px 48px rgba(245,158,11,0.22), 0 0 0 1px rgba(245,158,11,0.2); } }
        ...
        .mockup-3d::after { content: ''; position: absolute; left: 0; right: 0; top: 0; height: 1px; background: linear-gradient(90deg, transparent 0%, rgba(245,158,11,0.6) 40%, rgba(245,158,11,0.8) 50%, rgba(245,158,11,0.6) 60%, transparent 100%); animation: scan-line 8s ease-in-out infinite; pointer-events: none; }
