...
export default function SignalsPage() {
  const accent = "#ec4899";
  const accentLight = "#f9a8d4";
  const accentGlow = "rgba(236,72,153,0.6)";
  ...
  const s3 = useCountUp(24, 2100, statsVisible);

  const navLinks = [
    { id: "features", label: "Features" },
    { id: "mockup", label: "Live signals" },
  ...
  return (
    <div style={{ background: "#09090f", color: "white", fontFamily: "inherit" }}>
        ...
        >
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 36 }}>
              {[
                "15 channels monitored for metric anomalies",
                "Three severity levels: critical, warning, info",
                "AI recommends next steps for each signal",
                "Deduplication across computed and AI-generated signals",
              ].map((item, i) => (
      ...
      <style>{`
        ...
        @keyframes accent-pulse-anim { 0%, 100% { opacity: 1; box-shadow: 0 0 10px rgba(236,72,153,0.9); } 50% { opacity: 0.35; box-shadow: 0 0 24px rgba(236,72,153,0.2); } }
        ...
        @keyframes pulse-accent { 0%, 100% { box-shadow: 0 0 20px rgba(236,72,153,0.4); } 50% { box-shadow: 0 0 40px rgba(236,72,153,0.75), 0 0 80px rgba(236,72,153,0.15); } }
        ...
        .feature-card:hover { transform: translateY(-6px) scale(1.015) !important; border-color: rgba(236,72,153,0.2) !important; box-shadow: 0 16px 48px rgba(236,72,153,0.12); }
        .stat-card:hover { transform: translateY(-6px) scale(1.025) !important; border-color: rgba(236,72,153,0.2) !important; box-shadow: 0 16px 48px rgba(236,72,153,0.12); }
        @keyframes card-float { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-10px); } }
        @keyframes card-glow { 0%, 100% { box-shadow: 0 4px 32px rgba(236,72,153,0.08), 0 0 0 1px rgba(236,72,153,0.1); } 50% { box-shadow: 0 8px 48px rgba(236,72,153,0.22), 0 0 0 1px rgba(236,72,153,0.2); } }
        ...
        .mockup-3d::after { content: ''; position: absolute; left: 0; right: 0; top: 0; height: 1px; background: linear-gradient(90deg, transparent 0%, rgba(236,72,153,0.6) 40%, rgba(236,72,153,0.8) 50%, rgba(236,72,153,0.6) 60%, transparent 100%); animation: scan-line 8s ease-in-out infinite; pointer-events: none; }
