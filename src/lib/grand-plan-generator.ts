...
function parseStrategistAudienceNames(raw: string | undefined, limit = 20): string[] {
  if (!raw) return [];
  return raw
    .split(/\n+/)
    .map((line) => {
      ...
      if (!trimmed) return "";
      // Remove bullets, numbering, or decorative prefix symbols.
      const cleaned = trimmed.replace(/^[-•*\d.)\s]+/, "");
      // Keep only the audience name before any description separator.
      const sepMatch = cleaned.match(/[:–—]/);
      const name = sepMatch?.index !== undefined ? cleaned.slice(0, sepMatch.index) : cleaned;
...
function buildAudienceBlock(sources: GrandPlanSources): string {
  ...
  const audienceNames = parseStrategistAudienceNames(sources.targetAudiences, 20);
  const namesBlock = audienceNames.length
    ? `\n\nNamed audiences from strategist (use these EXACT names in recommendations):\n- ${audienceNames.join("\n- ")}`
    : "";
  const fidelityRule = audienceNames.length
    ? "\n\nAudience fidelity rule: recommendations must explicitly reference the named audiences above. Do NOT merge them, rename them, or replace them with generic labels."
    : "";
  return `\n\nTarget audiences for this client (these MUST shape the output — reference them by name, address their pain points, and meet them on the channels they use):\n${sources.targetAudiences}${namesBlock}${fidelityRule}`;
}
...
async function generateContentCalendar(
  ...
  contentData:
    | {
        blogPosts?: Array<{ title?: string; keyword?: string }>;
      }
    | null
    | undefined,
  sources: GrandPlanSources,
...
${context}${buildSharedContextBlocks(sources, "calendar")}`,
      ...
      console.error(
        `[grand-plan] content calendar half "${label}" failed; using placeholder month slots:`,
        err,
      );
      return [];
    }
...
Rules: British English, no AI jargon, no fluff. Each strength/weakness must reference a specific number, keyword, complaint, or scraped messaging signal — no platitudes. Return ONLY valid JSON, no markdown fences.`,
      ...
      console.error(
        "[grand-plan] competitorIntel enrichment failed; using structural fallback:",
        err,
      );
    }
...
async function generateAudiences(
  ...
  const inferredAudienceFloor =
    signalCount >= 3 ? 4 : signalCount === 2 ? 3 : signalCount === 1 ? 2 : 1;
  const inferredAudienceCeiling =
    signalCount >= 3 ? 10 : signalCount === 2 ? 7 : signalCount === 1 ? 5 : 2;

  // Parse strategist-supplied audience names so the AI is constrained to them
  // rather than inventing renamed or merged variants.
  const strategistAudienceNames = parseStrategistAudienceNames(sources.targetAudiences, 20);
  const audienceCountHint = strategistAudienceNames.length || inferredAudienceFloor;
  ...
  const buildFallbackAudiences = (): AudienceItem[] => {
    ...
    const fallbackNames = strategistAudienceNames.length
      ...
      : brainAudiences
          ...
          .filter(Boolean)
          .slice(0, inferredAudienceCeiling);
    ...
    return [
      {
        ...
        description:
          "People with buying responsibility who are currently comparing options in your market.",
        painPoints: [],
  ...
  const res = await withAnthropicRetry("audiences", () =>
    anthropic.messages.create({
      ...
      // Floor at 2200 and ceiling at 10k for long strategist audience lists.
      max_tokens: Math.min(10000, Math.max(2200, audienceCountHint * 700)),
      messages: [
        {
          role: "user",
...
- Do NOT invent audiences. Every audience must be supported by at least one concrete signal from context (strategist brief, GA4 demographics/devices, Search Console intent, or customer voice pain points)
- Return ONLY a valid JSON array, no prose, no markdown fences
- ${strategistAudienceNames.length ? `Produce EXACTLY ${strategistAudienceNames.length} audiences — one per strategist-supplied name below, in the order given.` : `Aim for ${inferredAudienceFloor}-${inferredAudienceCeiling} audiences when evidence supports that range. If context supports fewer, return fewer instead of guessing.`}
...
${context}${buildSharedContextBlocks(sources)}`,
  ...
  const parsedAudiences = parsed
    .filter((a) => a && typeof a.name === "string")
    .slice(0, strategistAudienceNames.length || inferredAudienceCeiling);

  const value = parsedAudiences.map(
    (a, idx): AudienceItem => ({
