...
const toolsNavItems: NavItem[] = [
  ...
  },
  {
    ...
    permission: "internal_linking",
  },
  {
    href: "/tools/meta-audience-scraper",
    label: "Meta Assassin",
  ...
  return (
    <div
      style={{
        margin: "8px 12px 0",
        borderRadius: 8,
        border: isActive ? "1px solid rgba(245,158,11,0.4)" : "1px solid var(--border)",
...
export function Sidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener("platform:open-command-palette", openPalette);
    window.addEventListener("platform:open-shortcuts", openShortcuts);

    return () => {
      window.removeEventListener("platform:open-command-palette", openPalette);
      window.removeEventListener("platform:open-shortcuts", openShortcuts);
    };
  }, []);
      ...
      <nav className="sidebar-nav" aria-label="Main navigation">
        ...
        {navItems
          ...
          .map((item) => {
            ...
            const href =
              item.permission === "settings" && permissions.includes("users")
                ? "/admin/settings"
                : item.href;
  ...
  if (isMobile) {
    return (
      <>
        ...
        <aside
          className={cn("sidebar sidebar-mobile", mobileOpen && "open")}
