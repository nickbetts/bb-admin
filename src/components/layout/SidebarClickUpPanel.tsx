...
import { Loader2, Check, ExternalLink, RefreshCw, Search, Users } from "lucide-react";
...
export function SidebarClickUpPanel() {
  ...
  function toggleGroup(group: ClickUpGroup) {
    const allSelected = group.memberIds.every((id) => selectedAssignees.includes(id));
    if (allSelected) {
        ...
        {loadingMembers ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        background: allSelected ? "var(--accent)" : "var(--bg)",
            ...
            <div style={{ position: "relative" }}>
              <Search style={{ position: "absolute", left: 7, top: "50%", transform: "translateY(-50%)", width: 11, height: 11, color: "var(--text-4)", pointerEvents: "none" }} />
              <input
                type="text"
                value={memberSearch}
                onChange={(e) => setMemberSearch(e.target.value)}
                placeholder="Search members…"
                style={{ ...iStyle, paddingLeft: 24, fontSize: 11 }}
              />
            </div>
            {/* Members list */}
            <div style={{ border: "1px solid var(--border)", borderRadius: 6, overflow: "hidden", maxHeight: 140, overflowY: "auto" }}>
              {filteredMembers.length === 0 ? (
                <p style={{ fontSize: 11, color: "var(--text-4)", margin: 0, padding: "8px", fontStyle: "italic" }}>No members match</p>
