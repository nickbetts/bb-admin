...
export function ClientSettingsForm({
  ...
  const [form, setForm] = useState({
    ...
    competitorDomains: (() => {
      ...
      try {
        return (JSON.parse(client.competitorDomains) as string[]).join("\n");
      } catch {
        return client.competitorDomains;
      }
    })(),
  ...
  const [contractedHours, setContractedHours] = useState<
    Array<{ service: string; hoursPerMonth: number }>
  >(() => {
    try {
      const parsed = client.contractedHours ? JSON.parse(client.contractedHours) : [];
      return Array.isArray(parsed)
        ? (parsed as Array<{ service: string; hoursPerMonth: number }>)
        : [];
    } catch {
      return [];
    }
  ...
  const [clickFraudToken, setClickFraudToken] = useState<string | null>(
    client.clickFraudToken ?? null,
  );
  ...
  const [signalConfig, setSignalConfig] = useState<SignalConfig>(() => {
    ...
    try {
      return JSON.parse(client.signalConfig) as SignalConfig;
    } catch {
      return {};
    }
  });
    ...
    try {
      ...
      if (res.ok) {
        const updated = (await res.json()) as {
          client: Client;
          newlyAddedChannels?: PlatformKey[];
        };
        ...
        if (newlyAddedChannels.length > 0) {
          ...
          if (!backfillRes.ok) {
            ...
            setError(
              `Settings saved, but historical snapshot backfill failed: ${backfillData.error ?? "Unknown error"}`,
            );
            return;
          }
          ...
          if ((backfillData.totalErrors ?? 0) > 0) {
            setError(
              "Settings saved and backfill finished with some channel errors. You can rerun snapshots in Settings.",
            );
          }
        }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: "var(--text-4)", marginTop: 4 }}>
                Used to prefix local phone numbers with the correct international dialling code in
        ...
        <div className="card">
          <div className="card-header">
            <div className="flex items-center gap-3">
              ...
              <h2 className="card-title">SEO Data</h2>
            </div>
          </div>
          <div className="card-body">
            <p className="text-xs" style={{ color: "var(--text-3)" }}>
              Legacy SEO connector has been retired. Use Search Console and GA4 integrations for SEO
              reporting.
            </p>
          </div>
        </div>
          ...
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            >
              ...
              <p style={{ marginBottom: 8, color: "var(--info-text)", lineHeight: 1.5 }}>
                ...
                only appear here once that service account has been granted <strong>Viewer</strong>{" "}
              ...
              {metaLoading ? (
                ...
                <>
                  <input
                    ...
                    className="form-input"
                  />
                  {metaFetchError.toLowerCase().includes("developer registration") ||
                  metaFetchError.toLowerCase().includes("api access deactivated") ? (
                    <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
                      <p className="mb-1 font-semibold">Meta developer registration required</p>
                      <p className="mb-2">
                        The Facebook user account linked to your access token needs to complete
                        developer verification before the API can be used.
                      </p>
                      <ol className="list-inside list-decimal space-y-1 text-amber-700">
                        ...
                        <li>
                          Sign in with the account that generated the{" "}
                        ...
                        <li>
                          Regenerate your access token, update the environment variable, and
                          redeploy
                        </li>
                      </ol>
                    </div>
              ...
              {googleAdsFetchError &&
                ...
                googleAdsFetchError.toLowerCase().includes("expired") ? (
                  <div className="mt-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-xs text-red-800">
                    <p className="mb-1 font-semibold">Google account token expired</p>
                    <p className="mb-2">
                      One or more connected Google accounts have an expired or revoked token. Go to
                      Settings and reconnect the affected account to restore access.
                    </p>
                    <a
                      href="/settings"
                      className="inline-flex items-center gap-1 font-medium text-red-700 underline"
                    >
        ...
        <div className="card">
          ...
          <div className="card-body">
            <p className="text-xs text-slate-500" style={{ marginTop: 0, marginBottom: 12 }}>
              These tags are pre-filled into every new landing page generated for this client. Each
              landing page can override individual fields.
            </p>
            <AnalyticsConfigForm
          ...
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                The WooCommerce store root URL (no trailing slash)
              </p>
            </div>
            <div>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Found in TikTok Ads Manager → Account settings.
              </p>
            </div>
            <div>
              <label className="form-label">Access Token</label>
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Generate via TikTok for Business → Marketing API → Apps. If left blank, the global
                TIKTOK_ACCESS_TOKEN environment variable is used.
              </p>
            </div>
          </div>
        </div>
      ...
      {canSeeSetting("microsoftads") && (
        <div className="card">
          <div className="card-header">
            <div className="flex items-center gap-3">
              <span
                className="flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold"
                style={{ background: "#00a4ef", color: "white" }}
              >
                MS
              </span>
          ...
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              <input
                type="text"
                name="microsoftAdsAccountId"
                value={form.microsoftAdsAccountId}
                onChange={handleChange}
                placeholder="123456789"
                className="form-input"
              />
              <p className="mt-1 text-xs text-slate-500">
                Found in Microsoft Ads → Settings → Account information.
              </p>
            </div>
            <div>
              <label className="form-label">
                Account Name <span className="text-slate-400">(optional)</span>
              </label>
              <input
                type="text"
                name="microsoftAdsAccountName"
                value={form.microsoftAdsAccountName}
                onChange={handleChange}
                placeholder="My Bing Ads Account"
                className="form-input"
              />
            </div>
          </div>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                The URL to fetch real-user Core Web Vitals from Google&apos;s CrUX API. Leave blank
                to use the client website URL. Requires GOOGLE_CRUX_API_KEY environment variable.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="card">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="card-subtitle">
                  Connect LinkedIn Campaign Manager to track B2B ad performance
                </p>
              </div>
            </div>
          </div>
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Found in LinkedIn Campaign Manager → Account Assets → Account Settings.
              </p>
            </div>
            <div>
              <label className="form-label">
              ...
              <input
                ...
                value={form.linkedinAccountName}
                onChange={handleChange}
                placeholder="My Company LinkedIn Ads"
                className="form-input"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="LinkedIn Marketing API OAuth 2.0 access token"
                className="form-input"
                autoComplete="off"
              />
              <p className="mt-1 text-xs text-slate-500">
                Generate via the LinkedIn Developer portal with <code>r_ads_reporting</code> and{" "}
                <code>r_ads</code> scopes.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-header">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="card-subtitle">
                  Connect Klaviyo to track email campaign performance and revenue
                </p>
              </div>
            </div>
          </div>
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Create a Private API Key in Klaviyo → Account → Settings → API Keys. Requires{" "}
                <code>Read-only</code> access to Campaigns.
              </p>
            </div>
            <div>
              <label className="form-label">
              ...
              <input
                type="text"
                name="klaviyoAccountName"
                value={form.klaviyoAccountName}
                onChange={handleChange}
      ...
      {canSeeSetting("hubspot") && (
        <div className="card">
          <div className="card-header">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="card-subtitle">
                  Connect HubSpot to view contacts, open deals and pipeline value
                </p>
              </div>
            </div>
          </div>
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              <label className="form-label">
                Portal ID <span className="text-slate-400">(optional)</span>
              </label>
              <input
                type="text"
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Found in HubSpot → Settings → Account Defaults. Use &ldquo;demo&rdquo; to see sample
                data.
              </p>
            </div>
            <div>
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Create a Private App in HubSpot → Settings → Integrations → Private Apps. Use
                &ldquo;demo&rdquo; to see sample data.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-header">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="card-subtitle">
                  Connect a YouTube channel to track views, watch time, and top videos
                </p>
              </div>
            </div>
          </div>
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Found in YouTube Studio → Settings → Channel → Advanced. Use &ldquo;demo&rdquo; to
                see sample data.
              </p>
            </div>
            <div>
        ...
        <div className="card">
          <div className="card-header">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="card-subtitle">
                  Connect CallRail to track inbound calls, sources and answer rates
                </p>
              </div>
            </div>
          </div>
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Found in CallRail → Settings → Account. Use &ldquo;demo&rdquo; to see sample data.
              </p>
            </div>
            <div>
              <label className="form-label">API Key</label>
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Create in CallRail → Settings → Integrations → API Keys.
              </p>
            </div>
          </div>
        </div>
      ...
      {canSeeSetting("competitors") && (
        <div className="card">
          <div className="card-header">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="card-subtitle">
                  Domains to monitor for competitor analysis — one per line
                </p>
              </div>
            </div>
          </div>
          <div className="card-body">
            ...
            />
            <p className="mt-1 text-xs text-slate-500">
              These domains will be analysed via the Competitor Intelligence tool.
            </p>
          </div>
        </div>
      )}
      ...
      {canSeeSetting("automation") && (
        <div className="card">
          ...
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              <label className="form-label">
                Report Schedule <span className="text-slate-400">(JSON)</span>
              </label>
              <textarea
                name="reportSchedule"
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Set a schedule to auto-generate reports. Example:{" "}
                <code>{`{"frequency":"monthly","dayOfMonth":1,"autoApprove":false}`}</code>
              </p>
            </div>
            <div>
              <label className="form-label">
                Report Delivery Email <span className="text-slate-400">(optional override)</span>
              </label>
              <input
                type="email"
                name="notifyEmail"
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Override the default notification email for this client&apos;s automated reports.
              </p>
            </div>
            <div>
              <label className="form-label">
                Client Contact Emails{" "}
                <span className="text-slate-400">(for email &amp; meeting sync)</span>
              </label>
              <input
                type="text"
                name="contactEmails"
              ...
              />
              <p className="mt-1 text-xs text-slate-500">
                Comma-separated email addresses used by this client. When MS365 is connected, all
                emails and Teams meetings involving these addresses are automatically synced to
                Communications.
              </p>
            </div>
          </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="card-subtitle">
                    Monthly hours allocated per service — used by AI to generate realistic proposal
                    timelines
                  </p>
                </div>
              </div>
              <button
                ...
                onClick={() =>
                  setContractedHours((prev) => [...prev, { service: "", hoursPerMonth: 0 }])
                }
              >
          ...
          <div className="card-body">
            {contractedHours.length === 0 ? (
              <p style={{ fontSize: 13, color: "var(--text-3)" }}>
                No contracted hours set. Click &ldquo;Add Service&rdquo; to define monthly hour
                allocations.
              </p>
            ) : (
                ...
                {contractedHours.map((row, i) => (
                  ...
                  >
                    <input
                      ...
                      value={row.service}
                      onChange={(e) =>
                        setContractedHours((prev) =>
                          prev.map((r, idx) => (idx === i ? { ...r, service: e.target.value } : r)),
                        )
                      }
                    />
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setContractedHours((prev) =>
                            prev.map((r, idx) =>
                              idx === i
                                ? { ...r, hoursPerMonth: parseFloat(e.target.value) || 0 }
                                : r,
                            ),
                          )
                        }
                    ...
                    <button
                      ...
                      onClick={() =>
                        setContractedHours((prev) => prev.filter((_, idx) => idx !== i))
                      }
                    >
                      <Trash2 size={13} />
                    </button>
            ...
            <p className="text-xs text-slate-500" style={{ marginTop: 10 }}>
              These values are included in the AI prompt when generating a proposal from the Keyword
              Planner. The AI will calculate realistic deliverables based on hours and task
              benchmarks set in Settings.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="card">
          <div className="card-header">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="card-subtitle">
                  Generate a unique snippet to detect and log bot/invalid clicks on client landing
                  pages. Stats appear in the Google Ads and Meta dashboards.
                </p>
              </div>
            </div>
          </div>
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      const res = await fetch(`/api/clients/${client.id}/click-fraud-token`, {
                        method: "POST",
                      });
            ...
            <p className="text-xs text-slate-500">
              Add this snippet to the <code>&lt;head&gt;</code> or just before{" "}
              <code>&lt;/body&gt;</code> on the client&apos;s paid ad landing pages. It detects
              known bot user-agents and headless browsers, then logs visits to the dashboard. Stats
              are visible in the Google Ads and Meta sections under &ldquo;Click Fraud
              Protection&rdquo;.
            </p>
          </div>
        </div>
