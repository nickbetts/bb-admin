...
function buildCreativeSummary(creatives: MetaAdCreative[], adSetsData: MetaAdSet[]): string {
  ...
  const campaignMap = new Map<
    string,
    { name: string; adSets: Map<string, { name: string; creatives: MetaAdCreative[] }> }
  >();
    ...
    for (const [asKey, adSet] of camp.adSets) {
      ...
      const asBudget = adSetMeta?.dailyBudget
        ...
        : adSetMeta?.lifetimeBudget
          ...
          : "";
      const asFreq =
        adSetMeta?.frequency && adSetMeta.frequency > 0
          ? ` Freq: ${adSetMeta.frequency.toFixed(1)}x`
          : "";
...
function getPausedAdSetsInActiveCampaigns(
  ...
  const performanceByAdSetId = new Map(adSets.map((adSet) => [adSet.id, adSet]));
  ...
  return adSetAudiences
    ...
    .map((audience) => {
      ...
      return {
        ...
        campaignName: performance?.campaignName ?? campaign?.name ?? "Unknown Campaign",
...
function buildPausedAdSetContext(pausedAdSets: PausedAdSetInsight[]): string | undefined {
  if (!pausedAdSets.length) return undefined;

  const lines = pausedAdSets.slice(0, 8).map((adSet) => {
    const metrics: string[] = [];
    if (adSet.spend > 0) metrics.push(`spent ${formatCurrency(adSet.spend)}`);
    ...
    if (adSet.frequency > 0) metrics.push(`frequency ${adSet.frequency.toFixed(1)}x`);
    if (metrics.length === 0) metrics.push("no delivery in the selected period");
    return `  • "${adSet.adSetName}" in "${adSet.campaignName}" [PAUSED]: ${metrics.join(", ")}`;
  });

  return `PAUSED AD SETS INSIDE ACTIVE CAMPAIGNS (${pausedAdSets.length}):\n${lines.join("\n")}\nUse these as decision context: recommend reactivation only when prior results justify it; otherwise recommend keeping them paused or rebuilding them as fresh tests.`;
}

type MetaAlert = {
  severity: "high" | "medium";
  label: string;
  level: "Campaign" | "Ad Set" | "Creative";
  detail: string;
...
interface MetaInstantExp {
  adId: string;
  adName: string;
  clicksToOpen: number;
  outboundClicks: number;
}
interface MetaCustomConv {
  id: string;
  name: string;
  pixelRule: string;
  customEventType: string;
}
interface MetaSavedAud {
  id: string;
  name: string;
...
export function MetaSection({
  ...
  const [leadForms, setLeadForms] = useState<
    Array<{ formId: string; formName: string; leads: number; costPerLead: number; spend: number }>
  >([]);
  const [relevanceDiagnostics, setRelevanceDiagnostics] = useState<
    Array<{
      adName: string;
  ...
  const pausedAdSetsInActiveCampaigns = useMemo(
    () => getPausedAdSetsInActiveCampaigns(campaignsEnriched, adSets, adSetAudiences),
    [campaignsEnriched, adSets, adSetAudiences],
  );

  const pausedAdSetContext = useMemo(
    () => buildPausedAdSetContext(pausedAdSetsInActiveCampaigns),
    [pausedAdSetsInActiveCampaigns],
  );
  ...
  const metaAlerts = useMemo<MetaAlert[]>(() => {
    ...
    for (const aud of adSetAudiences) {
      ...
      if (isConversionCampaign && aud.customAudiences.length === 0 && spend > 20) {
        alerts.push({
          severity: "medium",
          level: "Ad Set",
          label: aud.adSetName,
  ...
  const audienceDemoContext = useMemo(() => {
    ...
    const lines = adSetAudiences.slice(0, 8).map((a) => {
      ...
      const customs = a.customAudiences
        .slice(0, 2)
        .map((c) => c.name)
        .join(", ");
      const excl = a.excludedAudiences
        .slice(0, 2)
        .map((c) => c.name)
        .join(", ");
  ...
  const metaAiContext = useMemo(
    () =>
      [pausedAdSetContext, creativeSummary, audienceDemoContext].filter(Boolean).join("\n\n") ||
      undefined,
    [pausedAdSetContext, creativeSummary, audienceDemoContext],
  );
    ...
    fetch("/api/ai/summary", {
      ...
      .catch((e) => {
        if (!(e instanceof Error && e.name === "AbortError"))
          console.error("Meta AI recs error", e);
      })
      .finally(() => setAlertAiLoading(false));
    return () => controller.abort();
  }, [metaAlerts, metaAiContext, clientId, clientName, campaignsEnriched, startDate, endDate]);
    ...
    async function fetchData() {
      ...
      try {
        ...
        const [
          ...
          fetch(`${base}&type=relevance-diagnostics`, { signal: controller.signal }).catch(
            () => null,
          ),
          ...
          fetch(`${base}&type=product-performance`, { signal: controller.signal }).catch(
            () => null,
          ),
          fetch(`${base}&type=country-breakdown`, { signal: controller.signal }).catch(() => null),
          fetch(`${base}&type=attribution-settings`, { signal: controller.signal }).catch(
            () => null,
          ),
        ...
        if (ov)
          onMetricsReady?.({
            totalSpend: ov.totalSpend,
            totalImpressions: ov.totalImpressions,
            totalClicks: ov.totalClicks,
            avgCtr: ov.avgCtr,
            avgCpc: ov.avgCpc,
            avgCpm: ov.avgCpm,
            totalConversions: ov.totalConversions,
            avgRoas: ov.avgRoas,
            reach: ov.reach,
        ...
        if (prevOv?.totalSpend != null)
          onPreviousMetricsReady?.({
            totalSpend: prevOv.totalSpend,
            totalImpressions: prevOv.totalImpressions,
            totalClicks: prevOv.totalClicks,
            avgCtr: prevOv.avgCtr,
            avgCpc: prevOv.avgCpc,
            avgCpm: prevOv.avgCpm,
            totalConversions: prevOv.totalConversions,
            avgRoas: prevOv.avgRoas,
            reach: prevOv.reach,
            frequency: prevOv.frequency,
          });
  ...
  useEffect(() => {
    ...
    fetch("/api/ai/snapshots", {
      ...
      console.debug("Snapshot save failed (non-critical):", err);
    });
  ...
  return (
    <div className="flex flex-col gap-8">
        ...
        <>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {highAlerts.length} high-priority · {medAlerts.length} medium-priority issue
                      {metaAlerts.length !== 1 ? "s" : ""} detected
                    </p>
                    {alertAiLoading && (
                      <span
                        style={{
          ...
          {show("kpis") && (
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 gap-5 md:grid-cols-3 xl:grid-cols-6">
                {showCard("kpis", "spend") && (
                  <MetricCard
                    ...
                    change={
                      prevOverview
                        ? pctChange(overview.totalSpend, prevOverview.totalSpend)
                        : undefined
                    }
                    changeDiff={
                      prevOverview
                        ? diffStr(overview.totalSpend, prevOverview.totalSpend, "currency")
                        : undefined
                    }
                  />
                )}
                {showCard("kpis", "impressions") && (
                  <MetricCard
                    title="Impressions"
                    value={formatNumber(overview.totalImpressions)}
                    change={
                      prevOverview
                        ? pctChange(overview.totalImpressions, prevOverview.totalImpressions)
                        : undefined
                    }
                    changeDiff={
                      prevOverview
                        ? diffStr(overview.totalImpressions, prevOverview.totalImpressions, "count")
                        : undefined
                    }
                  />
                )}
                {showCard("kpis", "clicks") && (
                  <MetricCard
                    title="Clicks"
                    value={formatNumber(overview.totalClicks)}
                    change={
                      prevOverview
                        ? pctChange(overview.totalClicks, prevOverview.totalClicks)
                        : undefined
                    }
                    changeDiff={
                      prevOverview
                        ? diffStr(overview.totalClicks, prevOverview.totalClicks, "count")
                        : undefined
                    }
                  />
                )}
                {showCard("kpis", "conversions") && (
                  <MetricCard
                    title={overview.conversionLabel}
                    value={formatNumber(overview.totalConversions)}
                    change={
                      prevOverview
                        ? pctChange(overview.totalConversions, prevOverview.totalConversions)
                        : undefined
                    }
                    changeDiff={
                      prevOverview
                        ? diffStr(overview.totalConversions, prevOverview.totalConversions, "count")
                        : undefined
                    }
                  />
                )}
                {showCard("kpis", "roas") && (
                  <MetricCard
                    ...
                    change={
                      prevOverview ? pctChange(overview.avgRoas, prevOverview.avgRoas) : undefined
                    }
                  />
                )}
                {showCard("kpis", "cpc") && (
                  <MetricCard
                    ...
                    change={
                      prevOverview ? pctChange(prevOverview.avgCpc, overview.avgCpc) : undefined
                    }
                    changeDiff={
                      prevOverview
                        ? diffStr(overview.avgCpc, prevOverview.avgCpc, "currency")
                        : undefined
                    }
                  />
                )}
              ...
              {/* Secondary metrics */}
              {(overview.reach > 0 ||
                overview.outboundClicks > 0 ||
                overview.landingPageViews > 0 ||
                ...
                <div className="grid grid-cols-2 gap-5 md:grid-cols-4">
                  {showCard("kpis", "conv_value") && overview.totalConversionValue > 0 && (
                    <MetricCard
                      ...
                      change={
                        prevOverview?.totalConversionValue != null
                          ? pctChange(
                              overview.totalConversionValue,
                              prevOverview.totalConversionValue ?? 0,
                            )
                          : undefined
                      }
                      changeDiff={
                  ...
                  {showCard("kpis", "reach") && (
                    <MetricCard
                      ...
                      change={
                        prevOverview ? pctChange(overview.reach, prevOverview.reach) : undefined
                      }
                      changeDiff={
                        prevOverview
                          ? diffStr(overview.reach, prevOverview.reach, "count")
                          : undefined
                      }
                    />
                  )}
                  {showCard("kpis", "frequency") && (
                    <MetricCard
                      title="Frequency"
                      value={overview.frequency.toFixed(2)}
                      change={
                        prevOverview
                          ? pctChange(overview.frequency, prevOverview.frequency)
                          : undefined
                      }
                    />
                  )}
                  {showCard("kpis", "outbound_clicks") && (
                    <MetricCard
                      title="Outbound Clicks"
                      value={formatNumber(overview.outboundClicks)}
                      change={
                        prevOverview
                          ? pctChange(overview.outboundClicks, prevOverview.outboundClicks)
                          : undefined
                      }
                      changeDiff={
                        prevOverview
                          ? diffStr(overview.outboundClicks, prevOverview.outboundClicks, "count")
                          : undefined
                      }
                    />
                  )}
                  {showCard("kpis", "landing_page_views") && (
                    <MetricCard
                      title="Landing Page Views"
                      value={formatNumber(overview.landingPageViews)}
                      change={
                        prevOverview
                          ? pctChange(overview.landingPageViews, prevOverview.landingPageViews)
                          : undefined
                      }
                      changeDiff={
                        prevOverview
                          ? diffStr(
                              overview.landingPageViews,
                              prevOverview.landingPageViews,
                              "count",
                            )
                          : undefined
                      }
                ...
                return (
                  <div className="flex flex-col gap-6">
                    {displayCampaigns.map((camp) => {
                      ...
                      const campCreatives = campAdSets.flatMap(
                        (as) => creativesByAdSet.get(as.id) ?? [],
                      );
                      const campPurchaseValue = camp.roas * camp.spend;
                      const prevPurchaseValue =
                        prevC != null ? prevC.roas * prevC.spend : undefined;
                      ...
                      const stats: {
                        ...
                        fmt: "currency" | "count" | "none";
                      }[] = [
                        {
                          label: "Spend",
                          display: formatCurrency(camp.spend),
                          current: camp.spend,
                          previous: prevC?.spend,
                          fmt: "currency",
                        },
                        {
                          label: "Impressions",
                          display: formatNumber(camp.impressions),
                          current: camp.impressions,
                          previous: prevC?.impressions,
                          fmt: "count",
                        },
                        {
                          label: "Clicks",
                          display: formatNumber(camp.clicks),
                          current: camp.clicks,
                          previous: prevC?.clicks,
                          fmt: "count",
                        },
                        {
                          label: "Purchases",
                          display: formatNumber(camp.conversions),
                          current: camp.conversions,
                          previous: prevC?.conversions,
                          fmt: "count",
                        },
                        {
                          label: "Purchase Value",
                          display: formatCurrency(campPurchaseValue),
                          current: campPurchaseValue,
                          previous: prevPurchaseValue,
                          fmt: "currency",
                        },
                      ];
                        ...
                        >
                          ...
                          <div
                            style={{
                              ...
                              paddingBottom: 20,
                              marginBottom:
                                campAdSets.length > 0 || campCreatives.length > 0 ? 24 : 0,
                              borderBottom:
                                campAdSets.length > 0 || campCreatives.length > 0
                              ...
                              <DataTable<MetaAdSet>
                                ...
                                columns={[
                                      ...
                                      <div>
                                        ...
                                        >
                                          {[
                                            ...
                                            (creativesByAdSet.get(adSet.id) ?? []).length > 0
                                              ? `${(creativesByAdSet.get(adSet.id) ?? []).length} ad${(creativesByAdSet.get(adSet.id) ?? []).length !== 1 ? "s" : ""}`
                                              : null,
                                          ]
                                            .filter(Boolean)
                                            .join(" · ")}
                                  ...
                                  },
                                  {
                                    ...
                                    label: "Value",
                                    align: "right",
                                    render: (_, adSet) => formatCurrency(adSet.roas * adSet.spend),
                                  },
                                  {
                                    key: "roas",
                                    label: "ROAS",
                              ...
                              <DataTable<MetaAdCreative>
                                ...
                                columns={[
                                  ...
                                  },
                                  {
                                    ...
                                    label: "Value",
                                    align: "right",
                                    render: (_, cr) => formatCurrency(cr.roas * cr.spend),
                                  },
                                  {
                                    key: "roas",
                                    label: "ROAS",
                  ...
                  <div className="card-header">
                    <div>
                      ...
                      <p className="card-subtitle">
                        Click campaigns to expand ad sets, then ad sets to see ad creatives
                      </p>
                    </div>
                  </div>
                  <div
                        ...
                        {displayCampaigns.map((camp) => {
                          ...
                          return (
                            <React.Fragment key={camp.id}>
                              {/* Campaign row */}
                              <tr
                                className={`cursor-pointer transition ${isExpanded ? "bg-[var(--border-subtle)]" : "hover:bg-[var(--border-subtle)]"}`}
                              ...
                              >
                                ...
                                >
                                  {camp.conversions > 0
                                    ? formatCurrency(camp.spend / camp.conversions)
                                    : "—"}
                                </td>
                                <td
                                  style={{
                                ...
                                >
                                  <span
                                    className={`font-semibold ${camp.roas >= 2 ? "text-emerald-600" : camp.roas >= 1 ? "text-amber-600" : "text-red-600"}`}
                                  >
                                    {camp.roas.toFixed(2)}x
                                  </span>
                                ...
                                >
                                  {typeof enriched.frequency === "number"
                                    ? enriched.frequency.toFixed(2)
                                    : "—"}
                                </td>
                                <td
                                  style={{
                                ...
                                campAdSets.map((adSet) => {
                                  ...
                                  return (
                                    <React.Fragment key={adSet.id}>
                                      {/* Ad set row */}
                                      <tr
                                        className={`cursor-pointer transition ${asExpanded ? "bg-blue-50/40" : "hover:bg-[var(--border-subtle)]"}`}
                                      ...
                                      >
                                          ...
                                          <div className="flex items-center gap-2">
                                            ...
                                            <div className="min-w-0">
                                              <p className="truncate text-[11px] font-medium text-[var(--text)]">
                                                <Layers className="-mt-0.5 mr-1 inline-block h-3 w-3 text-blue-400" />
                                                {adSet.name}
                                              </p>
                                              <p className="mt-0.5 text-[10px] text-[var(--text-3)]">
                                                {adSet.optimizationGoal || adSet.status}
                                                {asCreatives.length > 0 &&
                                                  ` · ${asCreatives.length} ad${asCreatives.length > 1 ? "s" : ""}`}
                                              </p>
                                            </div>
                                          </div>
                                        </td>
                                        ...
                                        >
                                          {adSet.conversions > 0
                                            ? formatCurrency(adSet.spend / adSet.conversions)
                                            : "—"}
                                        </td>
                                        <td
                                          style={{
                                        ...
                                        >
                                          <span
                                            className={`text-[11px] font-semibold ${adSet.roas >= 2 ? "text-emerald-600" : adSet.roas >= 1 ? "text-amber-600" : "text-red-600"}`}
                                          >
                                          ...
                                          >
                                            ...
                                            >
                                              <div className="flex items-center gap-3">
                                                ...
                                                <button
                                                  type="button"
                                                  className="relative shrink-0 cursor-pointer overflow-hidden rounded-md border border-[var(--border)] transition-all hover:ring-2 hover:ring-indigo-300"
                                                  style={{
                                                    width: 56,
                                                    height: 56,
                                                    background: "var(--bg)",
                                                ...
                                                <div className="min-w-0">
                                                  ...
                                                  <span
                                                    className="mt-1 inline-block rounded px-1.5 py-0.5 text-[9px] font-bold tracking-wider uppercase"
                                                    style={{
                                                      background:
                                                        cr.mediaType === "VIDEO"
                                                          ? "#ede9fe"
                                            ...
                                            >
                                              {cr.conversions > 0
                                                ? formatCurrency(cr.costPerConversion)
                                                : "—"}
                                            </td>
                                            <td
                                              style={{
                                            ...
                                            >
                                              <span
                                                className={`text-[11px] font-semibold ${cr.roas >= 2 ? "text-emerald-600" : cr.roas >= 1 ? "text-amber-600" : "text-red-600"}`}
                                              >
                                                {cr.roas.toFixed(2)}x
                                              </span>
                                            </td>
        ...
        <SuperSummary
          ...
          campaignData={
            campaignsEnriched.length
              ? (campaignsEnriched as unknown as Record<string, unknown>[])
              : undefined
          }
          landingPages={landingPages.length ? landingPages : undefined}
          clientName={clientName}
          dateRange={`${formatDateDisplay(startDate)} – ${formatDateDisplay(endDate)}`}
          extraContext={metaAiContext}
      ...
      {!hideAi && !loading && !error && overview && (
        <AiInsightsPanel
          ...
          metrics={{
            ...
            landingPageViews: overview.landingPageViews,
            avgFrequency:
              campaignsEnriched.length > 0
                ? campaignsEnriched.reduce((s, c) => s + (c.frequency ?? 0), 0) /
                  campaignsEnriched.length
                : overview.frequency,
          }}
          campaignData={
      ...
      {!hideAi && !loading && !error && landingPages.length > 0 && (
        <AiLandingPageAnalysis landingPages={landingPages} clientName={clientName} source="meta" />
      )}
      ...
      {show("lead_forms") && leadForms.length > 0 && (
        <SectionCard
          title="Lead Form Performance"
          subtitle={`${leadForms.length} form${leadForms.length !== 1 ? "s" : ""} with lead data`}
        >
          <DataTable
            data={leadForms}
            pageSize={0}
      ...
      {show("relevance") && relevanceDiagnostics.length > 0 && (
        <SectionCard
          ...
          subtitle="Quality, engagement, and conversion ranking per ad"
        >
          <DataTable
            ...
            columns={[
              { key: "adName", label: "Ad Name" },
              {
                ...
                render: (v) => {
                  ...
                  return (
                    <span
                      className={`inline-block rounded-full border px-2 py-0.5 text-[10px] font-semibold ${cls}`}
                    >
                      {label}
                    </span>
                  );
      ...
      {show("placements") && placements.length > 0 && (
        <SectionCard
          ...
          subtitle={`Performance across ${placements.length} placement${placements.length !== 1 ? "s" : ""}`}
        >
          <DataTable<MetaPlacement>
            ...
            columns={[
              { key: "publisherPlatform", label: "Platform" },
              {
                key: "placement",
                label: "Placement",
                render: (v) => (v as string).replace(/_/g, " "),
              },
              {
                key: "spend",
                label: "Spend",
              ...
              },
              {
                ...
                render: (v) => (
                  <span
                    className={`font-semibold ${(v as number) >= 2 ? "text-emerald-600" : (v as number) >= 1 ? "text-amber-600" : "text-red-600"}`}
                  >
                    {(v as number).toFixed(2)}x
                  </span>
                ),
      ...
      {show("audiences") &&
        ...
        (() => {
          ...
          return (
            <SectionCard
              title="Audience Targeting Performance"
              subtitle={`Performance by targeting configuration — ${enrichedAudiences.length} ad set${enrichedAudiences.length !== 1 ? "s" : ""}`}
            >
              {chartData.length > 0 && (
                  ...
                  { key: "geoSummary", label: "Location", render: (v) => (v as string) || "All" },
                  {
                    ...
                    render: (_, aud) => {
                      ...
                      if (a.interests.length > 0)
                        parts.push(
                          a.interests.slice(0, 2).join(", ") +
                            (a.interests.length > 2 ? ` +${a.interests.length - 2}` : ""),
                        );
                      if (a.customAudiences.length > 0)
                        parts.push(
                          `Custom: ${a.customAudiences
                            .map((c) => c.name)
                            .slice(0, 2)
                            .join(", ")}`,
                        );
      ...
      {show("audiences") &&
        adSetAudiences.some((a) => a.customAudiences.length > 0) &&
        (() => {
            ...
            >
              ...
              {stackedCount > 0 && (
                ...
                >
                  ...
                  <div>
                    <strong>API limitation:</strong> Meta does not split performance by individual
                    custom audience when an ad set stacks multiple audiences. {stackedCount} of your{" "}
                    {rows.length} ad set{rows.length !== 1 ? "s" : ""} target
                    {stackedCount === 1 ? "s" : ""} more than one audience, so their numbers are
                    reported as a combined total. To get true per-audience attribution, isolate each
                    custom audience in its own ad set.
                  </div>
                </div>
              )}
              ...
              <DataTable
                ...
                columns={[
                          ...
                          <p style={{ fontSize: 10, color: "var(--text-3)", marginTop: 2 }}>
                            {row.isolated ? (
                              ...
                              <span style={{ color: "#d97706", fontWeight: 600 }}>
                                ● {row.audienceCount} audiences stacked
                              </span>
                            )}
                          </p>
                        </div>
                    ...
                    render: (_v, r) => {
                      ...
                      return (
                        <div style={{ fontSize: 11, color: "var(--text-2)" }}>
                          {row.audiences.map((a, i) => (
                            <div key={i}>• {a}</div>
                          ))}
                          {row.excluded.length > 0 && (
                            <div style={{ marginTop: 4, color: "#dc2626" }}>
                              {row.excluded.map((a, i) => (
                                <div key={i}>✕ Excluded: {a}</div>
                              ))}
                            </div>
                          )}
                        </div>
                  ...
                  },
                  {
                    ...
                    align: "right",
                    sortable: true,
                    render: (v) => ((v as number) > 0 ? formatCurrency(v as number) : "—"),
                  },
                  {
                    key: "roas",
                    label: "ROAS",
              ...
              {isolatedRows.length > 0 && (
                ...
                >
                  ...
                  <p style={{ fontSize: 12, color: "var(--text-3)", marginBottom: 12 }}>
                    These ad sets target a single custom audience, so the metrics below are
                    attributable to that exact audience.
                  </p>
                  <DataTable
                    data={isolatedRows.map((r) => ({
                      audience: r.audiences[0],
      ...
      {show("demographics") &&
        ...
        (() => {
          const byAge = Object.values(
            ...
            >((acc, d) => {
              if (!acc[d.age])
                acc[d.age] = { age: d.age, spend: 0, clicks: 0, impressions: 0, conversions: 0 };
      ...
      {show("reach_estimate") && adSets.length > 0 && (
        <SectionCard
          ...
          subtitle="Top ad sets by unique reach with frequency analysis"
        >
          <DataTable<MetaAdSet>
            ...
            columns={[
              ...
              },
              {
                ...
                label: "Campaign",
                render: (_v, row) => (
                  <span style={{ color: "var(--text-3)" }}>{row.campaignName}</span>
                ),
              },
              {
                key: "reach",
                ...
                align: "right",
                sortable: true,
                render: (_v, row) => formatNumber(row.reach),
              },
              {
                key: "impressions",
                label: "Impressions",
                align: "right",
                sortable: true,
                render: (_v, row) => formatNumber(row.impressions),
              },
              {
                key: "frequency",
                label: "Frequency",
                      ...
                      color:
                        row.frequency > 4
                          ...
                          : row.frequency > 2.5
                            ? "var(--warning)"
                            : "var(--text-2)",
                      fontWeight: row.frequency > 2.5 ? 600 : 400,
                    }}
                  >
              ...
              },
              {
                ...
                align: "right",
                sortable: true,
                render: (_v, row) => formatCurrency(row.spend),
              },
            ]}
            pageSize={0}
            exportable
          ...
          <p style={{ fontSize: 11, color: "var(--text-3)", marginTop: 10 }}>
            Frequency above 2.5 may indicate audience fatigue; above 4 strongly suggests creative
            refresh or audience expansion.
          </p>
        </SectionCard>
      )}
      ...
      {isExplicit("product_performance") && productPerformance.length === 0 && (
        <EmptyBlockState
          ...
          message="No product-level data — connect a product catalog to enable."
        />
      )}
      {show("product_performance") && productPerformance.length > 0 && (
        <SectionCard
          title="Product Performance"
          subtitle={`${productPerformance.length} product${productPerformance.length !== 1 ? "s" : ""} with ad data`}
        >
      ...
      {show("country_breakdown") && countryBreakdown.length > 0 && (
        <SectionCard
          ...
          subtitle={`Performance across ${countryBreakdown.length} ${countryBreakdown.length !== 1 ? "countries" : "country"}`}
        >
                ...
                render: (v) => {
                  ...
                  try {
                    ...
                    if (Array.isArray(spec))
                      windowLabel = spec
                        .map((s: Record<string, string>) => `${s.event_type}: ${s.window_days}d`)
                        .join(", ");
                  } catch {
                    /* use raw string */
                  }
      ...
      {show("custom_conversions") && customConversions.length > 0 && (
        <SectionCard
          ...
          subtitle={`${customConversions.length} custom conversion${customConversions.length !== 1 ? "s" : ""} configured`}
        >
      ...
      {show("saved_audiences") && savedAudiences.length > 0 && (
        <SectionCard
          title="Saved & Custom Audiences"
          subtitle={`${savedAudiences.length} audience${savedAudiences.length !== 1 ? "s" : ""} available`}
        >
          <DataTable<MetaSavedAud>
            ...
            columns={[
              ...
              { key: "type", label: "Type", render: (v) => (v as string).replace(/_/g, " ") },
              {
                key: "subtype",
                label: "Subtype",
                render: (v) => (v ? (v as string).replace(/_/g, " ").toLowerCase() : "—"),
              },
              {
                ...
                align: "right",
                sortable: true,
                render: (v) => ((v as number) > 0 ? formatNumber(v as number) : "—"),
              },
            ]}
          />
        </SectionCard>
            ...
            columns={[
              ...
              },
              {
                ...
                render: (_, s) => {
                  ...
                  return util != null ? (
                    <span
                      className={`font-semibold ${util >= 90 ? "text-red-600" : util >= 70 ? "text-amber-600" : "text-emerald-600"}`}
                    >
                      {util.toFixed(0)}%
                    </span>
                  ) : (
      ...
      {show("hourly_breakdown") &&
        ...
        (() => {
          ...
          const sorted = [...hourlyBreakdown].sort(
            (a, b) => parseHour(a.hourOfDay) - parseHour(b.hourOfDay),
          );
          return (
            <SectionCard
              title="Hourly Performance"
              subtitle="Performance by hour of day (advertiser timezone)"
            >
      ...
      {lightbox && (
        ...
        >
          <button
            type="button"
            className="absolute top-4 right-4 z-10 text-white hover:text-[var(--text-3)]"
            onClick={(e) => {
              e.stopPropagation();
              setLightbox(null);
            }}
          ...
          <div className="relative max-h-[85vh] max-w-[90vw]" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-3 max-w-[90vw] truncate text-center text-sm text-white/80">
              {lightbox.title}
            </p>
          </div>
        </div>
