...
export function SemrushSection({
  ...
  const isExplicit = (block: string) =>
    Array.isArray(visibleBlocks) && visibleBlocks.includes(block);
  ...
  const [contentGap, setContentGap] = useState<
    Array<{ keyword: string; volume: number; difficulty: number; competitors: string[] }>
  >([]);
  const [serpFeatures, setSerpFeatures] = useState<
    Array<{ feature: string; count: number; percentage: number }>
  >([]);
  const [backlinkChanges, setBacklinkChanges] = useState<
    Array<{ url: string; type: string; domain: string; firstSeen: string; lost: boolean }>
  >([]);
  ...
  const [adCopyIntelligence, setAdCopyIntelligence] = useState<
    Array<{
      title: string;
      description: string;
      url: string;
  ...
  >([]);
  const [displayAdvertising, setDisplayAdvertising] = useState<
    Array<{ domain: string; displayAds: number; displayTraffic: number; displayCost: number }>
  >([]);
  const [shoppingCompetitors, setShoppingCompetitors] = useState<
    Array<{
      domain: string;
  ...
  >([]);
  const [backlinkComparison, setBacklinkComparison] = useState<
    Array<{
      domain: string;
      ascore: number;
  ...
  const [taggedFetchRequested, setTaggedFetchRequested] = useState<boolean>(() =>
    Boolean(hideAlerts),
  );
  ...
  const visibleTaggedKws = useMemo(() => {
    let kws = showUnranked
      ? taggedKeywords
      : taggedKeywords.filter((kw) => kw.currentPosition != null);
  ...
  function setTagFilter(tag: string | null) {
    ...
    if (typeof window !== "undefined") {
      ...
      if (tag) params.set("semrush_tag", tag);
      else params.delete("semrush_tag");
    ...
    async function fetchData() {
      ...
      try {
        const fetchList: Promise<Response | null>[] = [
          ...
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=rank_movers`, {
            signal: controller.signal,
          }),
          ...
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=competitors`, {
            signal: controller.signal,
          }),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=backlinks`, {
            signal: controller.signal,
          }),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=content-gap`, {
            signal: controller.signal,
          }),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=serp-features`, {
            signal: controller.signal,
          }),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=backlink-changes`, {
            signal: controller.signal,
          }),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=topic-research`, {
            signal: controller.signal,
          }).catch(() => null),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=site-audit`, {
            signal: controller.signal,
          }).catch(() => null),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=ad-copy`, {
            signal: controller.signal,
          }).catch(() => null),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=display-advertising`, {
            signal: controller.signal,
          }).catch(() => null),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=shopping-competitors`, {
            signal: controller.signal,
          }).catch(() => null),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=keyword-trends`, {
            signal: controller.signal,
          }).catch(() => null),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=referring-domains`, {
            signal: controller.signal,
          }).catch(() => null),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=anchor-text`, {
            signal: controller.signal,
          }).catch(() => null),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=backlink-comparison`, {
            signal: controller.signal,
          }).catch(() => null),
          fetch(`/api/semrush?domain=${encodeURIComponent(domain)}&type=position-changes`, {
            signal: controller.signal,
          }).catch(() => null),
        ];
        ...
        if (activeCampaignId) {
          fetchList.push(
            fetch(
              `/api/semrush?type=project-keywords&campaignId=${encodeURIComponent(activeCampaignId)}`,
              { signal: controller.signal },
            ),
          );
          fetchList.push(
            fetch(
              `/api/semrush?type=ai-visibility&campaignId=${encodeURIComponent(activeCampaignId)}`,
              { signal: controller.signal },
            ),
          );
          if (taggedFetchRequested) {
            setTaggedKwError(null);
            fetchList.push(
              fetch(
                `/api/semrush?type=tagged-positions&campaignId=${encodeURIComponent(activeCampaignId)}&domain=${encodeURIComponent(domain)}&period=custom&dateBegin=${fmtYMD(startDate)}&dateEnd=${fmtYMD(endDate)}`,
                { signal: controller.signal },
              ).catch(() => null),
            );
          } else {
        ...
        const [ov, kw, movers, hist, dist, comps, bls] = await Promise.all([
          ...
          backlinksRes!.ok
            ...
            : backlinksRes!
                .json()
                .then((e) => {
                  setBacklinkError(e.error ?? "Failed to load backlinks");
                  return [];
                })
                .catch(() => []),
        ]);
        ...
        if (ov)
          onMetricsReady?.({
            ...
            organicCost: ov.organicCost,
            paidTraffic: ov.paidTraffic,
            paidKeywords: ov.paidKeywords,
          });
        ...
        if (taggedFetchRequested && taggedRes?.ok) {
          const tagged = await taggedRes.json().catch(() => []);
          setTaggedKeywords(Array.isArray(tagged) ? tagged : []);
        } else if (taggedFetchRequested && taggedRes && !taggedRes.ok) {
  ...
  }, [domain, projectId, campaignIdsKey, startDate, endDate, taggedFetchRequested]);
  ...
  const semrushAlerts = useMemo<SemrushAlert[]>(() => {
    ...
    if (trafficPct != null && trafficPct <= -25)
      alerts.push({
        ...
        detail: `Organic traffic dropped ${Math.abs(trafficPct).toFixed(0)}% month-over-month (${formatNumber(prev.organicTraffic)} \u2192 ${formatNumber(curr.organicTraffic)})`,
        recommendation:
          "Investigate for algorithm updates, lost backlinks, or deindexed pages. Review Search Console for coverage issues and check rankings for top pages.",
      });
    else if (trafficPct != null && trafficPct <= -10)
      alerts.push({
        severity: "medium",
        label: "Organic Traffic",
        detail: `Organic traffic declined ${Math.abs(trafficPct).toFixed(0)}% month-over-month`,
        recommendation:
          "Monitor for a continued trend. Review keyword rankings and top pages for position changes. Check for seasonal traffic patterns.",
      });
    ...
    if (keywordPct != null && keywordPct <= -20)
      alerts.push({
        ...
        detail: `Ranking keywords dropped ${Math.abs(keywordPct).toFixed(0)}% (${formatNumber(prev.organicKeywords)} \u2192 ${formatNumber(curr.organicKeywords)})`,
        recommendation:
          "Check for site-wide issues \u2014 technical SEO problems, major content removal, or domain-level penalties. Audit indexation status.",
      });
    else if (keywordPct != null && keywordPct <= -10)
      alerts.push({
        severity: "medium",
        label: "Ranking Keywords",
        detail: `Ranking keywords declined ${Math.abs(keywordPct).toFixed(0)}% month-over-month`,
        recommendation:
          "Review recently dropped keywords and the pages they targeted. Check for content freshness issues or increased competitor activity.",
      });

    // Keywords losing page 1 positions
    const losingPage1 = keywords.filter(
      (k) => k.position > 10 && k.previousPosition > 0 && k.previousPosition <= 10,
    );
    if (losingPage1.length >= 5)
      alerts.push({
        severity: "high",
        label: "Page 1 Losses",
        detail: `${losingPage1.length} keywords dropped off page 1 this period`,
        recommendation:
          "Prioritise content refresh and internal linking for these pages. Consider acquiring backlinks to restore authority for high-volume terms.",
      });
    else if (losingPage1.length >= 2)
      alerts.push({
        severity: "medium",
        label: "Page 1 Losses",
        detail: `${losingPage1.length} keyword${losingPage1.length === 1 ? "" : "s"} dropped off page 1`,
        recommendation:
          "Review the affected keywords and update content to match current search intent. Strengthen internal linking to these pages.",
      });

    // Competitors outpacing organic traffic
    if (overview && competitors.length > 0) {
      const outpacing = competitors.filter(
        (c) => c.organicTraffic > overview.organicTraffic * 1.5 && c.commonKeywords > 50,
      );
      if (outpacing.length >= 2)
        alerts.push({
          severity: "medium",
          label: "Competitor Gap",
          detail: `${outpacing.length} competitors have 50%+ more organic traffic with significant keyword overlap`,
          recommendation:
            "Conduct a gap analysis on the top competitors. Identify high-value keywords they rank for that you don\u2019t, and create targeted content to close the gap.",
        });
    }
    ...
    if (overview && overview.organicCost > 5000 && overview.organicTraffic < 1000)
      alerts.push({
        severity: "medium",
        label: "High Organic Value",
        detail: `Organic keyword portfolio valued at ${formatCurrency(overview.organicCost)} but only ${formatNumber(overview.organicTraffic)} monthly visits`,
        recommendation:
          "The keyword portfolio has high CPC value. Focus on improving rankings for high-CPC keywords already in positions 5\u201320 to capture more of this traffic value.",
      });
  ...
  useEffect(() => {
    ...
    fetch("/api/ai/summary", {
      ...
      .then((json) => {
        if (json?.recommendations?.length) setAlertAiRecs(json.recommendations);
      })
      .catch(() => {})
        ...
        <>
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      issue{semrushAlerts.length !== 1 ? "s" : ""} detected
                    </p>
                    {alertAiLoading && (
                      <span
                        style={{
                        ...
                        >
                          <span
                            style={{
                              ...
                              textTransform: "uppercase",
                              background: alertAiRecs[i] ? "#d1fae5" : "#f0fdf4",
                              color: alertAiRecs[i] ? "#065f46" : "#0f766e",
                              borderRadius: 4,
                              padding: "1px 5px",
                              marginRight: 6,
                            }}
          ...
          {show("kpis") && (
            <div className="grid grid-cols-2 gap-5 md:grid-cols-3">
              {showCard("kpis", "organic_traffic") && (
                <MetricCard
                  ...
                  change={
                    history.length >= 2
                      ? pctChange(
                          history[history.length - 1].organicTraffic,
                          history[history.length - 2].organicTraffic,
                        )
                      : undefined
                  }
                  changeDiff={
              ...
              {showCard("kpis", "organic_keywords") && (
                <MetricCard
                  ...
                  change={
                    history.length >= 2
                      ? pctChange(
                          history[history.length - 1].organicKeywords,
                          history[history.length - 2].organicKeywords,
                        )
                      : undefined
                  }
                  changeDiff={
          ...
          {show("ranking_distribution") && history.length > 0 && (
            <SectionCard title="Organic Traffic Trend" subtitle={`${domain} — last 12 months`}>
          ...
          {(show("ranking_distribution") || show("top_keywords")) && (
            <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              {show("ranking_distribution") && distribution.length > 0 && (
                <SectionCard title="Keyword Position Distribution" subtitle="SERP positions">
              ...
              {/* Keyword history */}
              {history.length > 0 && (
                <SectionCard title="Keyword Count Trend" subtitle="Total ranking keywords">
                  <ResponsiveContainer width="100%" height={240}>
                    <AreaChart data={history}>
                      <defs>
          ...
          {show("top_keywords") && keywords.length > 0 && (
            <SectionCard title="Top Organic Keywords" subtitle="By traffic percentage">
              <DataTable<Keyword>
                data={keywords}
                searchable
                exportable
                ...
                columns={[
                  {
                    ...
                    render: (_v, row) => (
                      <div>
                        <p className="max-w-[200px] truncate font-medium text-[var(--text)]">
                          {row.keyword}
                        </p>
                        <p className="max-w-[200px] truncate text-xs text-[var(--text-3)]">
                          {row.url}
                        </p>
                      </div>
                    ),
          ...
          {show("rank_improvers") && (
            <SectionCard
              title="Top Rank Improvers"
              subtitle="Keywords with biggest position gains this month"
            >
              {rankMovers.length === 0 ? (
                <p
                  style={{
                ...
                >
                  No position improvements detected this period — SEMrush may not yet have
                  comparison data for this domain.
                </p>
              ) : (
                <DataTable<Keyword>
                  data={rankMovers}
          ...
          {show("tracked_keywords") && (
            <SectionCard
              ...
              subtitle="Positions from your SEMrush Position Tracking campaign"
            >
                  ...
                  columns={[
                    {
                      ...
                      render: (_v, row) => (
                        <div>
                          <p className="max-w-[220px] truncate font-medium text-[var(--text)]">
                            {row.keyword}
                          </p>
                          {row.landingPage && (
                            <p className="max-w-[220px] truncate text-xs text-[var(--text-3)]">
                              {row.landingPage}
                            </p>
                          )}
                        </div>
          ...
          {show("tagged_kw_positions") && (campaignIds?.[0] ?? null) && (
            ...
            >
              {!taggedFetchRequested ? (
                <div className="py-2">
                  <p className="mb-3 text-sm text-[var(--text-3)]">
                    Tag-level rankings are loaded on demand to reduce SEMrush API usage.
                  </p>
                  <button onClick={() => setTaggedFetchRequested(true)} className="btn btn-sm">
                    Load Tag Rankings
                  </button>
                </div>
              ) : taggedKwError ? (
                <p className="py-2 text-sm text-red-600">{taggedKwError}</p>
              ) : taggedKeywords.length === 0 ? (
                <p className="py-2 text-sm text-[var(--text-4)] italic">
                  No keyword data returned. SEMrush may still be crawling or the campaign may not
                  have any tracked keywords.
                </p>
              ) : (
                <>
                  {/* SERP Feature legend */}
                  <details className="group mb-5">
                    <summary className="inline-flex cursor-pointer list-none items-center gap-1.5 text-xs text-[var(--text-3)] select-none hover:text-[var(--text-2)]">
                      <svg
                        width="12"
                        height="12"
                        viewBox="0 0 24 24"
                    ...
                    <div className="mt-2 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-subtle,#f8f9fb)] p-4 text-xs text-[var(--text-2)]">
                      <p className="mb-2 text-[var(--text-3)]">
                        Badges show which Google search features appear for each keyword.{" "}
                        <strong className="text-[var(--text-2)]">Solid badges with ✓</strong> mean
                        your site appears in that feature.
                      </p>
                      <div className="grid grid-cols-2 gap-x-8 gap-y-2 sm:grid-cols-3">
                        {(
                          [
                            ...
                            [
                              "aio",
                              "AI Overview — client is cited as a source in Google's AI-generated answer.",
                            ],
                          ...
                          <div key={code} className="flex items-center gap-2">
                            <span className="inline-block shrink-0 rounded bg-[var(--border-subtle)] px-1.5 py-0.5 text-[9px] font-medium text-[var(--text-2)] uppercase">
                              {code}
                            </span>
                      ...
                      <div className="mb-4 flex flex-wrap items-center gap-2">
                            ...
                            <button
                              ...
                              className={`rounded-full border px-2.5 py-1 text-xs font-medium transition-colors ${
                                activeTagFilter == null
                                  ...
                                  : "border-[var(--border)] bg-transparent text-[var(--text-2)] hover:border-indigo-400 hover:text-indigo-600"
                              }`}
                            >
                              All
                            </button>
                        ...
                        {taggedKeywords.some((kw) => kw.currentPosition == null) && (
                          <button
                            ...
                            className={`ml-auto rounded-full border px-2.5 py-1 text-xs font-medium transition-colors ${
                              showUnranked
                                ? "border-slate-600 bg-slate-600 text-white"
                                : "border-[var(--border)] bg-transparent text-[var(--text-3)] hover:border-slate-400"
                            }`}
                          >
                            {showUnranked
                              ? "Hide unranked"
                              : `Show unranked (${taggedKeywords.filter((kw) => kw.currentPosition == null).length})`}
                          </button>
                        )}
                      </div>
                    )}
                  <DataTable<TaggedKeyword>
                    ...
                    columns={[
                      ...
                      },
                      {
                        ...
                        sortable: true,
                        render: (_v, row) =>
                          row.estTraffic != null && row.estTraffic > 0 ? (
                            <span className="text-sm text-[var(--text)]">
                              {row.estTraffic.toFixed(2)}
                            </span>
                          ) : (
                            <span className="text-xs text-[var(--text-3)]">—</span>
                          ),
                      ...
                      },
                      {
                        ...
                        render: (_v, row) => {
                          ...
                          const ownedSerpFeats = row.serpFeatures.filter(
                            (f) =>
                              ownedFeats.includes(f) || (f === "org" && ownedFeats.includes("aio")),
                          );
                              ...
                              {ownedSerpFeats.map((f) => {
                                ...
                                if (isAio) {
                                  return (
                                    <span
                                      ...
                                      className="inline-flex items-center gap-1 rounded border border-purple-300 bg-gradient-to-r from-blue-50 via-purple-50 to-pink-50 px-2 py-1.5 text-[10px] font-semibold text-purple-700"
                                    >
                                      <svg
                                        width="9"
                                        height="9"
                                ...
                                return (
                                  <span
                                    ...
                                    className="inline-flex items-center gap-1 rounded bg-[var(--border-subtle)] px-2 py-1.5 text-[10px] font-medium text-[var(--text-1)] uppercase"
                                  >
                                    {f}
          ...
          {show("backlinks") && (backlinkError || backlinks.length > 0) && (
            <SectionCard
              ...
              subtitle="Top referring domains by authority score"
            >
              {backlinkError ? (
                <p className="py-2 text-sm text-red-600">{backlinkError}</p>
              ) : (
                <DataTable<Backlink>
                  data={backlinks}
                  exportable
                  ...
                  columns={[
                      ...
                      render: (_v, row) => {
                        ...
                        return (
                          <a
                            ...
                            className="block max-w-[180px] truncate font-medium text-[var(--text)] transition hover:text-indigo-600"
                          >
                            {sourceDomain}
                          </a>
                        );
                    ...
                    },
                    {
                      ...
                      render: (_v, row) =>
                        row.anchorText || (
                          <span className="text-[var(--text-3)] italic">No anchor</span>
                        ),
                    },
                    {
                      key: "authority",
          ...
          {show("ai_visibility") && (
            <SectionCard
              ...
              subtitle="How often your brand appears in Google AI Overviews for tracked keywords"
            >
              {!projectId ? (
                <p
                  style={{
                ...
                >
                  No SEMrush project linked — add a project ID in client settings to enable AI
                  visibility tracking.
                </p>
                ...
                >
                  No AI visibility data returned yet. This requires a SEMrush Position Tracking
                  project with AI Overview data enabled.
                </p>
              ) : (
                <>
                  <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
                    <div className="rounded-xl bg-[var(--border-subtle)] p-4 text-center">
                      <p className="mb-1 text-xs font-medium tracking-wide text-[var(--text-3)] uppercase">
                        AI Visibility Score
                      </p>
                      <p className="text-2xl font-bold text-indigo-600">
                        {aiVisibility.aiVisibilityScore.toFixed(1)}%
                      </p>
                      <p className="mt-0.5 text-xs text-[var(--text-3)]">of tracked keywords</p>
                    </div>
                    <div className="rounded-xl bg-[var(--border-subtle)] p-4 text-center">
                      <p className="mb-1 text-xs font-medium tracking-wide text-[var(--text-3)] uppercase">
                        Brand Citations
                      ...
                      <p className="text-2xl font-bold text-emerald-600">
                        {aiVisibility.brandCitations}
                      </p>
                      <p className="mt-0.5 text-xs text-[var(--text-3)]">in AI Overviews</p>
                    </div>
                    <div className="rounded-xl bg-[var(--border-subtle)] p-4 text-center">
                      <p className="mb-1 text-xs font-medium tracking-wide text-[var(--text-3)] uppercase">
                      ...
                      <p className="text-2xl font-bold text-blue-600">
                        {aiVisibility.aiOverviewKeywords}
                      </p>
                      <p className="mt-0.5 text-xs text-[var(--text-3)]">trigger AI Overviews</p>
                    </div>
                    <div className="rounded-xl bg-[var(--border-subtle)] p-4 text-center">
                      <p className="mb-1 text-xs font-medium tracking-wide text-[var(--text-3)] uppercase">
                        Total Tracked
                      ...
                      <p className="text-2xl font-bold text-[var(--text)]">
                        {aiVisibility.totalTracked}
                      </p>
                      <p className="mt-0.5 text-xs text-[var(--text-3)]">keywords monitored</p>
                    </div>
                  </div>
                  {aiVisibility.keywords.some((k) => k.hasAIOverview) && (
                    <div>
                      <p className="mb-2 text-xs font-semibold tracking-wide text-[var(--text-3)] uppercase">
                        Keywords with AI Overview presence
                      </p>
                      <DataTable<AIKeyword>
                        data={aiVisibility.keywords
                          ...
                          .sort(
                            (a, b) =>
                              (b.brandInAIOverview ? 1 : 0) - (a.brandInAIOverview ? 1 : 0) ||
          ...
          {show("competitors") && competitorRows.length > 0 && (
            <SectionCard
              ...
              subtitle={`Top organic competitors for ${domain}`}
            >
                ...
                columns={[
                  {
                    ...
                    render: (_v, row) => (
                      <div className="flex items-center gap-2">
                        {row.isClient ? (
                          <span className="inline-flex items-center rounded-full border border-emerald-300 bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-emerald-800 uppercase">
                            Client
                          </span>
                        ) : null}
                        <a
                          ...
                          className={
                            row.isClient
                              ? "font-semibold text-[var(--text)] underline decoration-emerald-500 decoration-2 underline-offset-4 transition hover:text-emerald-700"
                              : "font-medium text-[var(--text)] transition hover:text-indigo-600"
                          }
                        >
                          {row.domain}
                        </a>
                  ...
                  },
                  {
                    ...
                    sortable: true,
                    render: (_v, row) => (
                      <span className={row.isClient ? "font-bold text-[var(--text)]" : undefined}>
                        {formatNumber(row.organicKeywords)}
                      </span>
                    ),
                  },
                  {
                    ...
                    render: (_v, row) => (
                      <span className={row.isClient ? "font-bold text-[var(--text)]" : undefined}>
                        {formatNumber(row.organicTraffic)}
                      </span>
                    ),
                  },
                  {
                    ...
                    render: (_v, row) => (
                      <span className={row.isClient ? "font-bold text-[var(--text)]" : undefined}>
                        {formatCurrency(row.organicCost)}
                      </span>
                    ),
                  },
                ]}
                ...
                keywords.length > 0
                  ? [
                      ...
                      ...keywords.slice(0, 10).map((kw) => {
                        const delta =
                          kw.previousPosition > 0 ? kw.previousPosition - kw.position : null;
                        const deltaStr =
                          delta != null
                            ? delta > 0
                              ? ` (\u2191${delta})`
              ...
              extraContext={(() => {
                  ...
                  keywords.slice(0, 10).forEach((kw) => {
                    ...
                    const deltaStr =
                      delta != null
                        ...
                        : "";
                    parts.push(
                      `  • "${kw.keyword}" — pos ${kw.position}${deltaStr}, vol ${kw.searchVolume.toLocaleString()}, ${kw.trafficPercent.toFixed(1)}% traffic`,
                    );
                  });
                }
                ...
                if (taggedKeywords.length > 0) {
                  ...
                  if (aioOwned.length > 0) {
                    parts.push(
                      `AI Overview (AIO) appearances — ${aioOwned.length} keyword(s) where client is cited in Google's AI-generated answer:`,
                    );
                    aioOwned.slice(0, 8).forEach((kw) => {
                      const deltaStr =
                        kw.delta != null
                          ...
                          : "";
                      const sov =
                        kw.shareOfVoice != null ? `, SOV ${kw.shareOfVoice.toFixed(1)}%` : "";
                      parts.push(
                        `  • "${kw.keyword}" — pos ${kw.currentPosition}${deltaStr}, vol ${kw.searchVolume.toLocaleString()}${sov}`,
                      );
                    });
                  }
                  ...
                  if (fsnOwned.length > 0) {
                    ...
                    fsnOwned.slice(0, 5).forEach((kw) => {
                      parts.push(
                        `  • "${kw.keyword}" — pos ${kw.currentPosition}, vol ${kw.searchVolume.toLocaleString()}`,
                      );
                    });
                  }
                    ...
                    topByTraffic.forEach((kw) => {
                      const deltaStr =
                        kw.delta != null
                          ...
                          : "";
                      const owned =
                        kw.ownedFeatures.length > 0
                          ? ` [owns: ${kw.ownedFeatures.join(", ")}]`
                          : "";
                      parts.push(
                        `  • "${kw.keyword}" — pos ${kw.currentPosition}${deltaStr}, est. traffic ${kw.estTraffic?.toFixed(2)}, vol ${kw.searchVolume.toLocaleString()}${owned}`,
                      );
                    });
                  }

                  // Keywords with notable drops
                  const dropping = ranked
                    .filter((kw) => kw.delta != null && kw.delta < -3)
                  ...
                  if (dropping.length > 0) {
                    ...
                    dropping.forEach((kw) => {
                      parts.push(
                        `  • "${kw.keyword}" — pos ${kw.currentPosition} (↓${Math.abs(kw.delta ?? 0)}), vol ${kw.searchVolume.toLocaleString()}`,
                      );
                    });
                  }
          ...
          {show("content_gap") && contentGap.length > 0 && (
            <SectionCard
              ...
              subtitle="Keyword opportunities where competitors rank but you don't"
            >
              <DataTable<{
                keyword: string;
                volume: number;
          ...
          {show("serp_features") && serpFeatures.length > 0 && (
            <SectionCard
              title="SERP Features"
              subtitle="Distribution of SERP feature types for tracked keywords"
            >
          ...
          {show("backlink_changes") && backlinkChanges.length > 0 && (
            <SectionCard
              title="Recent Backlink Changes"
              subtitle="New and lost backlinks detected recently"
            >
              <DataTable<{
                url: string;
                type: string;
                ...
                columns={[
                  {
                    ...
                    render: (_v, row) => (
                      <a
                        ...
                        className="block max-w-[300px] overflow-hidden font-medium text-ellipsis whitespace-nowrap"
                      >
                        {row.url}
                      </a>
                    ),
          ...
          {show("topic_research") && topicResearch && (
            <SectionCard
              ...
              subtitle={`${topicResearch.topic} — volume ${formatNumber(topicResearch.volume)}, difficulty ${topicResearch.difficulty}`}
            >
              <div className="mb-4 grid grid-cols-3 gap-4">
                <div className="rounded-lg bg-[var(--border-subtle)] p-3 text-center">
                  <p className="text-[10px] tracking-wider text-[var(--text-3)] uppercase">
                    Volume
                  </p>
                  <p className="text-lg font-bold text-[var(--text)]">
                    {formatNumber(topicResearch.volume)}
                  </p>
                </div>
                <div className="rounded-lg bg-[var(--border-subtle)] p-3 text-center">
                  <p className="text-[10px] tracking-wider text-[var(--text-3)] uppercase">
                ...
                <div className="rounded-lg bg-[var(--border-subtle)] p-3 text-center">
                  ...
                  <p className="text-lg font-bold text-[var(--text)]">
                    {topicResearch.topicEfficiency.toFixed(1)}
                  </p>
                </div>
              </div>
                      ...
                      {st.questions && st.questions.length > 0 && (
                        <ul className="mt-1 space-y-0.5">
                          {st.questions.slice(0, 3).map((q, qi) => (
                            <li key={qi} className="text-[11px] text-[var(--text-3)]">
                              • {q}
                            </li>
                          ))}
                        </ul>
          ...
          {isExplicit("site_audit") && !siteAudit && (
            <EmptyBlockState
              title="Site Audit"
              message="No SEMrush Site Audit project configured for this domain."
            />
          )}
          {show("site_audit") && siteAudit && (
            <SectionCard
              title="Site Audit Summary"
              subtitle={`${formatNumber(siteAudit.totalPages)} pages crawled`}
            >
              <div className="mb-4 grid grid-cols-4 gap-4">
                <div className="rounded-lg bg-[var(--border-subtle)] p-3 text-center">
                  ...
                  <p
                    className={`text-lg font-bold ${siteAudit.healthScore >= 80 ? "text-emerald-600" : siteAudit.healthScore >= 50 ? "text-amber-600" : "text-red-600"}`}
                  >
                    {siteAudit.healthScore}%
                  </p>
                </div>
                <div className="rounded-lg bg-red-50 p-3 text-center">
                  <p className="text-[10px] tracking-wider text-red-500 uppercase">Errors</p>
                  <p className="text-lg font-bold text-red-600">{formatNumber(siteAudit.errors)}</p>
                </div>
                <div className="rounded-lg bg-amber-50 p-3 text-center">
                  <p className="text-[10px] tracking-wider text-amber-500 uppercase">Warnings</p>
                  <p className="text-lg font-bold text-amber-600">
                    {formatNumber(siteAudit.warnings)}
                  </p>
                </div>
                <div className="rounded-lg bg-[var(--border-subtle)] p-3 text-center">
                  <p className="text-[10px] tracking-wider text-[var(--text-3)] uppercase">
                  ...
                  <p className="text-lg font-bold text-[var(--text-2)]">
                    {formatNumber(siteAudit.notices)}
                  </p>
                </div>
              </div>
          ...
          {show("ad_copy_intelligence") && adCopyIntelligence.length > 0 && (
            <SectionCard
              title="Ad Copy Intelligence"
              subtitle={`${adCopyIntelligence.length} competitor ad${adCopyIntelligence.length !== 1 ? "s" : ""} detected`}
            >
              <div className="space-y-3">
                {adCopyIntelligence.slice(0, 15).map((ad, i) => (
                  <div key={i} className="rounded-lg border border-[var(--border-subtle)] p-3">
                    <p className="text-xs font-semibold text-indigo-600">{ad.title}</p>
                    <p className="mt-0.5 text-[11px] text-[var(--text-2)]">{ad.description}</p>
                    <div className="mt-1 flex items-center gap-3 text-[10px] text-[var(--text-3)]">
                      <span className="max-w-[200px] truncate">{ad.url}</span>
          ...
          {show("display_advertising") && displayAdvertising.length > 0 && (
            <SectionCard
              title="Display Advertising Competitors"
              subtitle={`${displayAdvertising.length} competitor${displayAdvertising.length !== 1 ? "s" : ""}`}
            >
              <DataTable<{
                domain: string;
                displayAds: number;
          ...
          {show("shopping_competitors") && shoppingCompetitors.length > 0 && (
            <SectionCard
              title="PLA / Shopping Competitors"
              subtitle={`${shoppingCompetitors.length} competitor${shoppingCompetitors.length !== 1 ? "s" : ""}`}
            >
              <DataTable<{
                domain: string;
                shoppingKeywords: number;
          ...
          {show("keyword_trends") && keywordTrends.length > 0 && (
            <SectionCard
              ...
              subtitle={`${keywordTrends.length} keyword${keywordTrends.length !== 1 ? "s" : ""} tracked`}
            >
              <DataTable<{
                keyword: string;
                searchVolume: number;
          ...
          {show("referring_domains") && referringDomains.length > 0 && (
            <SectionCard
              ...
              subtitle={`${referringDomains.length} domain${referringDomains.length !== 1 ? "s" : ""} linking`}
            >
              <DataTable<{
                domain: string;
                backlinks: number;
          ...
          {show("anchor_text") && anchorText.length > 0 && (
            <SectionCard
              ...
              subtitle={`${anchorText.length} unique anchor${anchorText.length !== 1 ? "s" : ""}`}
            >
              <DataTable<{
                anchor: string;
                domains: number;
          ...
          {show("backlink_comparison") && backlinkComparison.length > 0 && (
            <SectionCard
              title="Competitor Backlink Comparison"
              subtitle={`${backlinkComparison.length} domain${backlinkComparison.length !== 1 ? "s" : ""} compared`}
            >
              <DataTable<{
                domain: string;
                ascore: number;
          ...
          {show("position_changes") && positionChanges.length > 0 && (
            <SectionCard
              title="Organic Position Changes"
              subtitle={`${positionChanges.length} keyword${positionChanges.length !== 1 ? "s" : ""} with position changes`}
            >
              <DataTable<{
                keyword: string;
                previousPosition: number;
                  ...
                  },
                  {
                    ...
                    render: (_v, row) => (
                      ...
                      >
                        {row.change > 0 ? (
                          <>
                            <CssArrowUp /> +{row.change}
                          </>
                        ) : row.change < 0 ? (
                          <>
                            <CssArrowDown /> {row.change}
                          </>
                        ) : (
                          <>
                            <CssMinus /> 0
