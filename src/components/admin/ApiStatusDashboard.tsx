...
const INTEGRATIONS: Integration[] = [
  {
    ...
    clientCount: (d) =>
      Math.max(
        d.platformCounts.ga4 ?? 0,
        d.platformCounts.googleads ?? 0,
        d.platformCounts.searchconsole ?? 0,
      ),
    badge: (d) =>
      d.env.googleOAuth && d.googleConnections.count > 0
        ? {
            label: `${d.googleConnections.count} connected`,
  ...
  },
  {
    ...
    badge: (d) => {
      if (!d.semrush.configured)
        return { label: "Not configured", color: "var(--text-2)", bg: "#f3f4f6" };
      if (d.semrush.units === null)
        return { label: "API key set", color: "var(--info-text)", bg: "#dbeafe" };
      if (d.semrush.units === 0)
        return { label: "0 units — top up!", color: "var(--danger-text)", bg: "#fee2e2" };
      if (d.semrush.units < 1000)
        return {
          label: `${d.semrush.units.toLocaleString()} units left`,
          color: "#d97706",
          bg: "#fef3c7",
        };
      return {
        label: `${d.semrush.units.toLocaleString()} units`,
        color: "var(--success-text)",
        bg: "#dcfce7",
      };
    },
  ...
  },
  {
    ...
    badge: (d) => {
      ...
      if (d.openai.usage) {
        return {
          ...
          color:
            d.openai.usage.estimatedCostUsd > 50
              ? "#b91c1c"
              : d.openai.usage.estimatedCostUsd > 20
                ? "#d97706"
                : "#15803d",
          bg:
            d.openai.usage.estimatedCostUsd > 50
    ...
    detail: (d) => {
      ...
      if (!d.openai.usage)
        return "AI insights and anomaly commentary enabled across all clients. Live usage data requires a key with usage.read scope.";
  ...
  },
  {
    ...
    badge: (d) =>
      (d.platformCounts.tiktok ?? 0) > 0
        ? {
            label: `${d.platformCounts.tiktok} clients`,
            color: "var(--success-text)",
            bg: "#dcfce7",
          }
  ...
  },
  {
    ...
    badge: (d) =>
      (d.platformCounts.woocommerce ?? 0) > 0
        ? {
            label: `${d.platformCounts.woocommerce} clients`,
            color: "var(--success-text)",
            bg: "#dcfce7",
          }
        : { label: "No clients configured", color: "var(--text-2)", bg: "#f3f4f6" },
  ...
  },
  {
    ...
    badge: (d) =>
      (d.platformCounts.shopify ?? 0) > 0
        ? {
            label: `${d.platformCounts.shopify} clients`,
            color: "var(--success-text)",
            bg: "#dcfce7",
          }
        : { label: "No clients configured", color: "var(--text-2)", bg: "#f3f4f6" },
...
export function ApiStatusDashboard() {
  ...
  if (loading)
    return (
      <p style={{ fontSize: 13, color: "var(--text-3)", padding: "24px 0" }}>Loading API status…</p>
    );
  if (error)
    return <p style={{ fontSize: 13, color: "var(--danger)", padding: "24px 0" }}>{error}</p>;
  if (!data) return null;
        ...
        {[
          {
            ...
            color: configuredCount > 0 ? "#15803d" : "var(--text-3)",
          },
          { label: "Total Clients", value: data.totalClients, color: "var(--text)" },
          {
            label: "Google Accounts",
            value: data.googleConnections.count,
            color: data.googleConnections.count > 0 ? "#15803d" : "var(--text-3)",
          },
          {
            label: "SEMrush Units",
            value:
              data.semrush.units !== null
                ? data.semrush.units.toLocaleString()
                : data.semrush.configured
                  ? "—"
                  : "n/a",
            color:
              data.semrush.units === 0
                ? "#b91c1c"
                : data.semrush.units !== null && data.semrush.units < 1000
                  ? "#d97706"
                  : "#15803d",
          },
          {
            ...
            color:
              data.cronStats.lastRunStatus === "success"
                ? "#15803d"
                : data.cronStats.lastRunStatus === "error"
                  ? "#b91c1c"
                  : "var(--text-3)",
          },
          {
        ...
        <div key={category} style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {integrations.map((integration) => {
              ...
              const hasErrors = Object.entries(data.platformErrors).some(([k]) =>
                k.includes(
                  integration.id.replace("ads", "").replace("commerce", "").replace("search", ""),
                ),
              );
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {clientCount > 0 && (
                        <p style={{ fontSize: 12, color: "var(--text-3)", marginBottom: 4 }}>
                          Used by <strong style={{ color: "var(--text)" }}>{clientCount}</strong>{" "}
                        ...
                        <div style={{ marginTop: 14 }}>
                          ...
                          >
                            {[
                              {
                                label: "Input tokens",
                                value: data.openai.usage.totalInputTokens.toLocaleString(),
                              },
                              {
                                label: "Output tokens",
                                value: data.openai.usage.totalOutputTokens.toLocaleString(),
                              },
                              {
                                label: "Total tokens",
                                value: (
                                  data.openai.usage.totalInputTokens +
                                  data.openai.usage.totalOutputTokens
                                ).toLocaleString(),
                              },
                              {
                                label: "Requests",
                                value: data.openai.usage.totalRequests.toLocaleString(),
                              },
                              {
                                label: "Est. cost (USD)",
                                value: `$${data.openai.usage.estimatedCostUsd.toFixed(2)}`,
                                highlight: true,
                              },
                            ].map((s) => (
                              <div
                              ...
                              >
                                ...
                                <p
                                  style={{
                                    fontSize: 16,
                                    fontWeight: 700,
                                    color: s.highlight ? "#92400e" : "var(--text)",
                                  }}
                                >
                                  {s.value}
                                </p>
                                  ...
                                  >
                                    ...
                                    <td
                                      style={{
                                        ...
                                        fontWeight: 600,
                                        color:
                                          row.estimatedCostUsd > 10 ? "#92400e" : "var(--text)",
                                      }}
                                    >
                          ...
                          >
                            * Cost estimates based on published OpenAI list prices. Actual billing
                            may differ. Requires API key with <code>usage.read</code> scope.
                          </p>
                        </div>
                      )}

                      {/* SEMrush units breakdown — used vs remaining */}
                      {integration.id === "semrush" &&
                        data.semrush.configured &&
                        data.semrush.units !== null &&
                        (() => {
                          ...
                          const highWater = history.reduce(
                            (m, e) => Math.max(m, e.balance),
                            remaining,
                          );
                            ...
                            <div style={{ marginTop: 14 }}>
                                    ...
                                    (s) =>
                                      s && (
                                        ...
                                        >
                                          ...
                                          <p
                                            style={{
                                              fontSize: s.label === "Tracking since" ? 13 : 18,
                                              fontWeight: 700,
                                              color: s.colour,
                                            }}
                                          >
                              ...
                              {used > 0 && (
                                <div style={{ marginBottom: 10 }}>
                                  ...
                                  >
                                    <span>
                                      {used.toLocaleString()} used ({usedPct}%)
                                    </span>
                              ...
                              <p style={{ fontSize: 11, color: "var(--text-3)", marginTop: 4 }}>
                                ...
                                Top up at{" "}
                                <a
                                  href="https://www.semrush.com/billing/"
                                  target="_blank"
      ...
      {Object.keys(data.platformErrors).length > 0 && (
        <div className="card" style={{ marginTop: 8 }}>
          <div className="card-header">
            <div>
              <h2 className="card-title" style={{ color: "var(--danger-text)" }}>
                Recent Pipeline Errors
              </h2>
              <p className="card-subtitle">
                Error counts across last 5 cron runs, grouped by platform
              </p>
            </div>
          </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {Object.entries(data.platformErrors).map(([key, count]) => (
                ...
                >
                  ...
                  <Badge variant="destructive" className="px-2 py-0 text-[11px] font-semibold">
                    {count}×
                  </Badge>
                </div>
              ))}
