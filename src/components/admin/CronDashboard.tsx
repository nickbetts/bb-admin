...
const PLATFORM_ORDER = ["ga4", "googleads", "meta", "searchconsole", "seo", "tiktok", "microsoftads", "woocommerce", "shopify", "cwv"];
...
export function CronDashboard() {
  ...
  async function handleRunNow() {
    ...
    try {
      ...
      setRunResult(`Done — ${d.totalSnapshots ?? d.snapshotsNew ?? 0} new snapshots, ${d.totalSkipped ?? d.snapshotsSkipped ?? 0} skipped, ${d.totalErrors ?? d.errors ?? 0} errors`);
      await load();
    } catch (e) {
  ...
  if (loading && !data) return <p style={{ fontSize: 13, color: "var(--text-3)", padding: 24 }}>Loading…</p>;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
        ...
        <div className="card" style={{ padding: "16px 20px" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Next Run</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "var(--text)", lineHeight: 1 }}>{formatCountdown(countdown)}</div>
          <div style={{ fontSize: 12, color: "var(--text-3)", marginTop: 4 }}>{data.schedule.description}</div>
        </div>

        {/* Last run */}
        <div className="card" style={{ padding: "16px 20px" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Last Run</div>
          {lastRun ? (
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {statusBadge(lastRun.status)}
                <span style={{ fontSize: 13, color: "var(--text-2)" }}>{formatRelative(lastRun.startedAt)}</span>
              </div>
        ...
        {/* Totals */}
        <div className="card" style={{ padding: "16px 20px" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Stored</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "var(--text)", lineHeight: 1 }}>{data.totals.snapshotsStored.toLocaleString()}</div>
          <div style={{ fontSize: 12, color: "var(--text-3)", marginTop: 4 }}>snapshots · {data.totals.clients} clients</div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        ...
        <button onClick={load} disabled={loading} style={{ fontSize: 12, color: "var(--text-3)", background: "none", border: "none", cursor: "pointer", padding: 0 }}>
          {loading ? "Refreshing…" : "↻ Refresh"}
        </button>
        {runResult && (
          <span style={{ fontSize: 13, color: runResult.startsWith("Error") ? "var(--danger)" : "#16a34a" }}>{runResult}</span>
        )}
      </div>

      {/* ── API Quota Notes ─────────────────────────────────────────────────── */}
      <div className="card" style={{ padding: "14px 20px" }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text-2)", marginBottom: 8 }}>API Rate Limit Notes</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 12, color: "var(--text-3)" }}>
          <span><span style={{ fontWeight: 600, color: "var(--text-2)" }}>SEMrush</span> — unit-based quota. Balance zero = 402 error. Check SEMrush account. Cron skips SEMrush on 402.</span>
          <span><span style={{ fontWeight: 600, color: "var(--text-2)" }}>Google Ads</span> — 15,000 operations/day. Monthly snapshots use ~2 ops per client.</span>
      ...
      {data.recentRuns.length > 0 && (
        <div className="card">
          <div className="card-header" style={{ padding: "14px 20px 0" }}>
            <h3 className="card-title" style={{ fontSize: 14 }}>Recent Runs</h3>
          </div>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border-subtle)" }}>
                  {["Started", "By", "Status", "Duration", "New", "Skipped", "Errors", "Clients", ""].map((h) => (
                    <th key={h} style={{ textAlign: "left", padding: "8px 12px", fontWeight: 600, color: "var(--text-2)", whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.recentRuns.map((run) => {
                  const details = run.details ? (() => { try { return JSON.parse(run.details!); } catch { return []; } })() : [];
                  const hasDetails = Array.isArray(details) && details.length > 0;
                  return (
                    <>
                      <tr key={run.id} style={{ borderBottom: "1px solid var(--border-subtle)" }}>
                        <td style={{ padding: "8px 12px", whiteSpace: "nowrap" }}>
                          <span title={new Date(run.startedAt).toLocaleString()}>{formatRelative(run.startedAt)}</span>
                        </td>
                        ...
                        <td style={{ padding: "8px 12px" }}>
                          {hasDetails && (
                            <button
                              ...
                              style={{ fontSize: 11, color: "var(--primary, #6366f1)", background: "none", border: "none", cursor: "pointer", padding: 0 }}
                            >
                      ...
                      {expandedRun === run.id && hasDetails && (
                        <tr key={`${run.id}-expand`}>
                          <td colSpan={9} style={{ padding: "0 12px 12px", background: "var(--bg-subtle, #f9fafb)" }}>
                            <div style={{ display: "flex", flexDirection: "column", gap: 2, maxHeight: 240, overflowY: "auto", paddingTop: 8 }}>
                              {(details as Array<{ clientName: string; sections: string[]; skipped: string[]; errors: string[] }>)
            ...
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border-subtle)", background: "var(--bg-subtle, #f9fafb)" }}>
                ...
                {activePlatforms.map((key) => (
                  <th key={key} style={{ textAlign: "center", padding: "8px 8px", fontWeight: 600, color: "var(--text-2)", whiteSpace: "nowrap", fontSize: 11 }}>
                    {data.coverage[0]?.platforms[key]?.label ?? key}
                  </th>
                ))}
                <th style={{ textAlign: "center", padding: "8px 12px", fontWeight: 600, color: "var(--text-2)", whiteSpace: "nowrap" }}>Coverage</th>
              </tr>
            </thead>
            <tbody>
              {data.coverage.map((client, i) => (
                <tr key={client.clientId} style={{ borderBottom: i < data.coverage.length - 1 ? "1px solid var(--border-subtle)" : "none" }}>
                  ...
                  {activePlatforms.map((key) => {
                    ...
                    if (!p?.configured) {
                      return <td key={key} style={{ textAlign: "center", padding: "7px 8px", color: "var(--border-muted, #d1d5db)" }}>—</td>;
                    }
                  ...
                  <td style={{ textAlign: "center", padding: "7px 12px" }}>
                    <span style={{ fontSize: 11, fontWeight: 600, color: client.coveredCount === client.configuredCount && client.configuredCount > 0 ? "#16a34a" : "var(--text-3)" }}>
