              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    textAlign: "left",
                    fontSize: 11,
                    fontWeight: isCardVisible ? 500 : 400,
                    transition: "all 0.15s",
                    width: "100%",
                  }}
                >
                  {isCardVisible ? (
                    ...
                    <EyeOff size={10} style={{ flexShrink: 0 }} />
                  )}
                  {card.label}
                </button>
              );
      ...
      {isExpanded && isEnabled && (
        <div style={{ padding: "4px 16px 12px", display: "flex", flexDirection: "column", gap: 2 }}>
          ...
          <button
            ...
            title={
              pageBreakBefore
                ? "Remove forced page break before this section"
                : "Force a new page before this section in PDFs"
            }
            style={{
              display: "flex",
              alignItems: "center",
              ...
              >
                {subSections.map((sub) => (
                  <SortableBlockItem
                    ...
                    block={{
                      id: sub.id,
                      label: TEXT_SECTION_LABELS[sub.sectionType as TextSectionType] ?? sub.title,
                    }}
...
export function ReportView({ report: initialReport }: ReportViewProps) {
  ...
  const [sectionPreviousMetrics, setSectionPreviousMetrics] = useState<
  ...
  const [narrativeResult, setNarrativeResult] = useState<NarrativeResult | null>(() => {
    if (initialReport.narrativeData) {
      try {
        return JSON.parse(initialReport.narrativeData);
      } catch {
        /* ignore */
      }
    }
  ...
  const mainContentSensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: { distance: 5 } }),
  );
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      const reordered = arrayMove(report.sections, oldIndex, newIndex).map((s, i) => ({
        ...s,
        orderIndex: i,
      }));
  ...
  const getVisibleBlocks = (section: Section): string[] | undefined => {
    ...
    try {
      const parsed = JSON.parse(section.cardConfig) as {
      ...
      const visibleSet =
        parsed.visibleBlocks && parsed.visibleBlocks.length > 0
      ...
      if (parsed.blockOrder && parsed.blockOrder.length > 0) {
        ...
        const remaining = allBlockIds.filter(
          (b) => !parsed.blockOrder?.includes(b) && (!visibleSet || visibleSet.has(b)),
        );
      ...
      return parsed.visibleBlocks && parsed.visibleBlocks.length > 0
        ? parsed.visibleBlocks
        : undefined;
    } catch {
      return undefined;
    }
  ...
  const getHiddenCards = (section: Section): Record<string, string[]> | undefined => {
    ...
    try {
      ...
      return parsed.hiddenCards && Object.keys(parsed.hiddenCards).length > 0
        ? parsed.hiddenCards
        : undefined;
    } catch {
      return undefined;
  ...
  const filterSectionMetricsForPreflight = (
    ...
    const filtered = Object.entries(metrics).reduce<Record<string, number>>(
      (acc, [metricKey, value]) => {
  ...
  const getFilteredSectionPreviousMetrics = (
    section: Section,
  ): Record<string, number> | undefined =>
  ...
  const handleToggleBlock = async (sectionId: string, blockId: string) => {
    ...
    try {
      if (section.cardConfig) existing = JSON.parse(section.cardConfig);
    } catch {
      /* ignore */
    }
    const currentBlocks =
      existing.visibleBlocks && existing.visibleBlocks.length > 0
        ? [...existing.visibleBlocks]
        : [...allBlocks];
    const newBlocks = currentBlocks.includes(blockId)
      ? currentBlocks.filter((b) => b !== blockId)
      : [...currentBlocks, blockId];
    const newCardConfig =
      newBlocks.length === allBlocks.length && !existing.blockOrder
        ? null
        : JSON.stringify({
            ...existing,
            visibleBlocks: newBlocks.length === allBlocks.length ? undefined : newBlocks,
          });
    setReport((prev) => ({
      ...prev,
      sections: prev.sections.map((s) =>
        s.id === sectionId ? { ...s, cardConfig: newCardConfig } : s,
      ),
    }));
    await fetch(`/api/reports/${report.id}/sections`, {
      method: "PATCH",
  ...
  const handleReorderBlocks = useCallback(
    async (sectionId: string, newBlockOrder: string[]) => {
      const section = report.sections.find((s) => s.id === sectionId);
      if (!section) return;
      let existing: { visibleBlocks?: string[]; blockOrder?: string[] } = {};
      try {
  ...
  const handleToggleCard = useCallback(
    async (sectionId: string, blockId: string, cardId: string) => {
      const section = report.sections.find((s) => s.id === sectionId);
      if (!section) return;
      let existing: {
        visibleBlocks?: string[];
  ...
  const handleResetCardConfig = useCallback(
    async (sectionId: string) => {
      const section = report.sections.find((s) => s.id === sectionId);
      ...
      if (
        !(await confirm({
          ...
          description:
            "This restores default block visibility, ordering, and page-break settings. Commentary and AI insights are not affected.",
          confirmLabel: "Reset",
        }))
      )
        return;
  ...
  const handleSaveSection = async (sectionId: string) => {
    ...
    if (section?.sectionType === "overview" && narrativeDraft) {
      ...
      try {
        ...
        if (!normalisedNarrative.crossSectionStories?.length)
          delete normalisedNarrative.crossSectionStories;
    ...
    if (res.ok) {
      ...
      setReport((prev) => ({
        ...
        screenshots: prev.screenshots.map((s) =>
          s.id === screenshotId ? { ...s, caption: updated.caption } : s,
        ),
      }));
    }
      ...
      try {
        ...
        if (res.ok) {
          ...
          setReport((prev) => ({
            ...prev,
            sections: prev.sections.map((s) =>
              s.id === section.id ? { ...s, commentary: text } : s,
            ),
          }));
        }
      } catch {
  ...
  const buildMergedGenerationContext = useCallback(
    (questions: PreflightQuestion[], answers: Record<string, string>) => {
      ...
      if (clarified.length > 0) {
        contextParts.push(
          `Preflight clarification notes (account manager provided, use all relevant points):\n${clarified
            .map(
              (item, index) =>
                `${index + 1}. Question: ${item.q.question}\n   Answer: ${item.answer}`,
            )
            .join("\n\n")}`,
        );
      }
  ...
  const handleStartPreflightQuestions = async () => {
    ...
    try {
      ...
      toast(
        `Preflight check failed: ${err instanceof Error ? err.message : "unknown error"}`,
        "error",
      );
    } finally {
  ...
  const handleGenerateCombined = async (contextOverride?: string) => {
    ...
    for (const s of report.sections) {
      ...
      const text =
        allCommentaries[s.sectionType] ?? (s.commentary?.trim() ? s.commentary : undefined);
  ...
  const handleDeleteScreenshot = async (screenshotId: string) => {
    const res = await fetch(`/api/reports/${report.id}/screenshots?screenshotId=${screenshotId}`, {
      method: "DELETE",
    });
    if (res.ok) {
      setReport((prev) => ({
  ...
  const SECTION_META: Record<string, { icon: React.ReactNode; badge: string; subtitle?: string }> =
    {
      overview: {
        ...
        badge: "badge-slate",
        subtitle:
          "A high-level snapshot of performance across all active channels for the period, highlighting key wins, areas of concern and overall trajectory.",
      },
      executive_summary: {
        icon: <Star size={14} />,
        badge: "badge-amber",
        subtitle:
          "An AI-generated summary of the full report, pulling together the key takeaways from every channel into a concise overview for stakeholders.",
      },
      seo: {
        icon: <TrendingUp size={14} />,
        badge: "badge-indigo",
        subtitle:
          "Covers organic search performance from Search Console, including clicks, impressions, CTR, average position, top pages and top queries.",
      },
      web: {
        icon: <Globe size={14} />,
        badge: "badge-blue",
        subtitle:
          "Shows website traffic performance including sessions, users, engagement rate, average session duration and conversions, alongside channel breakdowns and a sessions-over-time chart.",
      },
      paid_social: {
        icon: <BarChart2 size={14} />,
        badge: "badge-orange",
        subtitle:
          "Covers paid social advertising performance including spend, impressions, reach, clicks, conversions and cost metrics, with trend charts and campaign-level breakdowns.",
      },
      googleads: {
        icon: <Search size={14} />,
        badge: "badge-green",
        subtitle:
          "Shows paid search performance including spend, impressions, clicks, conversions and cost metrics, with trend charts and campaign-level breakdowns.",
      },
      searchconsole: {
        icon: <Search size={14} />,
        badge: "badge-purple",
        subtitle:
          "Covers organic search performance including clicks, impressions, click-through rate and average position, plus top pages, top queries, position movers and device breakdowns.",
      },
      ...
      ecommerce: {
        ...
        badge: "badge-emerald",
        subtitle:
          "Shows online store performance including revenue, orders, average order value and conversion rate, with breakdowns by product and channel.",
      },
      tiktok: {
        icon: <Video size={14} />,
        badge: "badge-pink",
        subtitle:
          "Covers TikTok paid advertising performance including spend, impressions, video views, clicks and conversions, with campaign-level breakdowns.",
      },
      microsoft_ads: {
        icon: <Search size={14} />,
        badge: "badge-cyan",
        subtitle:
          "Shows Microsoft Advertising (Bing Ads) performance including spend, clicks, impressions, conversions and ROAS, with campaign-level breakdowns.",
      },
      linkedin: {
        icon: <BarChart2 size={14} />,
        badge: "badge-blue",
        subtitle:
          "Covers LinkedIn paid campaign performance including impressions, clicks, spend, conversions and cost per lead, with campaign-level breakdowns.",
      },
      klaviyo: {
        icon: <CalendarRange size={14} />,
        badge: "badge-violet",
        subtitle:
          "Shows email marketing performance including sends, open rate, click rate and revenue attributed, with individual campaign breakdowns.",
      },
      goals: {
        icon: <TrendingUp size={14} />,
        badge: "badge-amber",
        subtitle:
          "Tracks progress against client goals and targets across all channels, highlighting what's on track, at risk or achieved.",
      },
      youtube: {
        icon: <Play size={14} />,
        badge: "badge-red",
        subtitle:
          "Shows YouTube channel performance including views, watch time, subscriber growth and top-performing videos for the period.",
      },
      hubspot: {
        icon: <Users size={14} />,
        badge: "badge-orange",
        subtitle:
          "Covers CRM pipeline health including open deals, pipeline value, closed revenue and recent contact activity.",
      },
      callrail: {
        icon: <Phone size={14} />,
        badge: "badge-teal",
        subtitle:
          "Shows call tracking performance including total calls, answer rate, average call duration and attribution by source.",
      },
      core_web_vitals: {
        icon: <Globe size={14} />,
        badge: "badge-green",
  ...
  const shareUrl = report.shareToken
    ? `${typeof window !== "undefined" ? window.location.origin : ""}/share/report/${report.shareToken}`
    : null;
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      {pendingUpload && (
        <ScreenshotCaptionDialog
          ...
          uploading={
            pendingUpload.sectionId ? uploadingSectionId === pendingUpload.sectionId : uploading
          }
          onUpload={(caption) =>
            handleUploadScreenshot(pendingUpload.file, caption, pendingUpload.sectionId)
          }
      ...
      {/* ── Top bar ─────────────────────────────────────────────────────── */}
      <div
        className="print-hidden print:hidden"
        style={{
          position: "sticky",
          top: 0,
          zIndex: 20,
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, position: "relative" }}>
          ...
          {report.shareToken ? (
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <button
                ...
                style={{
                  gap: 5,
                  color: shareCopied ? "#10b981" : undefined,
                  ...(isChaosTone ? { opacity: 0.4, cursor: "not-allowed" } : {}),
                }}
                title={
                  isChaosTone
                    ? "🚫 Sharing disabled — imagine sending THIS to a client"
          ...
          <div style={{ position: "relative" }}>
            <button
              onClick={() => {
                ...
                setDpStart(startDate);
                setDpEnd(endDate);
                setDpCompareStart(compareStartDate ?? "");
                setDpCompareEnd(compareEndDate ?? "");
              }}
              className="btn btn-secondary btn-sm"
            ...
            {datePickerOpen && (
              ...
              >
                <p
                  style={{ fontWeight: 600, fontSize: 13, color: "var(--text)", marginBottom: 14 }}
                >
                  Custom Date Range
                </p>
                ...
                <p style={{ fontSize: 12, color: "var(--text-4)", marginBottom: 10 }}>
                  Leave blank to use previous period automatically.
                </p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
                  {(() => {
                    ...
                    if (dpStart && dpEnd && !isNaN(s.getTime()) && !isNaN(e.getTime())) {
                      ...
                      const lyStart = new Date(s);
                      lyStart.setFullYear(lyStart.getFullYear() - 1);
                      const lyEnd = new Date(e);
                      ...
                      if (
                        mlyStart.getTime() !== lyStart.getTime() ||
                        mlyEnd.getTime() !== lyEnd.getTime()
                      ) {
                        presets.push({
                      ...
                      <>
                        {presets.map((p) => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              setDpCompareStart(p.from);
                              setDpCompareEnd(p.to);
                            }}
                ...
                >
                  <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    ...
                    <input
                      type="date"
                      value={dpCompareStart}
                      onChange={(e) => setDpCompareStart(e.target.value)}
                      style={{
                        padding: "6px 10px",
                        borderRadius: "var(--r-sm)",
                        border: "1px solid var(--border)",
                  ...
                  <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    ...
                    <input
                      type="date"
                      value={dpCompareEnd}
                      onChange={(e) => setDpCompareEnd(e.target.value)}
                      style={{
                        padding: "6px 10px",
                        borderRadius: "var(--r-sm)",
                        border: "1px solid var(--border)",
          ...
          <button
            ...
            title={
              isChaosTone
                ? "🚫 Export disabled — switch to a sensible tone first, yeah?"
                : undefined
            }
      ...
      {isChaosTone && (
        ...
        >
          ...
          <p style={{ fontSize: 12, fontWeight: 600, color: "var(--warning-text)" }}>
            INTERNAL PREVIEW ONLY — chaos tone active. Export PDF and Share are disabled until you
            switch back to a sensible tone.
          </p>
        </div>
      )}
      <div style={{ display: "flex", alignItems: "flex-start" }}>
              ...
              {enabledSections.map((section) => {
                        ...
                        {autosaveStatus[section.id] && (
                          <span
                            style={{
                              ...
                              gap: 3,
                              color:
                                autosaveStatus[section.id] === "saved"
                                  ? "#10b981"
                                  : "var(--text-4)",
                            }}
                          >
                      ...
                      {editingSection === section.id ? (
                        section.sectionType === "overview" ? (
                          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                              ...
                              <textarea
                                ...
                                onChange={(e) =>
                                  setNarrativeDraft((prev) =>
                                    prev ? { ...prev, executiveSummary: e.target.value } : prev,
                                  )
                                }
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                              <p style={{ fontSize: 12, fontWeight: 600, color: "var(--text-2)" }}>
                                Theme pills (one per line)
                              </p>
                              <textarea
                                value={narrativeDraft?.keyThemesText ?? ""}
                                onChange={(e) =>
                                  setNarrativeDraft((prev) =>
                                    prev ? { ...prev, keyThemesText: e.target.value } : prev,
                                  )
                                }
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                                    ...
                                    setNarrativeDraft((prev) =>
                                      prev
                                        ? {
                                            ...prev,
                                            crossSectionStories: [
                                              ...prev.crossSectionStories,
                                              { sectionsText: "", narrative: "" },
                                            ],
                                          }
                                        : prev,
                                    )
                              ...
                              {(narrativeDraft?.crossSectionStories ?? []).length === 0 && (
                                ...
                                >
                                  No cross-channel stories yet. Click &quot;Add Story&quot; to
                                  create one.
                                </p>
                              )}

                              {(narrativeDraft?.crossSectionStories ?? []).map(
                                (story, storyIndex) => (
                                  <div
                                    key={storyIndex}
                                    style={{
                                          ...
                                          setNarrativeDraft((prev) =>
                                            prev
                                              ? {
                                                  ...prev,
                                                  crossSectionStories:
                                                    prev.crossSectionStories.filter(
                                                      (_, i) => i !== storyIndex,
                                                    ),
                                                }
                                              : prev,
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setCommentary((prev) => ({ ...prev, [section.id]: e.target.value }))
                              }
                            ...
                            <>
                              {section.sectionType === "overview" && narrativeResult ? (
                                ...
                                >
                                  ...
                                  {narrativeResult.keyThemes &&
                                    narrativeResult.keyThemes.length > 0 && (
                                  ...
                                  {narrativeResult.crossSectionStories &&
                                    narrativeResult.crossSectionStories.length > 0 && (
                                      <div
                                ...
                                >
                                  No commentary added yet — click &quot;Commentary&quot; to add
                                  insights.
                                </p>
                              )}
                                        ...
                                        {editingCaptionId === ss.id ? (
                                          ...
                                          >
                                            <input
                                              ...
                                              onKeyDown={(e) => {
                                                if (e.key === "Enter")
                                                  handleUpdateCaption(ss.id, captionEditValue);
                                          ...
                                          >
                                            ...
                                            <button
                                              onClick={() => {
                                                setEditingCaptionId(ss.id);
                                                setCaptionEditValue(ss.caption ?? "");
                                              }}
                                              style={{
                                                background: "none",
                                                border: "none",
                        ...
                        <div className="card">
                          ...
                          <div className="card-body" style={{ padding: "20px 28px" }}>
                            {editingSection === section.id ? (
                              ...
                              >
                                No executive summary yet — click &quot;Generate Summary&quot; to
                                auto-create one from all section commentaries, or &quot;Edit&quot;
                                to write manually.
                              </p>
                            )}
                          </div>
                ...
                if (section.sectionType === "overview") {
                  const textSubSections = report.sections.filter(
                    (s) =>
                      s.enabled !== false &&
                      isTextSection(s.sectionType) &&
                      s.sectionType !== "text_screenshots",
                  );
                  return (
                    <SortableMainSectionWrapper
                      key={section.id}
                      ...
                      {section.sectionType === "seo" &&
                        (report.client.searchConsoleSiteUrl ? (
                          <SearchConsoleSection
                            ...
                            afterHeader={sectionAfterHeader}
                            onMetricsReady={(m) =>
                              setSectionMetrics((p) => ({ ...p, [section.id]: m }))
                            }
                            onPreviousMetricsReady={(m) =>
                              setSectionPreviousMetrics((p) => ({ ...p, [section.id]: m }))
                            }
                          />
                        ) : (
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No Search Console property connected — configure it in client settings to enable SEO data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "web" &&
                        (report.client.ga4PropertyId ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No GA4 property connected — configure it in client settings to enable web analytics.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "paid_social" &&
                        (report.client.metaAccountId ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No Meta ad account connected — configure it in client settings to enable paid social data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "googleads" &&
                        (report.client.googleAdsCustomerId ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No Google Ads account connected — configure it in client settings to enable ads data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "searchconsole" &&
                        (report.client.searchConsoleSiteUrl ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No Search Console property connected — configure it in client settings to enable search data.",
                            )}
                          </>
                        ))}
                        ...
                        (report.client.woocommerceUrl || report.client.shopifyStoreDomain ? (
                          <>
                            ...
                            <EcommerceSection
                              clientId={report.client.id}
                              platform={
                                report.client.shopifyStoreDomain ? "shopify" : "woocommerce"
                              }
                              startDate={startDate}
                              endDate={endDate}
                              visibleBlocks={visibleBlocks}
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No WooCommerce or Shopify store connected — configure it in client settings to enable e-commerce data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "tiktok" &&
                        (report.client.tiktokAdvertiserId ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No TikTok advertiser account connected — configure it in client settings to enable TikTok Ads data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "microsoft_ads" &&
                        (report.client.microsoftAdsAccountId ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No Microsoft Ads account connected — configure it in client settings to enable Microsoft Advertising data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "linkedin" &&
                        (report.client.linkedinAccountId ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No LinkedIn ad account connected — configure it in client settings to enable LinkedIn Ads data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "klaviyo" &&
                        (report.client.klaviyoApiKey ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No Klaviyo account connected — configure it in client settings to enable email marketing data.",
                            )}
                          </>
                        ))}
                      ...
                      {section.sectionType === "youtube" &&
                        (report.client.youtubeChannelId ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No YouTube channel connected — configure it in client settings to enable YouTube analytics data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "hubspot" &&
                        (report.client.hubspotPortalId ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No HubSpot portal connected — configure it in client settings to enable CRM data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "callrail" &&
                        (report.client.callrailAccountId ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No CallRail account connected — configure it in client settings to enable call tracking data.",
                            )}
                          </>
                        ))}
                      {section.sectionType === "core_web_vitals" &&
                        (report.client.website ? (
                          ...
                          <>
                            {commentaryCard}
                            {unconfiguredNotice(
                              "No website URL set for this client — configure it in client settings to enable Core Web Vitals data.",
                            )}
                          </>
                        ))}
                    </div>
                        ...
                        {editingCaptionId === screenshot.id ? (
                          <div style={{ display: "flex", gap: 6, padding: "8px 10px" }}>
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === "Enter")
                                  handleUpdateCaption(screenshot.id, captionEditValue);
                                if (e.key === "Escape") setEditingCaptionId(null);
                              }}
                              placeholder="Add caption…"
                              style={{
                            ...
                            />
                            <button
                              onClick={() => handleUpdateCaption(screenshot.id, captionEditValue)}
                              style={{
                                background: "none",
                                border: "none",
                                cursor: "pointer",
                          ...
                          >
                            ...
                            <button
                              onClick={() => {
                                ...
                                setCaptionEditValue(screenshot.caption ?? "");
                              }}
                              style={{
                                background: "none",
                                border: "none",
        ...
        />
        <aside
          className={`print:hidden report-builder-sidebar${mobileDrawerOpen ? "is-open" : ""}`}
          style={{
            width: 300,
            flexShrink: 0,
            position: "sticky",
            ...
            >
              <SortableContext
                items={report.sections
                  .filter((s) => !isTextSection(s.sectionType))
                  .map((s) => s.id)}
                strategy={verticalListSortingStrategy}
              >
                {report.sections
                    ...
                    return (
                      <>
                        <SortableSectionItem
                          ...
                          hiddenCards={getHiddenCards(section)}
                          onToggleCard={(blockId, cardId) =>
                            handleToggleCard(section.id, blockId, cardId)
                          }
                ...
                <>
                  <button
                    ...
                    style={{
                      ...
                      justifyContent: "center",
                      gap: 6,
                      marginBottom: generatingAll || generatingNarrative ? 6 : 0,
                    }}
                    ...
                    >
                      <div
                        style={{
                          ...
                          width:
                            generateAllTotal > 0
                              ? `${(generateAllProgress / generateAllTotal) * 100}%`
                              : "0%",
                    ...
                    >
                          ...
                          <>
                            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                              <span className="badge badge-indigo">
                                {preflightQuestions.length} question
                                {preflightQuestions.length === 1 ? "" : "s"}
                              </span>
                            ...
                            >
                              ...
                              >
                                Guidance only: these AI hints are hypotheses from visible report
                                data and context. They may be incomplete and do not reflect all live
                                campaign activity or actions already taken by the team.
                              </p>
                            </div>
                          ...
                          <>
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  The AI always runs a preflight check before generating report
                                  ...
                                  {(["positive", "balanced", "neutral"] as const).map((s) => (
                                    <button
                                      ...
                                      style={{
                                        ...
                                        fontSize: 12,
                                        background:
                                          aiSpin === s ? "var(--accent)" : "var(--surface-2)",
                      ...
                      <div
                        style={{
                          ...
                          background: "linear-gradient(to top, var(--surface-2), rgba(0,0,0,0))",
                          position: "sticky",
                          bottom: 0,
                        }}
                      >
              ...
              {!templateSaveOpen ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  <p style={{ fontSize: 11, color: "var(--text-4)", lineHeight: 1.4 }}>
                    Saves all {report.sections.length} sections with their current show/hide
                    configuration.
                  </p>
